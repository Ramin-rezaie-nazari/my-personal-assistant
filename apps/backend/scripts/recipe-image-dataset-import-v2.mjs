...
async function resetState() {
  ...
  for (let offset = 0; ; offset += 1000) {
    const r = await fetch(`${SUPABASE_URL}/storage/v1/object/list/${BUCKET}`, { method: 'POST', headers: { ...authHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ prefix: 'recipes', limit: 1000, offset, sortBy: { column: 'name', order: 'asc' } }) });
    const text = await r.text();
    if (!r.ok) throw new Error(`Storage list failed: ${r.status} ${text}`);
    const page = JSON.parse(text);
    for (const item of page) if (item?.name) files.push(`recipes/${item.name}`);
...
function imageIndex(root) {
  const stack = [root]; const index = new Map(); let count = 0; const exts = new Set(['.jpg', '.jpeg', '.png', '.webp']);
  while (stack.length) {
    const dir = stack.pop();
...
async function sourceRows() {
  ...
  for (let offset = 0; ; offset += 1000) {
    ...
    rows.push(...(page || [])); if (!page || page.length < 1000) break;
  }
  return rows;
}
async function existingIds() {
  ...
  for (let offset = 0; ; offset += 1000) {
    ...
    for (const row of page || []) ids.add(row.recipe_id); if (!page || page.length < 1000) break;
  }
  return ids;
}
...
async function compress(input) {
  for (const width of [960, 880, 800, 720, 640, 576, 512, 448, 384, 320]) for (const quality of [76, 70, 64, 58, 52, 46, 40, 34, 28]) {
...
async function importOne(row, file) {
  const input = await readFile(file); const c = await compress(input); const key = `recipes/${row.recipe_id}/hero.webp`; const url = `${SUPABASE_URL}/storage/v1/object/public/${BUCKET}/${key.split('/').map(encodeURIComponent).join('/')}`;
  ...
  const uploadText = await upload.text(); if (!upload.ok) throw new Error(`Storage upload ${upload.status}: ${uploadText}`);
  try {
...
async function main() {
  await ensureBucket(); if (RESET) await resetState();
  const root = datasetRoot(); const { index, count } = imageIndex(root); const rows = await sourceRows(); const done = await existingIds(); const work = rows.filter((row) => row.image_name && !done.has(row.recipe_id));
