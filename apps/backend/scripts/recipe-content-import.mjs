...
function normalize(value = '') {
  return clean(value).toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9\u0600-\u06ff]+/gi, ' ').replace(/\s+/g, ' ').trim();
}
...
function parseRecipe(row) {
  ...
  const description = paragraphs.find(Boolean) ?? null;
  const sourceUrl = data.url || (title ? `https://en.wikibooks.org/wiki/Cookbook:${encodeURIComponent(title.replace(/ /g, '_'))}` : null);
  return { title, description, servings: parseServings(data.infobox?.servings), sourceUrl, ingredients, steps };
}
...
async function getOrCreateFood(name) {
  const existing = await prisma.foodItem.findFirst({ where: { userId: null, name } });
  if (existing) return existing;
  return prisma.foodItem.create({ data: { id: randomUUID(), userId: null, name, category: 'recipe ingredient', verified: false, calories: 0, protein: 0, carbs: 0, fat: 0 } });
}
async function importRecipe(parsed) {
  ...
  const recipe = existing
    ? await prisma.recipe.update({ where: { id: existing.id }, data: { description: parsed.description, servings: parsed.servings, verified: false, imageSource: DATASET_SOURCE } })
    : await prisma.recipe.create({ data: { id: randomUUID(), userId: null, name: parsed.title, description: parsed.description, servings: parsed.servings, calories: 0, protein: 0, carbs: 0, fat: 0, verified: false, imageSource: DATASET_SOURCE } });

  await prisma.$transaction(async (tx) => {
    await tx.recipeStep.deleteMany({ where: { recipeId: recipe.id } });
    await tx.recipeIngredient.deleteMany({ where: { recipeId: recipe.id } });
    for (const ingredient of parsed.ingredients) {
      const food = await getOrCreateFood(ingredient.name);
...
async function main() {
  ...
  const limit = process.env.RECIPE_IMPORT_LIMIT ? Math.max(1, Number(process.env.RECIPE_IMPORT_LIMIT)) : dataset.length;
  ...
  console.log(JSON.stringify({ source: DATASET_SOURCE, license: DATASET_LICENSE, offset, limit, ...stats }, null, 2));
