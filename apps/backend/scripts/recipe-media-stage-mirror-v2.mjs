...
const USER_AGENT = 'MYPA-recipe-stage-mirror/2.0';
...
async function pageData(title) {
  const params = new URLSearchParams({ action: 'query', titles: `Cookbook:${title}`, prop: 'revisions|images', rvprop: 'content', rvslots: 'main', imlimit: '60', format: 'json', formatversion: '2' });
  return (await json(`${WIKIBOOKS_API}?${params}`))?.query?.pages?.[0] ?? {};
}
function revisionText(page) {
  const main = page?.revisions?.[0]?.slots?.main;
  return typeof main === 'string' ? main : main?.content ?? main?.['*'] ?? page?.revisions?.[0]?.content ?? '';
}
function headingBlocks(text) {
  const matches = [...text.matchAll(/^(={2,6})\s*(.*?)\s*\1\s*$\n?/gm)];
...
function uniqueRefs(refs) { const seen = new Set(); return refs.filter((r) => !seen.has(r.file) && seen.add(r.file)); }
function isMethod(name) { return /^(method|procedure|preparation|directions?|instructions?|steps?)$/i.test(name); }
function isFinalCaption(caption) { return /finally|done!?$|finished|ready(?: to (?:eat|serve))?|served|serving|plated|plating|presentation|ready for serving/i.test(clean(caption).toLowerCase()); }
...
async function mirror(title) {
  ...
  const blocks = headingBlocks(revision);
  const methodBlocks = blocks.filter((b) => isMethod(b.name));
  const methodRefs = uniqueRefs(methodBlocks.flatMap((b) => refsFromText(revision.slice(b.start, b.end))));
  const allRefs = uniqueRefs(refsFromText(revision));
  const refsToInspect = uniqueRefs([...methodRefs, ...allRefs.slice(0, 8)]);
  const infos = new Map();
  ...
  const explicitFinal = methodInfos.filter(({ ref }) => isFinalCaption(ref.caption)).at(-1)?.info ?? null;
  const processPool = methodInfos.filter(({ ref }) => !isFinalCaption(ref.caption));
  const process = processPool.slice(0, REQUIRED_PROCESS);
  let final = explicitFinal;
  if (!final) {
    const nonMethodRefs = allRefs.filter((ref) => !methodRefs.some((m) => m.file === ref.file));
    final = nonMethodRefs.map((ref) => infos.get(ref.file)).find(Boolean) ?? null;
    if (final && final.width < 300) final = null;
  }
  ...
  for (const item of selected) {
    try {
      ...
      media.push({ position: item.position, stage: item.stage, localPath: path.relative(ROOT, target).split(path.sep).join('/'), objectKey: path.relative(path.join(ROOT, 'media'), target).split(path.sep).join('/'), sourceUrl: item.sourceUrl, downloadUrl: item.url, sha256: digest, sizeBytes, format: 'webp', provider: 'Wikimedia Commons/Wikibooks', license: item.license, attribution: item.attribution, status: 'ready', evidence: item.stage === 'final' ? (explicitFinal ? 'recipe-step-final-caption' : 'recipe-hero-image') : 'recipe-method-image' });
  ...
  return { media, mediaCount: media.filter((x) => x.status === 'ready').length, stageStatus: processReady === REQUIRED_PROCESS && finalReady ? 'complete' : 'incomplete', stageRequirement: { processImages: REQUIRED_PROCESS, finalImage: 1 }, sourceEvidence: { methodImagesFound: methodRefs.length, finalCaptionFound: Boolean(explicitFinal), pageImagesFound: allRefs.length } };
}
async function main() {
  ...
  let manifest = { schemaVersion: 7, generatedAt: null, root: ROOT, requiredMediaPerItem: REQUIRED_TOTAL, items: {} }; if (await exists(MANIFEST_PATH)) { try { manifest = JSON.parse(await readFile(MANIFEST_PATH, 'utf8')); } catch {} }
  let cursor = 0; let done = 0; const total = rows.length;
  const worker = async () => { while (cursor < rows.length) { const item = rows[cursor++]; const key = `recipe:${slug(item.title)}`; try { const result = await mirror(item.title); manifest.items[key] = { kind: 'recipe', name: item.title, slug: slug(item.title), ...result, legacyImagesRemoved: true }; } catch (error) { manifest.items[key] = { kind: 'recipe', name: item.title, slug: slug(item.title), stageStatus: 'failed', media: [], mediaCount: 0, legacyImagesRemoved: true, error: error instanceof Error ? error.message : String(error) }; } done += 1; await saveJson(MANIFEST_PATH, { ...manifest, schemaVersion: 7, generatedAt: new Date().toISOString() }); console.log(`[recipe-stage] ${done}/${total} ${item.title} status=${manifest.items[key].stageStatus} process=${manifest.items[key].media.filter((x) => x.status === 'ready' && x.stage.startsWith('process-')).length}/3 final=${manifest.items[key].media.some((x) => x.status === 'ready' && x.stage === 'final') ? 1 : 0} evidence=${manifest.items[key].sourceEvidence?.finalCaptionFound ? 'caption-final' : manifest.items[key].media.some((x) => x.evidence === 'recipe-hero-image') ? 'recipe-hero-image' : manifest.items[key].sourceEvidence?.methodImagesFound ? `method-${manifest.items[key].sourceEvidence.methodImagesFound}` : 'none'}`); } };
  await Promise.all(Array.from({ length: CONCURRENCY }, worker));
  const recipes = Object.values(manifest.items).filter((x) => x.kind === 'recipe'); const complete = recipes.filter((x) => x.stageStatus === 'complete').length; const report = { schemaVersion: 6, generatedAt: new Date().toISOString(), root: ROOT, required: { processImages: REQUIRED_PROCESS, finalImage: 1, total: REQUIRED_TOTAL }, recipes: recipes.length, complete, incomplete: recipes.length - complete }; await saveJson(path.join(ROOT, 'recipe-stage-summary.json'), report); console.log(JSON.stringify(report, null, 2)); if (recipes.length && incomplete) process.exitCode = 2;
}
