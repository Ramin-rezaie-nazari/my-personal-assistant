import { resolveFoodEntity } from './food-entity-resolver-v2.mjs';
import { resolveLocalizedFoodEntity } from './localized-food-entity-resolver-v2.mjs';
import { FOOD_ENTITY_RESOLVER_VERSION } from './food-entity-resolver-v2.mjs';
...
const VERSION = 'recipe-food-entity-intelligence-v2';
...
function splitIfClearlyMultiple(line) {
  const pieces = line.split(/\s*[,;]\s*/).map((x) => x.trim()).filter(Boolean);
  if (pieces.length < 2 || pieces.length > 4) return null;
  const candidates = pieces.map((piece) => resolveFoodEntity(piece));
  if (candidates.every((item) => item.canonical_id && !item.review_required)) return candidates;
  return null;
}
...
function resolveLine(line) {
  ...
  return splitIfClearlyMultiple(line) || [direct];
}

function analyzeRecipe(recipe, rawValue) {
  const sourceLines = parseSourceIngredients(rawValue);
  const entities = sourceLines.flatMap(resolveLine);
  const unresolved = entities.filter((item) => !item.canonical_id || item.review_required);
  const canonicalIds = [...new Set(entities.map((item) => item.canonical_id).filter(Boolean))];
  return {
...
async function main() {
  ...
  for (let i = 0; i < recipes.length; i += BATCH) {
    for (const recipe of recipes.slice(i, i + BATCH)) {
      ...
      if (analysis.review_required) unresolved += analysis.unresolved_count;
      else fullyResolved += 1;
      entities += analysis.entity_count;
      if (!DRY_RUN) await patchProfile(recipe.id, analysis);
      processed += 1;
    }
    console.log(JSON.stringify({ progress: processed, total: recipes.length, withIngredients, fullyResolved, unresolved, entities }, null, 2));
  }

  console.log(JSON.stringify({ status: 'complete', mode: DRY_RUN ? 'dry-run' : 'apply', processed, total: recipes.length, withIngredients, fullyResolved, unresolved, entities, resolverVersion: FOOD_ENTITY_RESOLVER_VERSION, version: VERSION }, null, 2));
}
