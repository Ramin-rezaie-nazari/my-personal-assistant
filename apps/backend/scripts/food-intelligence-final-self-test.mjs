...
const resolverCases = [
  ['450 g chicken breasts, boneless, skinless', 'chicken_breast'],
  ['1 1/2 cups cherry tomatoes', 'tomato_cherry'],
  ['EVOO', 'olive_oil_extra_virgin'],
  ['aceite de oliva', 'olive_oil'],
  ['روغن زیتون فرابکر', 'olive_oil_extra_virgin'],
  ['olive pomace oil', 'olive_pomace_oil'],
  ['10–12 curry leaves', 'curry_leaf'],
  ['1 to 2 jalapeño peppers', 'jalapeno_pepper'],
  ['2 cups 3/4-inch cubes peeled jicama', 'jicama'],
  ['1/3 cup 1% buttermilk', 'buttermilk'],
...
for (const [input, expected] of resolverCases) {
  assert.equal(resolveFoodEntity(input).canonical_id, expected, input);
}

assert.equal(resolveFoodEntity('olive pomace oil').relations.length, 1);
...
console.log(JSON.stringify({
  status: 'pass',
  resolver_cases: resolverCases.length,
  package_size_cases: packageSizeCases.length,
  decomposer_cases: decomposer.cases,
