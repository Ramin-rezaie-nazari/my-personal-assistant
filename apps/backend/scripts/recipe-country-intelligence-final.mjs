...
), origin_map(iso2,relation_type,confidence,evidence,pattern) AS (VALUES ${patternValues}),
...
SELECT id,(t ~* '\\m(pizza|pasta|burger|hamburger|fried rice|noodle|sushi|ramen|taco|curry|sandwich|ice cream|cheesecake|pancake|waffle|hot dog|french fries|chocolate cake|chocolate chip cookie|mochi|dumpling|shawarma|falafel|hummus|teriyaki|pad thai|pho|biryani|empanada|crepe|smoothie|muffin|doughnut|donut|fried chicken)\\M' OR cuisine ~* '(international|world cuisine|worldwide|fusion)') AS is_global,
CASE WHEN t ~* '\\m(pizza|pasta|burger|hamburger|fried rice|noodle|sushi|ramen|taco|curry|sandwich|ice cream|cheesecake|pancake|waffle|hot dog|french fries|chocolate cake|chocolate chip cookie|mochi|dumpling|shawarma|falafel|hummus|teriyaki|pad thai|pho|biryani|empanada|crepe|smoothie|muffin|doughnut|donut|fried chicken)\\M' THEN 0.92 WHEN cuisine ~* '(international|world cuisine|worldwide|fusion)' THEN 0.84 ELSE NULL END AS global_confidence FROM base;

-- Cleanup is scoped to the same working set as LIMIT. A bounded run never erases prior classifications outside that set.
DELETE FROM recipe_country_relations
WHERE source LIKE 'country-intelligence-final-%'
  AND recipe_id IN (SELECT id FROM tmp_recipe_global_final);
DELETE FROM recipe_intelligence_profiles
WHERE classification_version LIKE 'country-intelligence-final-%'
  AND recipe_id IN (SELECT id FROM tmp_recipe_global_final);
...
SELECT r.id,'${VERSION}','rule-engine-final',jsonb_build_object(
  ...
  'global',r.is_global,'global_confidence',r.global_confidence,
...
SELECT count(*)::int AS recipes_processed,(SELECT count(*) FROM recipe_country_relations WHERE source='${VERSION}')::int AS country_relations,(SELECT count(*) FROM recipes WHERE classification_version='${VERSION}' AND is_global)::int AS global_recipes,(SELECT count(*) FROM recipes WHERE classification_version='${VERSION}' AND NOT is_global AND id NOT IN (SELECT DISTINCT recipe_id FROM recipe_country_relations WHERE source='${VERSION}'))::int AS unknown_recipes FROM recipes ${LIMIT > 0 ? 'WHERE id IN (SELECT id FROM tmp_recipe_global_final)' : ''};
`;

const client = new Client({ connectionString: DATABASE_URL });
await client.connect();
try { await client.query(sql); if (DRY_RUN) await client.query('ROLLBACK'); else await client.query('COMMIT'); console.log(JSON.stringify({ status: 'complete', mode: DRY_RUN ? 'dry-run' : 'apply', version: VERSION }, null, 2)); }
