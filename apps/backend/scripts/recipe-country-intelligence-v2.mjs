...
const VERSION = 'country-rules-v2';
const V1_SOURCE = 'country-rules-v1';
const V2_SOURCE = 'country-intelligence';
const V2_SOURCE_REF = VERSION;

if (!DRY_RUN && (!SUPABASE_URL || !SERVICE_KEY)) {
  throw new Error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required when not in dry-run mode.');
}
...
function infer(recipe) {
  ...
  for (const [re, iso2, evidence] of strongDishOrigins) {
    if (re.test(allText)) add(iso2, 'origin', 0.96, evidence, 100);
  }
  ...
  if (recipe.cuisine) {
    ...
    for (const [alias, iso2] of countryNameAliases) {
      if (!containsWord(cuisine, alias)) continue;
  ...
  // A recipe is considered global only when no country-level evidence is available.
  // Generic food words such as taco, salad, pasta, soup, etc. are deliberately excluded:
  // they are too broad to establish global classification reliably at corpus scale.
  return {
    relations,
    global: relations.length === 0,
    globalConfidence: relations.length === 0 ? 0.62 : null,
  };
}
...
async function cleanupForRecipes(recipeIds) {
  if (DRY_RUN || recipeIds.length === 0) return;
  const ids = inFilter(recipeIds);
  for (const source of [V1_SOURCE, V2_SOURCE]) {
    await rest(`recipe_country_relations?recipe_id=${ids}&source=eq.${encodeURIComponent(source)}`, {
      method: 'DELETE',
      headers: { Prefer: 'return=minimal' },
    });
  }
  for (const idBatch of chunk(recipeIds, 500)) {
    await rest(`recipes?id=${inFilter(idBatch)}`, {
      method: 'PATCH',
      headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ is_global: false, global_confidence: null, classification_version: null }),
    });
  }
}

async function persist(countryRows, classifications, countries) {
  ...
  for (const idBatch of chunk(globalIds, 500)) {
    await rest(`recipes?id=${inFilter(idBatch)}`, {
      method: 'PATCH',
      headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ is_global: true, global_confidence: 0.62, classification_version: VERSION }),
    });
  }
...
async function main() {
  ...
  if (DRY_RUN) {
    // Dry-run still needs DB access to inspect the real recipe corpus.
    if (!SUPABASE_URL || !SERVICE_KEY) throw new Error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required for dry-run inspection.');
  }
  ...
  if (!DRY_RUN) await cleanupForRecipes(recipes.map((recipe) => recipe.id));
  ...
  await persist(countryRows, classifications, countries);
