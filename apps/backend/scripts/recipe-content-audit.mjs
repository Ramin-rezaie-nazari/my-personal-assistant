...
const MIN_MEDIA = Math.max(1, Number(process.env.RECIPE_MIN_MEDIA ?? 1));
...
async function main() {
  const recipes = await prisma.recipe.findMany({
    ...
    select: {
      ...
      media: {
        ...
        select: { id: true, position: true, url: true, sourceUrl: true, sourceProvider: true, license: true, mimeType: true },
      },
  ...
  const counts = {
    ...
    withHero: recipes.filter((r) => cleanText(r.imageUrl)).length,
    ...
    withApprovedMedia: recipes.filter((r) => r.media.length >= MIN_MEDIA).length,
  ...
  for (const recipe of recipes) {
    ...
    if (REQUIRE_IMAGE && recipe.media.length < MIN_MEDIA && !cleanText(recipe.imageUrl)) {
      fail('missing_image', 'Recipe has no approved media and no hero image.', { id: recipe.id, name: recipe.name });
    }
    ...
    for (const media of recipe.media) {
      ...
      if (media.mimeType.toLowerCase() !== 'image/webp') warn.push({ code: 'non_webp_media', id: recipe.id, mediaId: media.id, mimeType: media.mimeType });
    }
  }
  ...
  for (const row of duplicateNames) {
    warn.push({ code: 'duplicate_recipe_name', normalized: row.normalized, count: Number(row.count) });
  }
  ...
  const sample = recipes.slice(0, MAX_SAMPLE).map((r) => ({
    ...
    hero: Boolean(cleanText(r.imageUrl)),
  }));

  const report = {
    status: failures.length ? 'failed' : 'pass',
    thresholds: { MIN_INGREDIENTS, MIN_STEPS, MIN_MEDIA, REQUIRE_VERIFIED, REQUIRE_IMAGE },
    counts,
