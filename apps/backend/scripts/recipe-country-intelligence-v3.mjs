...
if (!SUPABASE_URL || !SERVICE_KEY) {
  console.warn('Dry-run requested without Supabase credentials; database-backed preview cannot run.');
}
...
function infer(recipe) {
  ...
  const explicitVariantCountries = variantMatch
    ? [...countryNameAliases.entries()].filter(([alias]) => containsWord(norm(variantMatch[1]), alias))
    : [];
  ...
  relations.sort((a, b) => (
    a.is_primary !== b.is_primary
      ? (a.is_primary ? -1 : 1)
      : b.confidence - a.confidence || a.iso2.localeCompare(b.iso2)
  ));
  return {
    relations,
...
async function removePreviousV2(recipeIds) {
  ...
  for (const idBatch of chunk(recipeIds, 500)) {
    await rest(`recipe_country_relations?recipe_id=${inFilter(idBatch)}&source=eq.${encodeURIComponent(INTELLIGENCE_SOURCE)}`, {
      method: 'DELETE',
...
async function removeLegacyV1(recipeIds) {
  if (DRY_RUN || recipeIds.length === 0) return;
  for (const idBatch of chunk(recipeIds, 500)) {
    await rest(`recipe_country_relations?recipe_id=${inFilter(idBatch)}&source=eq.${encodeURIComponent(V1_SOURCE)}`, {
      method: 'DELETE',
      headers: { Prefer: 'return=minimal' },
    });
  }
