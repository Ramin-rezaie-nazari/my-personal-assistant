...
async function parsePage(title) {
  const page = title.startsWith('Cookbook:') ? title : `Cookbook:${title}`;
  const params = new URLSearchParams({ action: 'parse', page, prop: 'text', format: 'json', formatversion: '2' });
  return (await json(`${WIKIBOOKS_API}?${params}`))?.parse ?? null;
}
function methodHtml(html = '') {
  ...
  const next = tail.search(/<h[2-6][^>]*>/i);
  return tail.slice(0, next >= 0 ? next : tail.length);
}
...
async function convert(input, target) {
  ...
  if (image.length > 64 * 1024) image = await sharp(input, { failOn: 'none' }).rotate().resize({ width: 700, height: 700, fit: 'inside', withoutEnlargement: true }).webp({ quality: 60, effort: 5 }).toBuffer();
...
async function mirror(title) {
  ...
  const method = methodHtml(html);
  const refs = methodImageRefs(method);
  const infos = [];
  ...
  const process = infos.slice(0, Math.max(0, infos.length - 1)).slice(0, REQUIRED_PROCESS);
  const final = infos.length >= 4 ? infos[infos.length - 1] : null;
  const selected = process.map((x, i) => ({ ...x.info, caption: x.ref.caption, stage: `process-${i + 1}`, position: i + 1, evidence: 'recipe-method-image' }));
  if (final) selected.push({ ...final.info, caption: final.ref.caption, stage: 'final', position: REQUIRED_TOTAL, evidence: /finally|done!?$|finished|served|plated|ready/i.test(clean(final.ref.caption).toLowerCase()) ? 'recipe-step-final-caption' : 'recipe-method-final-image' });
  ...
  for (const item of selected) {
    try {
      ...
      media.push({ position: item.position, stage: item.stage, caption: item.caption, localPath: path.relative(ROOT, target).split(path.sep).join('/'), objectKey: path.relative(path.join(ROOT, 'media'), target).split(path.sep).join('/'), sourceUrl: item.sourceUrl, downloadUrl: item.url, sha256: digest, sizeBytes, format: 'webp', provider: 'Wikimedia Commons/Wikibooks', license: item.license, attribution: item.attribution, status: 'ready', evidence: item.evidence });
    } catch (error) { media.push({ position: item.position, stage: item.stage, caption: item.caption, sourceUrl: item.sourceUrl, status: 'failed', error: error instanceof Error ? error.message : String(error) }); }
  }
...
async function main() {
  ...
  let cursor = 0; let done = 0; const worker = async () => { while (cursor < rows.length) { const item = rows[cursor++]; const key = `recipe:${slug(item.title)}`; try { const result = await mirror(item.title); manifest.items[key] = { kind: 'recipe', name: item.title, slug: slug(item.title), ...result, legacyImagesRemoved: true }; } catch (error) { manifest.items[key] = { kind: 'recipe', name: item.title, slug: slug(item.title), stageStatus: 'failed', media: [], mediaCount: 0, legacyImagesRemoved: true, sourceEvidence: { methodImagesFound: 0, approvedMethodImages: 0 }, error: error instanceof Error ? error.message : String(error) }; } done += 1; await saveJson(MANIFEST_PATH, { ...manifest, schemaVersion: 8, generatedAt: new Date().toISOString() }); const current = manifest.items[key]; console.log(`[recipe-stage-v3] ${done}/${rows.length} '${item.title}' status=${current.stageStatus} process=${current.media.filter((x) => x.status === 'ready' && x.stage.startsWith('process-')).length}/3 final=${current.media.some((x) => x.status === 'ready' && x.stage === 'final') ? 1 : 0} method=${current.sourceEvidence?.methodImagesFound ?? 0} approved=${current.sourceEvidence?.approvedMethodImages ?? 0} finalCaption=${current.sourceEvidence?.finalCaption ? 'yes' : 'no'}`); } };
  ...
  const report = { schemaVersion: 7, generatedAt: new Date().toISOString(), root: ROOT, required: { processImages: REQUIRED_PROCESS, finalImage: 1, total: REQUIRED_TOTAL }, recipes: recipes.length, complete, incomplete: recipes.length - complete };
