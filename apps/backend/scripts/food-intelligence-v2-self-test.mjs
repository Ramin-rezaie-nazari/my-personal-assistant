...
const prep = resolveFoodEntity('450 g chicken breasts, boneless, skinless');
assert.equal(prep.canonical_id, 'chicken_breast');
assert.equal(prep.quantity, 450);
assert.equal(prep.unit, 'g');
...
const unknown = resolveFoodEntity('unknown synthetic food concentrate');
