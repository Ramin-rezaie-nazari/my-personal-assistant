...
- The previous `Full Content Media Corpus` failure is preserved as historical evidence: it failed at `pnpm install --frozen-lockfile`, so none of its corpus-generation steps executed.
- Earlier Android CI failure is also preserved: dependency installation failed before Gradle, so that run did not prove an Android compile failure.
- Because the latest content/media jobs may still be queued/in-progress or may still depend on missing runtime database/storage secrets, no statement of full-corpus completion is made here.
