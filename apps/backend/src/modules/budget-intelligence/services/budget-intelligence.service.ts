...
export class BudgetIntelligenceService {
  ...
  async quoteItems(input: BudgetQuoteInput[], currency: string, budget?: number) {
    ...
    for (const candidate of input) {
      ...
      const target = normalizeQuantity(candidate.quantity, candidate.unit);
      const compatibleUnitRows = compatibleCurrency.filter((row) => row.unit && row.unitPrice != null && normalizeQuantity(1, row.unit)?.kind === target?.kind);
      if (!target || !compatibleUnitRows.length) { const hasUnitEvidence = compatibleCurrency.some((row) => row.unit); items.push({ ...base, price: null, estimatedCost: null, priceObservedAt: null, priceSourceId: null, status: hasUnitEvidence ? 'unit_mismatch' : 'price_unavailable', reason: hasUnitEvidence ? 'price_unit_does_not_match_recipe_unit' : 'price_unit_missing' }); continue; }
      ...
      if (estimatedCost === null || !Number.isFinite(price) || price < 0) { items.push({ ...base, price: null, estimatedCost: null, priceObservedAt: observedAt, priceSourceId: selected.row.sourceId ?? null, status: 'price_unavailable', reason: estimatedCost === null ? 'price_unit_does_not_match_recipe_unit' : 'invalid_price_value' }); continue; }
