...
import { convertUnitPrice, normalizeQuantity } from '../../../common/units/quantity-conversion';
...
export class BudgetIntelligenceService {
  ...
  async quoteItems(input: BudgetQuoteInput[], currency: string, budget?: number) {
    ...
    for (const candidate of input) {
      ...
      const compatibleUnitRows = compatibleCurrency.filter((row) => row.unit && row.unitPrice != null && normalizeQuantity(1, row.unit) !== null && normalizeQuantity(candidate.quantity, candidate.unit) !== null && normalizeQuantity(1, row.unit)!.kind === normalizeQuantity(candidate.quantity, candidate.unit)!.kind);
      ...
      const price = Number(selected.row.unitPrice); const estimatedCost = convertUnitPrice(price, selected.row.unit!, candidate.quantity, candidate.unit);
      if (estimatedCost === null) { items.push({ ...base, price: null, estimatedCost: null, priceObservedAt: observedAt, priceSourceId: selected.row.sourceId ?? null, status: 'unit_mismatch', reason: 'price_unit_does_not_match_recipe_unit' }); continue; }
      const pricePerCandidateUnit = Number((estimatedCost / candidate.quantity).toFixed(2));
      if (!Number.isFinite(price) || price < 0 || !Number.isFinite(estimatedCost) || !Number.isFinite(pricePerCandidateUnit)) { items.push({ ...base, price: null, estimatedCost: null, priceObservedAt: observedAt, priceSourceId: selected.row.sourceId ?? null, status: 'price_unavailable', reason: 'invalid_price_value' }); continue; }
      if (estimatedCost > remaining) { items.push({ ...base, price: pricePerCandidateUnit, estimatedCost, priceObservedAt: observedAt, priceSourceId: selected.row.sourceId ?? null, status: 'over_budget', reason: 'recipe_missing_quantity_exceeds_remaining_budget' }); continue; }
      remaining -= estimatedCost; spent += estimatedCost; items.push({ ...base, price: pricePerCandidateUnit, estimatedCost, priceObservedAt: observedAt, priceSourceId: selected.row.sourceId ?? null, status: 'priced', reason: 'recipe_missing_ingredient_fits_verified_budget' });
    }
