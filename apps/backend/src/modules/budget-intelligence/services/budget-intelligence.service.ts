...
export class BudgetIntelligenceService {
  ...
  async createPlan(userId: string, budget: number, currency: string) {
    ...
    return { userId, budget, currency: normalizedCurrency, totalEstimatedCost: quote.totalEstimatedCost, budgetRemaining: quote.budgetRemaining, pricedItemCount: quote.items.filter((item) => item.status === 'priced').length, itemCount: quote.items.length, budgetStatus: deriveBudgetStatus(quote.items), generatedDeterministically: true, items: quote.items } as const;
  }
