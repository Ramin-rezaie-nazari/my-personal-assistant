...
export class BudgetIntelligenceService {
  ...
  async createPlan(userId: string, budget: number, currency: string) {
    ...
    return {
      userId, budget, currency: normalizedCurrency, totalEstimatedCost: quote.totalEstimatedCost, budgetRemaining: quote.budgetRemaining,
      pricedItemCount: quote.items.filter((item) => item.status === 'priced').length,
      itemCount: quote.items.length,
      budgetStatus: deriveBudgetStatus(quote.items),
  ...
  async quoteItems(input: BudgetQuoteInput[], currency: string, budget?: number) {
    const normalizedCurrency = normalizeCurrency(currency);
    if (budget !== undefined && (!Number.isFinite(budget) || budget < 0)) throw new BadRequestException('budget must be a non-negative number');
    const now = Date.now();
    ...
    for (const candidate of input) {
      ...
      const base = { foodId: candidate.foodId, name: candidate.name, productKey, recommendedQuantity: candidate.quantity, unit: candidate.unit, currency: normalizedCurrency, urgency };
      ...
      if (!Number.isFinite(observedAt.getTime())) { items.push({ ...base, price: null, estimatedCost: null, priceObservedAt: null, priceSourceId: compatibleUnit.sourceId ?? null, status: 'price_unavailable', reason: 'invalid_observed_at' }); continue; }
...
function deriveBudgetStatus(items: BudgetQuoteItem[]) {
...
function normalizeCurrency(currency: string) {
  ...
  if (!/^[A-Z]{3}$/.test(normalizedCurrency)) throw new BadRequestException('currency must be a 3-letter ISO currency code');
