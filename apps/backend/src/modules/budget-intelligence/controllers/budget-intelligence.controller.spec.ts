...
describe('BudgetIntelligenceController', () => {
  ...
  const globalCountryFinance = { getFinanceContext: jest.fn(), getSupportedCountryCodes: jest.fn() };
  ...
  it('passes the authenticated user, budget and currency to the budget service', async () => {
    ...
    await expect(controller.plan({ user: { id: 'user-1' } }, '15000000', 'IRR')).resolves.toEqual({ ok: true });
    expect(budget.createPlan).toHaveBeenCalledWith('user-1', 15000000, 'IRR');
  });

  it('rejects missing budget or currency', () => {
    expect(() => controller.plan({ user: { id: 'user-1' } }, undefined, 'USD')).toThrow();
    expect(() => controller.plan({ user: { id: 'user-1' } }, '10', undefined)).toThrow();
  });
  ...
  it('passes user, servings and country into meal planning', async () => {
    ...
    await expect(controller.mealPlan({ user: { id: 'user-1' } }, '4', 'JP')).resolves.toEqual({ generatedDeterministically: true });
    expect(mealPlanning.createMealPlan).toHaveBeenCalledWith('user-1', 4, 'JP');
  });

  it('rejects invalid meal-plan servings before calling the service', () => {
    expect(() => controller.mealPlan({ user: { id: 'user-1' } }, 'NaN', 'JP')).toThrow();
    expect(() => controller.mealPlan({ user: { id: 'user-1' } }, '0', 'JP')).toThrow();
    expect(() => controller.mealPlan({ user: { id: 'user-1' } }, '1.5', 'JP')).toThrow();
    expect(mealPlanning.createMealPlan).not.toHaveBeenCalled();
  });
});
