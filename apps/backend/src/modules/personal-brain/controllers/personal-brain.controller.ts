...
export class PersonalBrainController {
  ...
  @Post('decision/outcome') @UseGuards(JwtAuthGuard) async recordDecisionOutcome(@Body() body: { decisionId: string; outcome: 'positive' | 'neutral' | 'negative'; score?: number; note?: string }, @Req() req: AuthenticatedRequest) {
    return this.decisionOutcomeLearningService.record({ userId: req.user.id, ...body });
  }
