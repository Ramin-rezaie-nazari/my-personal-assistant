...
export class PersonalBrainController {
  ...
  @Get('overview') @UseGuards(JwtAuthGuard) async getOverview(
  ...
  @Get('plan') @UseGuards(JwtAuthGuard) async getPlan(
    @Req() req: AuthenticatedRequest,
  ) {
    return this.smartPlanningService.getPlan(req.user.id);
  }
  @Get('plan/history') @UseGuards(JwtAuthGuard) async getPlanHistory(
    @Req() req: AuthenticatedRequest,
  ) {
    return this.persistentPlanStateService.listRecent(req.user.id, 10);
  }
  @Get('trace') @UseGuards(JwtAuthGuard) async getTrace(
    @Req() req: AuthenticatedRequest,
  ) {
    return this.decisionAuditService.recent(req.user.id, 20);
  }
  @Get('explanations/history')
  @UseGuards(JwtAuthGuard)
  async getExplanationHistory(@Req() req: AuthenticatedRequest) {
    return this.decisionExplanationMemoryService.recent(req.user.id, 30);
  }
  @Get('explanations/trend') @UseGuards(JwtAuthGuard) async getExplanationTrend(
    @Req() req: AuthenticatedRequest,
  ) {
    return this.decisionExplanationMemoryService.trend(req.user.id, 90);
  }
  ...
  async recordDecisionOutcome(
    ...
    return this.decisionOutcomeLearningService.record({
      userId: req.user.id,
      ...body,
      source: 'user',
    });
  }
  @Get('decision/outcome-profile')
  @UseGuards(JwtAuthGuard)
  async getDecisionOutcomeProfile(@Req() req: AuthenticatedRequest) {
    return this.decisionOutcomeLearningService.profile(req.user.id);
  }
  @Post('decision/execute') @UseGuards(JwtAuthGuard) async executeDecision(
    @Body()
    body: { candidate: DecisionCandidate; context?: Record<string, unknown> },
    @Req() req: AuthenticatedRequest,
  ) {
    const candidate = body.candidate;
    if (!candidate?.id || !candidate?.action || !candidate?.domain)
      return { status: 'failed', reason: 'invalid_candidate' };
    return this.decisionExecutionCoordinatorService.execute(
      req.user.id,
      candidate,
      { ...(body.context ?? {}), userId: req.user.id },
    );
  }
  @Post('decision/confirm') @UseGuards(JwtAuthGuard) async confirmDecision(
  ...
  @Post('coach/cue') @UseGuards(JwtAuthGuard) async coachCue(
    @Body()
    body: {
      kind:
        'exerciseStart' | 'countdown' | 'transition' | 'safety' | 'explanation';
      name?: string;
      seconds?: number;
      message?: string;
      language?: 'fa' | 'en';
    ...
    if (body.kind === 'exerciseStart')
      return this.coachCueEngineService.buildExerciseStart(
        body.name ?? 'تمرین',
        body.durationSeconds ?? 60,
        language,
      );
    if (body.kind === 'countdown')
      return this.coachCueEngineService.buildCountdown(
        Math.max(1, Math.min(60, Math.round(body.seconds ?? 10))),
        language,
      );
    if (body.kind === 'transition')
      return this.coachCueEngineService.buildTransition(
        body.name ?? 'حرکت بعدی',
        language,
      );
    if (body.kind === 'safety')
      return this.coachCueEngineService.buildSafety(
        body.message ?? 'حرکت را متوقف کن و فرم بدنت را بررسی کن.',
        language,
      );
    return this.coachCueEngineService.buildExplanation(
      body.message ?? '',
      language,
    );
  }
  @Post('decision/explain') @UseGuards(JwtAuthGuard) async explainDecision(
    @Body() body: { message?: string },
    @Req() req: AuthenticatedRequest,
  ) {
    const message = body.message?.trim() || 'What should I do next?';
    const context = await this.brainReasoningContextService.build(
      message,
      req.user.id,
    );
  ...
  @Post('coach/notification-decision')
  @UseGuards(JwtAuthGuard)
  async getNotificationDecision(
    @Body() body: { event: any; preferences?: NotificationPreferences },
    @Req() req: AuthenticatedRequest,
  ) {
    const duplicate = this.notificationDeduplicationService.shouldSend(
      body.event,
    );
    if (!duplicate.allowed)
      return { send: false, event: body.event, reason: duplicate.reason };
    return this.notificationOrchestratorService.decide(
      body.event,
      body.preferences,
    );
  }
  @Post('coach/notification-feedback')
  @UseGuards(JwtAuthGuard)
  async recordNotificationFeedback(
    ...
    return this.notificationFeedbackService.record({
      userId: req.user.id,
      ...body,
    });
  }
  @Get('coach/notification-feedback')
  @UseGuards(JwtAuthGuard)
  async getNotificationFeedback(@Req() req: AuthenticatedRequest) {
    return this.notificationFeedbackService.getRecent(req.user.id);
  }
  @Get('coach/notification-signal')
  @UseGuards(JwtAuthGuard)
  async getNotificationSignal(@Req() req: AuthenticatedRequest) {
    return this.notificationFeedbackService.getSignal(
      req.user.id,
      String(req.query.eventType ?? 'next_action'),
    );
  }
  @Post('coach/device')
  @UseGuards(JwtAuthGuard)
  async registerNotificationDevice(
    ...
    return this.notificationDeviceRegistryService.register({
      userId: req.user.id,
      ...body,
    });
  }
  @Get('coach/devices') @UseGuards(JwtAuthGuard) async listNotificationDevices(
    @Req() req: AuthenticatedRequest,
  ) {
    return this.notificationDeviceRegistryService.listEnabled(req.user.id);
  }
  @Post('coach/device/disable')
  @UseGuards(JwtAuthGuard)
  ...
  @Post('scenario/compare') @UseGuards(JwtAuthGuard) async compareScenarios(
    ...
    return this.scenarioPlanningService.compare(body);
  }
  @Post() @UseGuards(JwtAuthGuard) async process(
    @Body() dto: CreateBrainRequestDto,
    @Req() req: AuthenticatedRequest,
  ) {
    return this.brainOrchestratorService.processRequest(
      dto.message,
      req.user.id,
    );
  }
