...
export class DecisionSafetyGuardService {
  sanitize(
    ...
    const maxActions = Math.max(
      1,
      (policy.maxActions ?? result.selected.length) || 1,
    );
    const maxPerDomain = Math.max(
      1,
      policy.maxPerDomain ?? Number.MAX_SAFE_INTEGER,
    );
    ...
    for (const candidate of result.selected) {
      ...
      if (
        blockedDomains.has(candidate.domain) ||
        count >= maxPerDomain ||
        selected.length >= maxActions
      ) {
    ...
    return {
      ...
      reason: safetyRejected.length
        ? `${result.reason};safety_guard_applied`
        : result.reason,
    };
  }
}
