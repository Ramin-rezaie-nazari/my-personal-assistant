...
export class ReplanPolicyService {
  ...
  async decide(userId: string, date = new Date()) {
    ...
    const shouldReplan =
      ...
      health.issues.overlaps > 0 ||
      health.issues.unscheduled > 0;
    return {
      shouldReplan,
      urgency:
        health.status === 'overloaded'
          ...
          : shouldReplan
            ...
            : 'none',
      reasons: [
        ...(health.issues.overlaps ? ['schedule conflict'] : []),
        ...(health.issues.unscheduled ? ['items could not fit'] : []),
        ...(health.status === 'overloaded' ? ['focus capacity exceeded'] : []),
      ],
      health,
    };
  }
