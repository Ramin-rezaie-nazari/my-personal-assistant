...
export class DynamicReplanningService {
  ...
  async replanRemainingDay(userId: string, now = new Date()) {
    ...
    return { generatedAt: now.toISOString(), date: plan.date, mode: 'remaining-day', items: remaining, overdue, unscheduled: plan.unscheduled, conflicts, requiresAttention: overdue.length > 0 || plan.unscheduled.length > 0 || conflicts > 0, nextAction: remaining.find(item => new Date(item.start) >= now) ?? null };
  }
}
