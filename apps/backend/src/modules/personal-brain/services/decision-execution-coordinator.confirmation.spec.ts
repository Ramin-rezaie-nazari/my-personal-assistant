...
describe('DecisionExecutionCoordinator confirmation flow', () => {
  it('does not execute a risky action until confirmed', async () => {
    ...
    const policy = { resolve: jest.fn().mockReturnValue({ timeoutMs: 1000, maxAttempts: 1, retryDelayMs: 0, dryRun: false }), run: jest.fn().mockImplementation((_c: any, _p: any, fn: any) => fn().then((result: any) => ({ result, attempts: [{}] }))) } as any;
    const feedback = { record: jest.fn() } as any;
    const history = { record: jest.fn() } as any;
    const service = new DecisionExecutionCoordinatorService(gate, adapters, feedback, policy, history, confirmation);
