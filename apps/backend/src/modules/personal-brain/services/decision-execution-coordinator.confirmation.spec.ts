...
describe('DecisionExecutionCoordinator confirmation flow', () => {
  it('does not execute a risky action until confirmed', async () => {
    ...
    const adapters = {
      execute: jest
        .fn()
        .mockResolvedValue({ handled: true, result: { ok: true } }),
    } as any;
    const gate = {
      open: jest.fn().mockReturnValue({ allowed: true }),
      complete: jest.fn(),
      fail: jest.fn(),
    } as any;
    const policy = {
      ...
      run: jest
        .fn()
        .mockImplementation((_c: any, _p: any, fn: any) =>
          fn().then((result: any) => ({ result, attempts: [{}] })),
        ),
    } as any;
    ...
    const done = await service.confirmAndExecute(
      'u1',
    ...
    expect(adapters.execute).toHaveBeenCalledWith(
      candidate,
      expect.objectContaining({ value: 'x', confirmed: true }),
    );
  });
});
