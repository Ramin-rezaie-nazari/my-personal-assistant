...
import {
  ...
  DeliveryJob,
} from './notification-delivery-queue.service';
import {
...
export class NotificationDeliveryDispatcherService {
  constructor(
    ...
    private readonly providers: NotificationDeliveryProviderRegistry,
  ) {}
  ...
  async dispatch(
    ...
    destination: string,
    now = new Date(),
  ): Promise<{ job: DeliveryJob | null; result: ProviderResult }> {
    ...
    if (now >= new Date(job.expiresAt)) {
      ...
      return {
        job: this.queue.get(jobId) ?? job,
        result: {
          accepted: false,
          provider: 'none',
          retryable: false,
          error: 'Delivery job expired.',
        },
      };
    }
    const provider = this.providers.get(channel);
    if (!provider) {
      const failed = this.queue.markFailed(
        jobId,
    ...
    try {
      ...
      if (result.accepted) this.queue.markDelivered(jobId, now);
        ...
        this.queue.markFailed(
          jobId,
          result.error ?? 'Permanent delivery failure.',
          now,
        );
      else
        this.queue.markFailed(
          jobId,
          result.error ?? 'Temporary delivery failure.',
          now,
        );
