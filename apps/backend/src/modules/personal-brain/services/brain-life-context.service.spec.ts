...
describe('BrainLifeContextService', () => {
  ...
  beforeEach(() => {
    ...
    fitness.buildRecommendationContext.mockResolvedValue({ disciplines: [], primaryGoal: null, equipment: [], constraints: [], targetAreas: [] });
  ...
  it('aggregates habits, reminders and supplements for one user', async () => {
    prisma.habit.findMany.mockResolvedValue([{ id: 'h1', name: 'Walk', targetPerWeek: 7, logs: [{ dateKey: '2026-08-12' }, { dateKey: '2026-08-11' }] }]);
    prisma.reminder.count.mockResolvedValue(2);
    prisma.reminder.findFirst.mockResolvedValue({ id: 'r1', title: 'Water', type: 'health', scheduledAt: new Date('2026-08-13T09:00:00Z') });
    prisma.supplement.findMany.mockResolvedValue([{ id: 's1', name: 'Vitamin D', dosage: '1000 IU', scheduledTime: '09:00', logs: [{ id: 'l1' }] }, { id: 's2', name: 'Magnesium', dosage: '200 mg', scheduledTime: '21:00', logs: [] }]);
    const service = new BrainLifeContextService(prisma as never, learning as never, fitness as never, performanceMemory as never, decisionMemory as never, outcomeMemory as never);
