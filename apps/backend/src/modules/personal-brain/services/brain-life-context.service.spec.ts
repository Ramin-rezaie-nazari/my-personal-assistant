...
describe('BrainLifeContextService', () => {
  ...
  const learning = { buildProfile: jest.fn().mockResolvedValue({ bestHours: [], preferredTaskMinutes: 30, snoozeRate: 0, acceptanceRate: 0 }) };
  const fitness = { buildRecommendationContext: jest.fn().mockResolvedValue({ disciplines: [], primaryGoal: null, equipment: [], constraints: [], targetAreas: [] }) };
  const performanceMemory = { get: jest.fn().mockResolvedValue({}) };
  const decisionMemory = { trend: jest.fn().mockResolvedValue({}) };
  const outcomeMemory = { profile: jest.fn().mockResolvedValue({}) };
  ...
  beforeEach(() => {
    ...
    service = new BrainLifeContextService(prisma as never, learning as never, fitness as never, performanceMemory as never, decisionMemory as never, outcomeMemory as never);
  });
