...
export class PersistentPlanStateService {
  ...
  async save(state: PersistedPlanState): Promise<PersistedPlanState> {
    const record = await this.prisma.planExecutionState.upsert({
      ...
      create: { userId: state.userId, planId: state.planId, status: state.status, stepIds: state.stepIds, completed: state.completed, blocked: state.blocked, failed: state.failed, currentStep: state.currentStep },
      update: { status: state.status, stepIds: state.stepIds, completed: state.completed, blocked: state.blocked, failed: state.failed, currentStep: state.currentStep },
    });
    return this.map(record);
  }

  async get(userId: string, planId: string): Promise<PersistedPlanState | null> {
    const record = await this.prisma.planExecutionState.findUnique({ where: { userId_planId: { userId, planId } } });
    return record ? this.map(record) : null;
  }

  async listRecent(userId: string, limit = 5): Promise<PersistedPlanState[]> {
    const records = await this.prisma.planExecutionState.findMany({ where: { userId }, orderBy: { updatedAt: 'desc' }, take: Math.min(Math.max(limit, 1), 20) });
  ...
  async resume(userId: string, planId: string): Promise<PersistedPlanState | null> {
    ...
    if (current.status === 'running') return this.save({ ...current, status: 'partial' });
    return current;
  }
  ...
  private map(record: { userId: string; planId: string; status: string; stepIds: unknown; completed: unknown; blocked: unknown; failed: unknown; currentStep: string | null; updatedAt: Date }): PersistedPlanState {
    return {
