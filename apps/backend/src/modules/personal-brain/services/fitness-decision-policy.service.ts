...
export class FitnessDecisionPolicyService {
  evaluate(context: BrainReasoningContext): BrainDecisionResult | null {
    ...
    if (/\byoga\b|یوگا/.test(input)) { candidates.find(c => c.discipline === 'yoga')!.score += 0.35; candidates.find(c => c.discipline === 'yoga')!.reasons.push('explicit-yoga-request'); }
    if (/calisthenics|کالیستنیکس/.test(input)) { candidates.find(c => c.discipline === 'calisthenics')!.score += 0.35; candidates.find(c => c.discipline === 'calisthenics')!.reasons.push('explicit-calisthenics-request'); }
    if (/\bgym\b|بدنسازی/.test(input)) { candidates.find(c => c.discipline === 'gym')!.score += 0.35; candidates.find(c => c.discipline === 'gym')!.reasons.push('explicit-gym-request'); }
    if (goal) {
      if (goal.kind === 'body_sculpt') { candidates.find(c => c.discipline === 'calisthenics')!.score += 0.15; candidates.find(c => c.discipline === 'yoga')!.score += 0.08; }
      if (goal.kind === 'strength') { candidates.find(c => c.discipline === 'gym')!.score += 0.18; candidates.find(c => c.discipline === 'calisthenics')!.score += 0.12; }
      if (goal.kind === 'fat_loss') { candidates.find(c => c.discipline === 'calisthenics')!.score += 0.10; candidates.find(c => c.discipline === 'gym')!.score += 0.08; }
      if (goal.avoidBulk) { candidates.find(c => c.discipline === 'yoga')!.score += 0.05; candidates.find(c => c.discipline === 'calisthenics')!.score += 0.04; }
    }
    ...
    if (decisionCount >= 5 && memory?.changeSignal === 'stable' && prior) {
      const priorCandidate = candidates.find(c => c.discipline === prior.id);
      if (priorCandidate && prior.count >= 3) { priorCandidate.score += 0.04; priorCandidate.reasons.push(`prior-choice-pattern:${prior.count}`); }
    }
    ...
    if (prior?.count >= 3) reasons.push(`prior-choice-pattern:${prior.count}`);
