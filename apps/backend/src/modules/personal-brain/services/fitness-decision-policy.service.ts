...
export class FitnessDecisionPolicyService {
  evaluate(context: BrainReasoningContext): BrainDecisionResult | null {
    ...
    if (constraints.includes('low_impact')) { candidates.find(c => c.discipline === 'yoga')!.score += 0.15; candidates.find(c => c.discipline === 'calisthenics')!.score -= 0.04; }

    const memory = context.state.lifeContext?.decisionMemory;
    const decisionCount = memory?.decisions ?? 0;
    if (decisionCount >= 5 && memory?.changeSignal === 'stable' && memory.selectedFrequency?.length) {
