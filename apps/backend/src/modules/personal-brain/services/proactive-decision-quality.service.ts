...
export class ProactiveDecisionQualityService {
  evaluate(input: { relevance: number; urgency: number; userBenefit: number; interruptionCost: number; duplicatePenalty?: number; snoozeRate?: number }): ProactiveDecisionQuality {
    ...
    const score = this.clamp(relevance * 0.32 + urgency * 0.25 + userBenefit * 0.33 - interruptionCost * 0.05 - duplicatePenalty * 0.05);
    const confidence = this.clamp(relevance * 0.35 + userBenefit * 0.35 + urgency * 0.15 + (1 - snoozeRate) * 0.15);
