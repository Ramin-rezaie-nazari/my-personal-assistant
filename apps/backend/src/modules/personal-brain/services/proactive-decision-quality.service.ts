...
export class ProactiveDecisionQualityService {
  evaluate(input: { relevance: number; urgency: number; userBenefit: number; interruptionCost: number; duplicatePenalty?: number; snoozeRate?: number }): ProactiveDecisionQuality {
    const relevance = this.clamp(input.relevance);
    const urgency = this.clamp(input.urgency);
    const userBenefit = this.clamp(input.userBenefit);
    const interruptionCost = this.clamp(input.interruptionCost);
    const duplicatePenalty = this.clamp(input.duplicatePenalty ?? 0);
    const snoozeRate = this.clamp(input.snoozeRate ?? 0);
    const score = this.clamp(relevance * 0.32 + urgency * 0.24 + userBenefit * 0.34 - interruptionCost * 0.05 - duplicatePenalty * 0.05);
    const confidence = this.clamp(relevance * 0.34 + userBenefit * 0.34 + urgency * 0.18 + (1 - snoozeRate) * 0.14 - duplicatePenalty * 0.04);
