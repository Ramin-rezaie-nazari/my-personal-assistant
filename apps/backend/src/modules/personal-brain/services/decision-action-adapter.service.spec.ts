...
describe('DecisionActionAdapterService', () => {
  ...
  it('dispatches to the first adapter that supports the action', async () => {
    ...
    service.register({ supports: (item) => item.action === 'start', execute: async () => ({ started: true }) });
    await expect(service.execute(candidate)).resolves.toEqual({ handled: true, status: 'executed', action: 'start', result: { started: true } });
  });

  it('returns unsupported without throwing when no adapter exists', async () => {
    const service = new DecisionActionAdapterService();
    await expect(service.execute(candidate)).resolves.toEqual({ handled: false, status: 'unsupported', action: 'start' });
  });
});
