...
type RawTask = { id: string; title: string; priority: number; estimatedMinutes: number | null; energy: string; dueAt: Date | null; scheduledAt: Date | null; goalId: string | null; goalTitle: string | null; blocked: boolean };
...
export class SmartPlanningService {
  ...
  async getPlan(userId: string, date = new Date()) {
    ...
    const [tasks, adaptive] = await Promise.all([
      this.prisma.$queryRaw<RawTask[]>`SELECT t."id",t."title",t."priority",t."estimatedMinutes",t."energy",t."dueAt",t."scheduledAt",t."goalId",g."title" AS "goalTitle",EXISTS (SELECT 1 FROM "TaskDependency" d JOIN "LifeTask" dep ON dep."id"=d."dependsOnTaskId" WHERE d."taskId"=t."id" AND dep."status" <> 'completed') AS "blocked" FROM "LifeTask" t LEFT JOIN "Goal" g ON g."id"=t."goalId" WHERE t."userId"=${userId} AND t."status" IN ('pending','in_progress') AND (t."scheduledAt" >= ${start} AND t."scheduledAt" < ${end} OR t."scheduledAt" IS NULL) ORDER BY t."priority" ASC,t."dueAt" ASC NULLS LAST`,
