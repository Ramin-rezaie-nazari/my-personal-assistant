...
type Candidate = {
  id: string; title: string; priority: number; estimatedMinutes: number; energyLevel: string;
  dueAt: Date | null; scheduledAt: Date | null; goalId: string | null; score: number; reasons: string[];
};
type TaskRow = {
  id: string; title: string; priority: number; estimatedMinutes: number; energyLevel: string;
  dueAt: Date | null; scheduledAt: Date | null; goalId: string | null; goalTitle: string | null; dependencyStatus: string[];
};
...
export class SmartPlanningService {
  ...
  async getPlan(userId: string, date = new Date()) {
    ...
    const dateKey = getDateKeyInTimezone(requestedDate, timezone);
    const start = zonedDateTimeToUtc(dateKey, '00:00:00', timezone);
    const end = zonedDateTimeToUtc(this.addDays(dateKey, 1), '00:00:00', timezone);
    const prismaRecord = this.prisma as unknown as { lifeTask?: { findMany?: Function } };
    const taskQuery = prismaRecord.lifeTask?.findMany
      ? prismaRecord.lifeTask.findMany({
          where: { userId, status: { in: ['pending', 'in_progress'] }, OR: [{ scheduledAt: { gte: start, lt: end } }, { scheduledAt: null }] },
          include: { goal: true, dependencies: { include: { dependsOnTask: true } } },
    ...
    const [tasks, adaptive] = await Promise.all([taskQuery, this.learning.buildProfile(userId)]);
    const now = new Date();
    const currentHour = getLocalHourWeekday(now, timezone).hour;
    const candidates: Candidate[] = tasks.map((task: any) => {
      const dependencyStatus: string[] = task.dependencyStatus ?? task.dependencies?.map((d: any) => d.dependsOnTask?.status).filter(Boolean) ?? [];
      ...
      if (task.goalId && goalTitle) { score += 8; reasons.push('supports an active goal'); }
      if (adaptive.preferredTaskMinutes && Math.abs((task.estimatedMinutes ?? 0) - adaptive.preferredTaskMinutes) <= 10) { score += 8; reasons.push('matches your usual task size'); }
      ...
      if (dependencyStatus.some((status) => status !== 'completed')) { score -= 100; reasons.push('blocked by another task'); }
      if (adaptive.snoozeRate >= 0.5 && (task.estimatedMinutes ?? 0) > 90) { score -= 8; reasons.push('long task is less suitable when snooze rate is high'); }
      if (adaptive.acceptanceRate >= 0.7 && task.priority <= 1) { score += 4; reasons.push('matches a pattern of accepting important suggestions'); }
      return { id: task.id, title: task.title, priority: task.priority, estimatedMinutes: task.estimatedMinutes ?? 0, energyLevel: task.energyLevel ?? task.energy ?? 'medium', dueAt: task.dueAt, scheduledAt: task.scheduledAt, goalId: task.goalId, score, reasons };
    });
    ...
    return {
      ...
      blocked: candidates.filter((c) => c.reasons.includes('blocked by another task')).slice(0, 10),
      adaptive: { bestHours: adaptive.bestHours, preferredTaskMinutes: adaptive.preferredTaskMinutes, snoozeRate: adaptive.snoozeRate, acceptanceRate: adaptive.acceptanceRate },
    };
  }
  ...
  async replan(userId: string, date = new Date()) {
    ...
    if (!plan.bestAction.scheduledAt) {
      const now = new Date();
      const currentHour = getLocalHourWeekday(now, timezone).hour;
      const preferred = plan.adaptive.bestHours.find((h) => h > currentHour) ?? plan.adaptive.bestHours[0];
      if (preferred !== undefined) {
        const dateKey = getDateKeyInTimezone(new Date(date), timezone);
        const scheduled = zonedDateTimeToUtc(dateKey, `${String(preferred).padStart(2, '0')}:00:00`, timezone);
        if (scheduled > now) {
          const prismaRecord = this.prisma as unknown as { lifeTask?: { update?: Function } };
          if (prismaRecord.lifeTask?.update) await prismaRecord.lifeTask.update({ where: { id: plan.bestAction.id }, data: { scheduledAt: scheduled } });
          else await this.prisma.$executeRaw`UPDATE "LifeTask" SET "scheduledAt"=${scheduled},"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${plan.bestAction.id} AND "userId"=${userId}`;
        }
      }
    }
