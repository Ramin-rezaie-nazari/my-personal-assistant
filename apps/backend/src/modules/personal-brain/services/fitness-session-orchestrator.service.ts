...
import type { Equipment } from '../../calisthenics/models/calisthenics.model';
import type { GymEquipment } from '../../gym/models/gym.model';
...
export class FitnessSessionOrchestratorService {
  ...
  generate(context: BrainReasoningContext, request: FitnessSessionRequest) {
    ...
    if (discipline === 'calisthenics') {
      const allowed: Equipment[] = ['none', 'pull_up_bar', 'parallel_bars', 'rings', 'bench', 'resistance_band', 'dip_belt', 'wall'];
      const mappedEquipment = equipment.filter((value): value is Equipment => allowed.includes(value as Equipment));
    ...
    const mappedGymEquipment = equipment.filter((value): value is GymEquipment => allowedGym.includes(value as GymEquipment));
