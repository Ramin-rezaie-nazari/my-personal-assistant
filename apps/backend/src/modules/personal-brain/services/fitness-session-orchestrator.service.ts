...
export type FitnessSessionRequest = { durationMin: number; level?: string; focus?: string; performance?: FitnessPerformanceSnapshot };
type CalisthenicsEquipment = NonNullable<Parameters<CalisthenicsSessionGeneratorService['generate']>[0]['equipment']>[number];
type GymEquipment = NonNullable<Parameters<GymSessionGeneratorService['generate']>[0]['equipment']>[number];

@Injectable()
export class FitnessSessionOrchestratorService {
  constructor(private readonly policy: FitnessDecisionPolicyService, private readonly yoga: YogaSessionGeneratorService, private readonly calisthenics: CalisthenicsSessionGeneratorService, private readonly gym: GymSessionGeneratorService, @Optional() private readonly progression?: FitnessProgressionService) {}
