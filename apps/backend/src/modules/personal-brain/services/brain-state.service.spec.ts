...
describe('BrainStateService', () => {
  it('returns normalized life context for reasoning', async () => {
    const dailyStatus = { value: 'today' }; const weeklyStatus = { value: 'week' }; const nutritionTargets = { calories: 2200 }; const workoutStatus = { sessions: 2 };
    const lifeContext = { reminders: { pending: 1 }, habits: { active: 2 }, supplements: { total: 3 } };
    const service = new BrainStateService(
      ...
      { build: jest.fn().mockImplementation((_userId, sources) => ({ userId: 'u1', generatedAt: '2026-08-13T04:00:00.000Z', ...Object.fromEntries(Object.entries(sources).map(([key, source]: any) => [key, { ...source, observedAt: source.observedAt, freshness: source.observedAt ? 'fresh' : 'missing', confidence: source.confidence }])) })) } as any,
    );
    const state = await service.buildState('hello', 'u1');
    expect(state.lifeContext.userId).toBe('u1'); expect(state.lifeContext.memory.freshness).toBe('fresh'); expect(state.lifeContext.wearable.freshness).toBe('missing');
  });
});
