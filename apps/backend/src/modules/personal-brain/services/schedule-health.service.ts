...
export class ScheduleHealthService {
  ...
  async evaluate(userId: string, date = new Date()) {
    ...
    const taskMinutes = plan.items
      ...
      .reduce(
        (n, i) =>
          n + (new Date(i.end).getTime() - new Date(i.start).getTime()) / 60000,
        0,
      );
    const fixedMinutes = plan.items
      .filter((i) => i.type !== 'task')
      .reduce(
        (n, i) =>
          n + (new Date(i.end).getTime() - new Date(i.start).getTime()) / 60000,
    ...
    const utilization = Math.round(
      (taskMinutes / Math.max(1, policy.maxFocusedMinutes)) * 100,
    );
    const penalties =
      plan.validation.overlapCount * 30 +
      plan.unscheduled.length * 10 +
      Math.max(0, utilization - 100) / 3;
    const score = Math.max(0, Math.min(100, 100 - penalties));
    const status =
      score >= 80 ? 'healthy' : score >= 55 ? 'strained' : 'overloaded';
    return {
      date: plan.date,
      status,
      ...
      capacity: {
        ...
        fixedMinutes: Math.round(fixedMinutes),
        maxFocusedMinutes: policy.maxFocusedMinutes,
        remainingMinutes: Math.max(0, policy.maxFocusedMinutes - taskMinutes),
        utilization,
      },
      issues: {
      ...
      recommendation:
        status === 'healthy'
          ? 'execute as planned'
          : status === 'strained'
            ? 'protect breaks and move low-priority items'
            : "replan and reduce today's load",
      plan,
    };
  }
}
