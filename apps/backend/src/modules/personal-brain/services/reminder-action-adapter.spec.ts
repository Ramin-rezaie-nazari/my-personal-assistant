...
describe('ReminderActionAdapter', () => {
  it('updates the linked reminder using the owning user', async () => {
    ...
    const reminders = {
      updateReminder: jest
        .fn()
        .mockResolvedValue({ id: 'r1', title: 'Workout' }),
    } as any;
    ...
    await expect(
      adapter.execute(
        ...
        {
          userId: 'u1',
          input: 'همون رو ساعت 08:30 بذار',
          contextualState: { targetExecutionId: 'r1' },
        },
      ),
    ).resolves.toEqual({ id: 'r1', title: 'Workout' });

    expect(reminders.updateReminder).toHaveBeenCalledWith('u1', 'r1', {
      time: '08:30',
    });
