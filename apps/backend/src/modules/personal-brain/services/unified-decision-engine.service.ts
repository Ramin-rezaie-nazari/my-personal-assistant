...
export type UnifiedDecision = {
  ...
  conflicts?: ReturnType<
    DecisionConflictResolutionService['resolve']
  >['conflicts'];
  rationale?: string[];
};
...
export class UnifiedDecisionEngineService {
  constructor(
    private readonly conflicts: DecisionConflictResolutionService = new DecisionConflictResolutionService(),
  ) {}
  decide(
  ...
  private buildRationale(
    ...
    for (const x of selected)
      lines.push(
        `${x.action} was selected because its priority (${this.pct(x.priority ?? 0)}) confidence (${this.pct(x.confidence)}) and score (${this.pct(x.score)}) made it the strongest current option.`,
      );
    for (const x of rejected.slice(0, 3))
      lines.push(
        `${x.action} ranked below the selected option after the current checks.`,
      );
    for (const x of blocked.slice(0, 3))
      lines.push(
        `${x.action} was blocked by ${x.blockedBy?.join(', ') || 'a prerequisite or safety constraint'}.`,
      );
  ...
  private pct(v: number) {
    return `${Math.round(this.clamp(v) * 100)}%`;
  }
  private weight(c: DecisionCandidate, ctx: UnifiedDecisionContext) {
    const p = this.clamp(c.priority ?? 0.5),
      cf = this.clamp(c.confidence),
      s = this.clamp(c.score),
      ga = this.clamp(c.goalAlignment ?? 0.5),
      gd = this.clamp(c.goalDownside ?? 0);
    return (
      p * 0.35 +
      cf * 0.25 +
      s * 0.25 +
      (ctx.longTermPlanning ? ga * 0.15 - gd * 0.1 : 0)
    );
  }
