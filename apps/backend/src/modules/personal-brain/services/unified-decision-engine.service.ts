...
export class UnifiedDecisionEngineService {
  ...
  decide(candidates: DecisionCandidate[], context: UnifiedDecisionContext = {}): UnifiedDecision {
    ...
    return {
      ...
      rationale: this.buildRationale(selected, rejected, blocked, resolved.rationale, reason),
    };
  }
  ...
  private buildRationale(
    ...
    for (const item of rejected.slice(0, 3)) {
      ...
      if ((item.goalAlignment ?? 0) < (selectedItem?.goalAlignment ?? 0)) lines.push(`${item.action} was not prioritized because it matched the current goal less closely.`);
      else if ((item.goalDownside ?? 0) > (selectedItem?.goalDownside ?? 0)) lines.push(`${item.action} was not prioritized because it carried more downside for the current goal.`);
      else lines.push(`${item.action} ranked below the selected option after the current checks.`);
    }
    for (const item of blocked.slice(0, 3)) {
      lines.push(`${item.action} was blocked by ${item.blockedBy?.join(', ') || 'a prerequisite or safety constraint'}.`);
    }
  ...
  private pct(value: number) {
    return `${Math.round(this.clamp(value) * 100)}%`;
  }
