...
export class DecisionExecutionHistoryService {
  ...
  recent(query: ExecutionHistoryQuery = {}, now = Date.now()): DecisionExecutionReceipt[] {
    this.purgeExpired(now);
    const limit = Math.min(Math.max(query.limit ?? 20, 1), 100);
    return this.entries.filter((entry) => (!query.userId || entry.userId === query.userId) && (!query.status || entry.status === query.status) && (!query.action || entry.action === query.action)).slice(-limit).reverse();
  }
  stats(userId?: string, now = Date.now()) {
    this.purgeExpired(now);
    const entries = this.entries.filter((entry) => !userId || entry.userId === userId);
    return entries.reduce((result, entry) => { result.total += 1; result.byStatus[entry.status] = (result.byStatus[entry.status] ?? 0) + 1; result.totalDurationMs += entry.durationMs; result.totalAttempts += entry.attempts; return result; }, { total: 0, totalDurationMs: 0, totalAttempts: 0, byStatus: {} as Record<string, number> });
  }
  ...
  private purgeExpired(now: number): void { for (let index = this.entries.length - 1; index >= 0; index -= 1) if (this.retention.isExpired(this.entries[index].userId, this.entries[index].recordedAt, now)) this.entries.splice(index, 1); }
}
