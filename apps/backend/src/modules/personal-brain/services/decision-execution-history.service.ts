...
export class DecisionExecutionHistoryService {
  ...
  constructor(private readonly retention: DecisionHistoryRetentionService) {}

  record(receipt: DecisionExecutionReceipt): DecisionExecutionReceipt {
    this.entries.push(receipt);
    this.purgeExpired(receipt.recordedAt);
    return receipt;
  }
  ...
  deleteRecentActivity(userId: string, hours: number, now = Date.now()): number {
    const safeHours = Math.min(Math.max(hours, 0.25), 24 * 365);
    const cutoff = now - safeHours * 60 * 60 * 1000;
    const before = this.entries.length;
    for (let index = this.entries.length - 1; index >= 0; index -= 1) {
      const entry = this.entries[index];
      if (entry.userId === userId && entry.recordedAt >= cutoff) this.entries.splice(index, 1);
    }
  ...
  private purgeExpired(now: number): void {
    for (let index = this.entries.length - 1; index >= 0; index -= 1) {
      if (this.retention.isExpired(this.entries[index].userId, this.entries[index].recordedAt, now)) this.entries.splice(index, 1);
    }
  }
}
