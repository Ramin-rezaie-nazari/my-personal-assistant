...
describe('LongTermDecisionImpactService', () => {
  it('rewards alignment and penalizes downside', () => {
    const service = new LongTermDecisionImpactService(
      new GoalHierarchyService(),
    );
    const goals = [
      {
        id: 'g1',
    ...
    const aligned = service.evaluate(goals, {
      id: 'a',
      action: 'workout',
      domain: 'workout',
      goalAlignment: 1,
    ...
    const harmful = service.evaluate(goals, {
      id: 'b',
      action: 'skip-workout',
      domain: 'workout',
      goalAlignment: 0.1,
    ...
    expect(harmful.rationale).toContain(
      'creates long-term trade-off against: Fitness',
    );
  });
});
