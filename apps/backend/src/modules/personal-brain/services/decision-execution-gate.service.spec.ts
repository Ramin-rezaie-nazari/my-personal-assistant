...
describe('DecisionExecutionGateService', () => {
  ...
  it('starts and completes a guarded execution', () => {
    ...
    const service = new DecisionExecutionGateService(
      {
        check: () => ({ allowed: true, key: 'u1:d1:start', reason: 'allowed' }),
        remember: (_u: string, _c: unknown, r: unknown) => remembered.push(r),
      } as any,
      {
        start: () => ({ state: 'running' }),
    ...
    expect(service.complete('u1', candidate, { ok: true }).state).toBe(
      'completed',
    );
  ...
  it('does not start when guardrails reject', () => {
    const service = new DecisionExecutionGateService(
      {
        check: () => ({ allowed: false, key: 'k', reason: 'rate_limited' }),
      } as any,
