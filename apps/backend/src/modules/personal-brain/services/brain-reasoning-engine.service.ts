...
export class BrainReasoningEngineService {
  analyze(reasoningContext: BrainReasoningInput): BrainReasoningResult {
    ...
    if (hasUserGoals && confidence >= 0.85) reasoningSummary = 'Brain has strong context, active goals, and enough life signals for a context-aware decision';
    else if (hasUserGoals) reasoningSummary = 'Brain understands active user goals and is using available life context with some uncertainty';
    else if (confidence >= 0.85) reasoningSummary = 'Brain has strong life context but no active user goals';
    else reasoningSummary = 'Brain requires additional or fresher information before making a high-confidence decision';
