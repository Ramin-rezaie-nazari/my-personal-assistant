...
import type { DecisionCandidate } from './unified-decision-engine.service';
...
export class DecisionExecutionCoordinatorService {
  constructor(
    ...
    @Optional() private readonly confirmation?: ActionConfirmationIntelligenceService,
    @Optional() private readonly audit?: DecisionAuditService,
    @Optional() private readonly outcomeLearning?: DecisionOutcomeLearningService,
  ) {}
  ...
  async execute(userId: string, candidate: DecisionCandidate, context: Record<string, unknown> = {}): Promise<DecisionExecutionReceipt> {
    ...
    const confirmation = this.confirmation?.assess(userId, candidate, context) ?? { required: false, reason: 'confirmation_not_configured' };
  ...
  async confirmAndExecute(userId: string, token: string): Promise<DecisionExecutionReceipt> {
    ...
    const startedAt = Date.now(); const resolved = this.policy.resolve(pending.candidate, pending.context);
  ...
  private async executeApproved(userId: string, candidate: DecisionCandidate, context: Record<string, unknown>, resolved: ReturnType<DecisionExecutionPolicyService['resolve']>, startedAt: number, base: Pick<DecisionExecutionReceipt, 'userId' | 'decisionId' | 'action' | 'domain' | 'recordedAt'>): Promise<DecisionExecutionReceipt> {
    ...
    try {
      ...
      if (!actionResult?.handled) {
        this.gate.fail(userId, candidate, 'unsupported_action'); this.feedback.record({ userId, candidate, outcome: 'skipped' });
      ...
      this.gate.complete(userId, candidate, actionResult.result); this.feedback.record({ userId, candidate, outcome: 'completed' });
      ...
      const message = error instanceof Error ? error.message : String(error); this.gate.fail(userId, candidate, message); this.feedback.record({ userId, candidate, outcome: 'failed' });
