...
import { NotificationFeedbackService, NotificationAction } from './notification-feedback.service';
...
export class NotificationFeedbackAdapterService {
  ...
  recordClientAction(input: { userId: string; eventType: string; dedupeKey: string; action: NotificationClientAction; snoozeUntil?: string; at?: string }) {
    const actionMap: Record<NotificationClientAction, NotificationAction> = {
      open: 'opened', complete: 'completed', snooze: 'snoozed', dismiss: 'dismissed',
    };
