...
export class ActionConfirmationIntelligenceService {
  ...
  assess(userId: string, candidate: DecisionCandidate, context: Record<string, unknown> = {}, now = Date.now()): ActionConfirmation {
    ...
    if (destructive || financial || sensitive || externalImpact) {
      ...
      return { required: true, requirement: 'confirm', reason: financial ? 'financial_action' : sensitive ? 'sensitive_action' : destructive ? 'destructive_action' : 'external_impact', token };
    }
  ...
  consume(userId: string, token: string, now = Date.now()): PendingAction | undefined {
    ...
    if (pending.expiresAt < now) { this.pending.delete(token); return undefined; }
    this.pending.delete(token);
    return pending;
  }
  private buildToken(userId: string, candidate: DecisionCandidate): string {
    let hash = 2166136261;
    for (const char of `${userId}:${candidate.id}:${candidate.action}`) { hash ^= char.charCodeAt(0); hash = Math.imul(hash, 16777619); }
