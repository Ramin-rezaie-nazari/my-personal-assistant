...
export class ActionConfirmationIntelligenceService {
  ...
  assess(
    ...
    const financial =
      candidate.domain === 'shopping' ||
      /purchase|buy|payment|checkout/.test(action);
    const sensitive =
      /health|medication|supplement|medical/.test(action) &&
      /delete|change|take/.test(action);
    const externalImpact =
      /calendar|reminder|notification/.test(candidate.domain) && destructive;
