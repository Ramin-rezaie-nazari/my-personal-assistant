...
export class PlanExecutionService {
  constructor(
    ...
    @Optional() private readonly persistentState?: PersistentPlanStateService,
  ) {}
  ...
  async execute(userId: string, decision: UnifiedDecision, context: Record<string, unknown> = {}): Promise<PlanExecutionResult> {
    ...
    const existing = this.persistentState ? await this.persistentState.resume(userId, planId) : null;
    ...
    for (const step of plan) {
      ...
      if (!dependenciesMet) {
        ...
        await save({ planId, userId, status: 'blocked', stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: step.candidateId, updatedAt: new Date() });
        continue;
      }
      ...
      if (normalized === 'completed' || normalized === 'dry_run') {
        ...
        await save({ planId, userId, status: 'running', stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: null, updatedAt: new Date() });
        continue;
      }
      ...
      await save({ planId, userId, status: completed.length ? 'partial' : 'failed', stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: step.candidateId, updatedAt: new Date() });
      break;
    }
