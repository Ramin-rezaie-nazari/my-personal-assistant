...
export class PlanExecutionService {
  ...
  async execute(userId: string, decision: UnifiedDecision, context: Record<string, unknown> = {}): Promise<PlanExecutionResult> {
    ...
    for (const step of plan) {
      ...
      if (completed.includes(step.candidateId)) continue;
      ...
      await this.persistentState.save({ planId, userId, status: 'running', stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: step.candidateId, updatedAt: new Date() });
      ...
      if (normalized === 'completed' || normalized === 'dry_run') {
        ...
        await this.persistentState.save({ planId, userId, status: 'running', stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: null, updatedAt: new Date() });
        continue;
      }
      ...
      if (!failed.includes(step.candidateId)) failed.push(step.candidateId);
      await this.persistentState.save({ planId, userId, status: completed.length ? 'partial' : 'failed', stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: step.candidateId, updatedAt: new Date() });
      break;
    }
    ...
    const reason = failed.length
      ...
      : blocked.length
        ...
        : unfinished.length
          ? 'plan_waiting_for_resume'
          : 'plan_completed';

    await this.persistentState.save({ planId, userId, status, stepIds: plan.map((item) => item.candidateId), completed, blocked, failed, currentStep: unfinished[0] ?? null, updatedAt: new Date() });
