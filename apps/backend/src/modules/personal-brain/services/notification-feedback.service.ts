...
export type NotificationAction =
  'delivered' | 'opened' | 'completed' | 'snoozed' | 'dismissed' | 'ignored';
...
export class NotificationFeedbackService {
  ...
  getSignal(userId: string, eventType: string): NotificationSignal {
    const items = (this.feedback.get(userId) ?? []).filter(
    ...
    const engaged = items.filter(
      (x) => x.action === 'opened' || x.action === 'completed',
    ).length;
    return {
      ...
      ignored,
      engaged,
      resistanceScore: items.length
        ? (snoozed + dismissed + ignored) / items.length
        : 0,
