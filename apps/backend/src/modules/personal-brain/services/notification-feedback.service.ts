...
export type NotificationSignal = { sampleSize: number; snoozed: number; dismissed: number; ignored: number; engaged: number; resistanceScore: number; engagementScore: number };
...
export class NotificationFeedbackService {
  ...
  getSignal(userId: string, eventType: string): NotificationSignal {
    ...
    return { sampleSize: items.length, snoozed, dismissed, ignored, engaged, resistanceScore: items.length ? (snoozed + dismissed + ignored) / items.length : 0, engagementScore: items.length ? engaged / items.length : 0 };
  }
}
