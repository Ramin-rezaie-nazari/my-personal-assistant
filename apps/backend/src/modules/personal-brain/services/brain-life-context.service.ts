...
export class BrainLifeContextService {
  ...
  async getToday(
    userId: string,
    dateKey?: string,
  ): Promise<BrainLifeContext & { adaptive: Awaited<ReturnType<LearningService['buildProfile']>> }> {
    ...
    const start = zonedDateTimeToUtc(this.addDays(resolvedDateKey, -6), '00:00:00', timezone);
    const [
      habits,
      pendingReminders,
      nextReminder,
      ...
      this.prisma.habit.findMany({
        ...
        include: { logs: { where: { dateKey: { gte: this.key(start), lte: resolvedDateKey } } } },
      ...
      this.prisma.supplement.findMany({
        where: { userId, active: true },
        include: { logs: { where: { dateKey: resolvedDateKey }, take: 1 } },
    ...
    const completedThisWeek = habits.reduce((sum, h) => sum + h.logs.length, 0);
    const possible = habits.reduce((sum, h) => sum + Math.min(h.targetPerWeek, 7), 0);
    const habitItems = habits.map((h) => {
      const dates = new Set(h.logs.map((l) => l.dateKey));
      let streak = 0;
      for (let i = 0; i < 14; i++) {
        const d = new Date(`${resolvedDateKey}T00:00:00.000Z`);
        d.setUTCDate(d.getUTCDate() - i);
        if (!dates.has(this.key(d))) break;
        streak++;
      }
      return { id: h.id, name: h.name, targetPerWeek: h.targetPerWeek, completedThisWeek: h.logs.length, streak };
    });
    ...
    const goalItems = goals.map((g) => ({
      ...
      daysRemaining: g.targetDate ? Math.ceil((g.targetDate.getTime() - end.getTime()) / 86400000) : null,
    }));
    ...
    return {
      habits: {
        ...
        currentStreak: habitItems.length ? Math.max(...habitItems.map((i) => i.streak)) : 0,
        items: habitItems,
      },
      reminders: {
      ...
      supplements: {
        ...
        next: (() => {
          const item = supplements.find((s) => s.logs.length === 0);
          return item ? { id: item.id, name: item.name, dosage: item.dosage, scheduledTime: item.scheduledTime } : null;
        })(),
      },
