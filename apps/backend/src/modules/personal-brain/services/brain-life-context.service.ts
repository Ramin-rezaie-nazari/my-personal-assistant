...
export class BrainLifeContextService {
  ...
  async getToday(
    ...
    BrainLifeContext & {
      adaptive: Awaited<ReturnType<LearningService['buildProfile']>>;
    }
  > {
    ...
    const [
      ...
      this.prisma.habit.findMany({
        ...
        include: {
          logs: { where: { dateKey: { gte: this.key(start), lte: dateKey } } },
        },
      ...
      this.prisma.$queryRaw<
        GoalRow[]
      >`SELECT "id","title","category","priority","progressPercent","targetDate" FROM "Goal" WHERE "userId"=${userId} AND "status"='active' ORDER BY "priority" ASC, "targetDate" ASC NULLS LAST, "createdAt" ASC LIMIT 10`,
    ...
    const possible = habits.reduce(
      (sum, h) => sum + Math.min(h.targetPerWeek, 7),
      0,
    );
    const habitItems = habits.map((h) => {
      ...
      return {
        ...
        completedThisWeek: h.logs.length,
        streak,
      };
    });
    ...
    const goalItems = goals.map((g) => ({
      ...
      daysRemaining: g.targetDate
        ? Math.ceil((g.targetDate.getTime() - end.getTime()) / 86400000)
        : null,
    }));
    const dueSoon = goalItems.filter(
      (g) =>
        g.daysRemaining !== null &&
        g.daysRemaining >= 0 &&
        g.daysRemaining <= 7,
    ).length;
    const averageProgress = goalItems.length
      ? Math.round(
          goalItems.reduce((s, g) => s + g.progressPercent, 0) /
            goalItems.length,
        )
      : 0;
    return {
      habits: {
        ...
        completedThisWeek,
        completionPercent: possible
          ? Math.min(100, Math.round((completedThisWeek / possible) * 100))
          : 0,
        currentStreak: habitItems.length
          ? Math.max(...habitItems.map((i) => i.streak))
          : 0,
        items: habitItems,
      },
      reminders: {
      ...
      supplements: {
        ...
        completionPercent: supplements.length
          ? Math.round((taken / supplements.length) * 100)
          : 0,
        next: (() => {
      ...
      goals: {
        active: goalItems.length,
        dueSoon,
        averageProgress,
        next: goalItems[0] ?? null,
        items: goalItems,
      },
      fitness: {
