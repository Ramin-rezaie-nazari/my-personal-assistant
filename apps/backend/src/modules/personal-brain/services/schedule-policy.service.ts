...
export class SchedulePolicyService {
  getPolicy(adaptive: Adaptive) {
    ...
    const focusWindow = bestHours.length ? { startHour: bestHours[0], endHour: Math.min(22, bestHours[0] + 2) } : { startHour: 9, endHour: 11 };
    return { workingWindow: { startHour: 7, endHour: 22 }, focusWindow, preferredTaskMinutes, maxFocusedMinutes, bufferMinutes: snoozeRate >= 0.4 ? 20 : 15, notificationLeadMinutes: snoozeRate >= 0.5 ? 5 : 10, snoozeRate };
  }
}
