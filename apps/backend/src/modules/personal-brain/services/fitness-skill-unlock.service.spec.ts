...
describe('FitnessSkillUnlockService', () => {
  ...
  const createService = (
    ...
    const library = {
      get: jest.fn((id: string) =>
        id === skill.id ? skill : { id, name: id, difficulty: 3 },
      ),
    } as any;
    const memory = {
      get: jest.fn(async () => ({
  ...
  it('locks a skill when its prerequisite has insufficient history or form', async () => {
    const service = createService([
      { exerciseId: 'split_squat', sessions: 1, latestScore: 0.82 },
    ]);
    const result = await service.evaluateCalisthenicsSkills('u1', [
      'pistol_squat',
    ]);
  ...
  it('marks a skill ready when prerequisite and current performance are strong', async () => {
    const service = createService([
      { exerciseId: 'split_squat', sessions: 4, latestScore: 0.92 },
    ]);
    const result = await service.evaluateCalisthenicsSkills('u1', [
      'pistol_squat',
    ]);
