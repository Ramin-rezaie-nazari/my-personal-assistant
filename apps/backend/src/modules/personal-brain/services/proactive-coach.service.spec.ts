...
describe('ProactiveCoachService', () => {
  it('prioritizes overdue work over normal planning', async () => {
    const recovery = {
      analyze: jest.fn().mockResolvedValue({
        overdue: [{ id: 't1', title: 'Urgent task' }],
        actions: [],
    ...
    const health = {
      evaluate: jest.fn().mockResolvedValue({
        status: 'healthy',
        capacity: { utilization: 50, remainingMinutes: 120 },
      }),
    };
    const planner = {
      getPlan: jest.fn().mockResolvedValue({
        bestAction: { id: 't2', title: 'Normal task', reasons: ['priority'] },
      }),
    };
    const service = new ProactiveCoachService(
      recovery as any,
      health as any,
      planner as any,
    );
    const result = await service.getNextCoach(
      'u1',
  ...
  it('recommends recovery for an overloaded day', async () => {
    ...
    const health = {
      evaluate: jest.fn().mockResolvedValue({
        status: 'overloaded',
        capacity: { utilization: 125, remainingMinutes: 0 },
      }),
    };
    const planner = {
      getPlan: jest.fn().mockResolvedValue({ bestAction: null }),
    };
    const service = new ProactiveCoachService(
      recovery as any,
      health as any,
