...
export class DecisionGuardrailService {
  constructor(
    private readonly idempotency: DecisionIdempotencyService,
    private readonly rateLimiter: DecisionRateLimiterService,
  ) {}

  check(userId: string, candidate: DecisionCandidate, now = Date.now()) {
    const key = `${userId}:${candidate.id}:${candidate.action}`;
    if (this.idempotency.has(key))
      return { allowed: false, reason: 'already_executed', key };
    if (
      !this.rateLimiter.allow(
        `decision:${userId}:${candidate.domain}`,
        10,
        60_000,
        now,
      )
  ...
  remember(userId: string, candidate: DecisionCandidate, result: unknown) {
    this.idempotency.remember(
      `${userId}:${candidate.id}:${candidate.action}`,
      result,
    );
  }
}
