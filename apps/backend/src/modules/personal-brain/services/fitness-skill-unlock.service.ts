...
export class FitnessSkillUnlockService {
  ...
  async evaluateCalisthenicsSkills(userId: string, candidateIds?: string[]): Promise<SkillUnlockStatus[]> {
    const memory = await this.performanceMemory.get(userId, 56);
    const exerciseTrends = new Map(memory.exerciseTrends.map((item) => [item.exerciseId, item]));
    const candidates = (candidateIds?.length
      ? candidateIds.map((id) => this.calisthenicsLibrary.get(id)).filter(Boolean)
      : ['push_up', 'pistol_squat', 'handstand', 'handstand_push_up', 'l_sit', 'muscle_up', 'front_lever_tuck']
    ...
    return candidates.map((skill) => {
      ...
      const prerequisiteResults = prerequisiteIds.map((id) => exerciseTrends.get(id));
      ...
      if (ownTrend && ownTrend.sessions >= 2 && (ownTrend.latestScore ?? 0) >= 0.9) {
        return {
          skillId: skill.id,
          skillName: skill.name,
          status: 'unlocked' as const,
          confidence: Math.min(0.99, 0.72 + Math.min(0.2, ownTrend.sessions / 20)),
          prerequisiteIds,
      ...
      const difficultyGate = skill.difficulty >= 8 ? 0.9 : skill.difficulty >= 6 ? 0.86 : 0.82;
      const ready = missingPrerequisites.length === 0 && memory.averageRecovery !== null && memory.averageRecovery >= 0.7 && (memory.averageForm ?? 0) >= difficultyGate;
      ...
      return {
        ...
        prerequisiteIds,
        missingPrerequisites,
        reason: ready ? 'prerequisites-and-current-performance-are-sufficient' : 'prerequisites-or-current-performance-not-yet-sufficient',
      };
    });
  }
}
