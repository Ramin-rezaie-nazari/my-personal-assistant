...
export class DecisionExplanationService {
  ...
  explainBrain(context: BrainReasoningContext, decision: BrainDecisionPipelineResult): DecisionExplanation {
    ...
    const performance = fitness?.performanceMemory ?? (fitness as any)?.performance;
  ...
  private compose(summary: string, baseDetails: string, confidence: number | null, reasons: string[], rejectedReasons: string[], blockedReasons: string[], historicalReasons: string[], conflictReason?: string): DecisionExplanation {
    const uniqueReasons = this.unique(reasons).slice(0, 8); const uniqueHistorical = this.unique(historicalReasons).slice(0, 5);
