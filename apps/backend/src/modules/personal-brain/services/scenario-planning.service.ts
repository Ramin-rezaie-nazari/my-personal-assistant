...
export class ScenarioPlanningService {
  ...
  private score(state: ScenarioState): number {
    const value = state.goalAlignment * 0.30
      + state.confidence * 0.20
      - state.goalDownside * 0.18
      - state.budgetImpact * 0.10
      - state.capacityImpact * 0.10
      - state.healthImpact * 0.12;
    return Number(Math.max(0, Math.min(1, value)).toFixed(3));
  }

  private recommendation(state: ScenarioState, score: number): Scenario['recommendation'] {
    if (state.healthImpact >= 0.85) return 'unsafe';
