...
export class ScenarioPlanningService {
  compare(input: ScenarioPlanningInput): { scenarios: Scenario[]; best: Scenario | null } {
    const baseline: ScenarioState = {
      goalAlignment: this.clamp(input.baseline?.goalAlignment ?? 0.5), goalDownside: this.clamp(input.baseline?.goalDownside ?? 0),
      budgetImpact: this.clamp(input.baseline?.budgetImpact ?? 0), capacityImpact: this.clamp(input.baseline?.capacityImpact ?? 0),
      healthImpact: this.clamp(input.baseline?.healthImpact ?? 0), confidence: this.clamp(input.baseline?.confidence ?? 0.5),
    };
    const scenarios = input.candidates.map((candidate) => {
      const state: ScenarioState = {
        goalAlignment: this.clamp(candidate.goalAlignment ?? baseline.goalAlignment), goalDownside: this.clamp(candidate.goalDownside ?? baseline.goalDownside),
        ...
        healthImpact: input.context?.healthConstraint && (candidate.domain === 'workout' || candidate.domain === 'nutrition') ? 0.8 : baseline.healthImpact,
  ...
  private score(state: ScenarioState): number { const value = state.goalAlignment * 0.30 + state.confidence * 0.20 - state.goalDownside * 0.18 - state.budgetImpact * 0.10 - state.capacityImpact * 0.10 - state.healthImpact * 0.12; return Number(Math.max(0, Math.min(1, value)).toFixed(3)); }
  private recommendation(state: ScenarioState, score: number): Scenario['recommendation'] { if (state.healthImpact >= 0.7) return 'unsafe'; if (score >= 0.7) return 'best'; if (score >= 0.5) return 'acceptable'; return 'weak'; }
