...
export class DecisionExecutionStateService {
  ...
  start(decisionId: string) {
    return this.set(decisionId, { state: 'running', startedAt: new Date() });
  }
  complete(decisionId: string) {
    return this.set(decisionId, {
      state: 'completed',
      completedAt: new Date(),
    });
  }
  fail(decisionId: string, error: string) {
    return this.set(decisionId, {
      state: 'failed',
      completedAt: new Date(),
      error,
  ...
  private set(decisionId: string, patch: Partial<ExecutionRecord>) {
    ...
    const next: ExecutionRecord = {
      ...(previous ?? { decisionId, state: 'pending' }),
      ...patch,
      decisionId,
    };
