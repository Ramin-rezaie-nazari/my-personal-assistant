...
export class AdaptiveNotificationDecisionService {
  ...
  decide(input: {
    ...
    basePriority: 'critical' | 'high' | 'normal';
    scheduledAt: Date;
    now?: Date;
  }) {
    ...
    const effectivePriority =
      priorityRank[input.basePriority] >=
      priorityRank[recommendation.recommendedMinimumPriority]
        ? input.basePriority
        : recommendation.recommendedMinimumPriority;
    const shiftMinutes =
      recommendation.recommendedAction === 'shift_timing'
        ? recommendation.recommendedQuietHoursExtensionMinutes
        : 0;
    const effectiveAt = new Date(
      input.scheduledAt.getTime() + shiftMinutes * 60000,
    );
    const suppressed =
      recommendation.recommendedAction === 'reduce_frequency' &&
      input.signal.resistanceScore >= 0.85 &&
      input.basePriority !== 'critical';
    return {
      notify: !suppressed,
      effectivePriority,
      effectiveAt,
