...
export class WorkoutActionAdapter implements DecisionActionAdapter {
  ...
  async execute(
    ...
    const durationMinutes = this.extractNumber(
      input,
      /(\d{1,3})\s*(?:min|mins|minute|minutes|دقیقه)/i,
    );
    const caloriesBurned = this.extractNumber(
      input,
      /(\d{2,5})\s*(?:cal|calories|کالری)/i,
    );
    const performedAt = this.extractDateTime(input);
    if (durationMinutes === null && caloriesBurned === null && !performedAt)
      throw new Error(
        'Please provide a workout change such as duration, calories, or date/time',
      );
