...
export class WorkoutActionAdapter implements DecisionActionAdapter {
  constructor(private readonly registry: DecisionActionAdapterService, private readonly workouts: WorkoutService) { registry.register(this); }
  ...
  async execute(candidate: DecisionCandidate, context: Record<string, unknown>) {
    ...
    const workoutId = String(state.targetResourceId ?? state.targetExecutionId ?? ''); if (!workoutId) throw new Error('Missing workout target');
    ...
    const durationMinutes = this.extractNumber(input, /(?:^|\s)(\d{1,3})\s*(?:min|mins|minute|minutes|دقیقه)(?:\s|$)/iu);
    const caloriesBurned = this.extractNumber(input, /(?:^|\s)(\d{2,5})\s*(?:cal|calories|کالری)(?:\s|$)/iu);
  ...
  private extractDateTime(input: string): string | null { const date = input.match(/(?:^|\s)(20\d{2}-\d{2}-\d{2})(?:\s|$)/); const time = input.match(/(?:^|\s)([01]?\d|2[0-3]):([0-5]\d)(?:\s|$)/); if (!date && !time) return null; const base = date?.[1] ?? new Date().toISOString().slice(0, 10); const clock = time ? `${time[1].padStart(2, '0')}:${time[2]}` : '12:00'; return `${base}T${clock}:00.000Z`; }
}
