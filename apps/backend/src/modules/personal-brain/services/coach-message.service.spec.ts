...
describe('CoachMessageService', () => {
  it('returns Persian copy for a critical coach action', async () => {
    const coach = { getNextCoach: jest.fn().mockResolvedValue({ primary: { type: 'start_task', priority: 'critical', title: 'Urgent', message: 'fallback', reason: 'overdue scheduled item' }, alternatives: [], context: {} }) };
