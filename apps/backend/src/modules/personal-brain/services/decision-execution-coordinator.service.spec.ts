...
describe('DecisionExecutionCoordinatorService', () => {
  const candidate = { id: 'd1', domain: 'workout', action: 'start', score: 1, confidence: 1 } as any;
  const policy = {
    resolve: () => ({ timeoutMs: 100, maxAttempts: 2, retryDelayMs: 0, dryRun: false }),
    run: async (_c: any, _p: any, operation: any) => ({ result: await operation(), attempts: [{ attempt: 1 }] }),
  };
  ...
  const build = (overrides: Record<string, unknown> = {}) => {
    ...
    const gate = {
      open: jest.fn(() => ({ allowed: true, key: 'u1:d1:start', reason: 'execution_started' })),
      complete: jest.fn(),
      fail: jest.fn(),
    };
    const adapters = { execute: jest.fn(async () => ({ handled: true, status: 'executed', action: 'start', result: { ok: true } })) };
    const confirmation = {
    ...
    return { service, feedback, history, audit, outcomeLearning, gate, adapters, confirmation };
  };
  ...
  it('runs the full happy path and records completion feedback', async () => {
    ...
    expect(feedback.record).toHaveBeenCalledWith(expect.objectContaining({ outcome: 'completed' }));
    expect(history.record).toHaveBeenCalledWith(expect.objectContaining({ status: 'completed' }));
    expect(outcomeLearning.record).toHaveBeenCalledWith(expect.objectContaining({ source: 'system' }));
  });

  it('turns an unsupported adapter into a safe skipped outcome', async () => {
    const { service, feedback, gate, adapters } = build();
    adapters.execute.mockResolvedValue({ handled: false, status: 'unsupported', action: 'start' });
    const receipt = await service.execute('u1', candidate);
    expect(receipt.status).toBe('unsupported');
    expect(feedback.record).toHaveBeenCalledWith(expect.objectContaining({ outcome: 'skipped' }));
    expect(gate.fail).toHaveBeenCalledWith('u1', candidate, 'unsupported_action');
  });
  ...
  it('converts adapter exceptions into failed execution and learning feedback', async () => {
    ...
    expect(feedback.record).toHaveBeenCalledWith(expect.objectContaining({ outcome: 'failed' }));
    expect(gate.fail).toHaveBeenCalledWith('u1', candidate, 'provider unavailable');
  });

  it('does not execute an action rejected by guardrails', async () => {
    const { service, adapters, gate } = build();
    gate.open.mockReturnValue({ allowed: false, key: 'u1:d1:start', reason: 'already_executed' });
    const receipt = await service.execute('u1', candidate);
  ...
  it('supports dry-run without invoking an adapter', async () => {
    ...
    const dryRunPolicy = { resolve: () => ({ timeoutMs: 100, maxAttempts: 2, retryDelayMs: 0, dryRun: true }), run: execute };
  ...
  it('requires confirmation before executing a sensitive action', async () => {
    ...
    confirmation.assess.mockReturnValue({ required: true, reason: 'confirmation_required', token: 'confirm-1' });
    const receipt = await service.execute('u1', candidate);
