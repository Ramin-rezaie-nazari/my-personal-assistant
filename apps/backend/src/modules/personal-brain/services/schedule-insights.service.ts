...
export class ScheduleInsightsService {
  constructor(private readonly scheduler: FullDaySchedulerService, private readonly policy: SchedulePolicyService, private readonly capacity: DailyCapacityService, private readonly explanation: ScheduleExplanationService) {}
  ...
  async getInsights(userId: string, date = new Date()) {
    ...
    const capacity = this.capacity.estimate({ taskMinutes, maxFocusedMinutes: policy.maxFocusedMinutes, itemCount: plan.items.length, unscheduledCount: plan.unscheduled.length });
    ...
    if (capacity.overloaded) recommendations.push('Your planned focus load is too high; protect breaks or move lower-priority tasks.');
    ...
    const explanations = plan.items.slice(0, 10).map(item => this.explanation.explain(item, { adaptiveMatch: item.type === 'task' && plan.adaptive.bestHours.length > 0, conflictAvoided: plan.validation.isConflictFree }));
    return { date: plan.date, policy, capacity, metrics: { taskMinutes: Math.round(taskMinutes), averageItemMinutes: density, utilization: capacity.utilization }, recommendations, explanations, plan };
  }
}
