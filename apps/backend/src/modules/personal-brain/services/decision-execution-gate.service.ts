...
export type ExecutionGateResult = { allowed: boolean; key: string; reason: string; state?: string };

@Injectable()
export class DecisionExecutionGateService {
  constructor(private readonly guardrails: DecisionGuardrailService, private readonly state: DecisionExecutionStateService) {}

  open(userId: string, candidate: DecisionCandidate, now = Date.now()): ExecutionGateResult {
    ...
    return { allowed: true, key: guard.key, reason: 'execution_started', state: execution.state };
  }
