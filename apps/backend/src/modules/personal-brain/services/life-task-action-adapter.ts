...
export class LifeTaskActionAdapter implements DecisionActionAdapter {
  ...
  supports(candidate: DecisionCandidate) {
    return (
      ...
      candidate.action === 'complete_life_task'
    );
  }
  ...
  async execute(
    ...
    if (task.status === 'completed')
      return { taskId: task.id, status: 'completed', alreadyCompleted: true };

    await this.prisma
      .$executeRaw`UPDATE "LifeTask" SET "status"='completed', "completedAt"=CURRENT_TIMESTAMP, "updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${task.id} AND "userId"=${userId}`;
