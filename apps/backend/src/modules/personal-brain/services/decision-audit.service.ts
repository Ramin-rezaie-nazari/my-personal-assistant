...
type AuditInput = Omit<DecisionAudit, 'id' | 'createdAt'> & { userId?: string };
...
export class DecisionAuditService {
  ...
  record(input: AuditInput): Promise<DecisionAudit> {
    ...
    return this.prisma.decisionAuditEntry.create({
      data: {
        userId,
        decisionId: input.decisionId,
        selectedIds: input.selectedIds,
  ...
  async recent(userId = '', limit = 20): Promise<DecisionAudit[]> {
    if (!this.prisma) return this.memory.filter((entry) => !userId || entry.userId === userId).slice(0, Math.min(Math.max(limit, 1), 50));
  ...
  async byDecision(userId: string, decisionId: string): Promise<DecisionAudit[]> {
    if (!this.prisma) return this.memory.filter((entry) => entry.userId === userId && entry.decisionId === decisionId);
