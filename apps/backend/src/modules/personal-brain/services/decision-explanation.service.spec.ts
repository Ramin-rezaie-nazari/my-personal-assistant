...
describe('DecisionExplanationService', () => {
  ...
  it('explains real user context without exposing raw implementation details', () => {
    const result = service.explainBrain({
      state: { lifeContext: { fitness: { primaryGoal: { active: true, title: 'Stronger lower body', targetAreas: ['legs'], avoidBulk: false }, targetAreas: ['legs'], equipment: ['dumbbells'], performance: { formTrend: 0.12, completionTrend: 0.08, recoveryTrend: 0.05 } } } },
    } as any, {
      canDecide: true, confidence: 0.91, blockers: [], intent: 'fitness-recommendation', recommendation: 'Gym', nextAction: 'Start today workout', message: 'fitness-aware brain decision ready',
    });
  ...
  it('explains proactive coach actions with evidence', () => {
    const result = service.fromCoachAction(
      ...
      'high',
      ['Only about 25 minutes remain.', 'Your schedule is already close to capacity.'],
    );
    expect(result.summary).toContain('Focus on your workout');
    expect(result.details).toContain('25 minutes');
    expect(result.reasons.some((reason) => reason.includes('higher priority'))).toBe(true);
  });
});
