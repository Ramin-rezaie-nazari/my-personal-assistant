...
export class LongTermDecisionImpactService {
  ...
  evaluate(goals: GoalLike[], candidate: GoalAwareCandidate) {
    ...
    if (!ranked.length) return { score: 0, alignment: 0, downside: 0, rationale: 'no active long-term goals available' };
    ...
    const impacts = ranked.map(goal => ({ goalId: goal.id, title: goal.title, impact: this.hierarchy.impact(goal, alignment, downside) }));
    ...
    return { score, alignment, downside, strongestGoal: strongest, rationale: score > 0 ? `supports long-term goal: ${strongest.title}` : score < 0 ? `creates long-term trade-off against: ${strongest.title}` : 'neutral long-term impact', impacts };
  }
  private inferAlignment(candidate: GoalAwareCandidate, goals: Array<GoalLike & { category: string }>) { const text = `${candidate.action} ${candidate.domain}`.toLowerCase(); const matching = goals.filter(goal => `${goal.title} ${goal.category}`.toLowerCase().split(/\s+/).some(token => token.length >= 4 && text.includes(token))); return matching.length ? 0.75 : 0.35; }
