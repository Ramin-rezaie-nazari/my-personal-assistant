...
export class PersonalizationEngineService {
  ...
  upsertSignal(
    ...
    signal: Omit<PreferenceSignal, 'key'> & { key: string },
  ) {
    ...
    const incomingConfidence = Math.max(
      0,
      Math.min(1, signal.confidence ?? 0.5),
    );
    const incomingScore = Math.max(-1, Math.min(1, signal.score));
    const weight =
      incomingConfidence /
      Math.max(0.001, previousConfidence + incomingConfidence);
    const blendedScore = previous
      ? previous.score * (1 - weight) + incomingScore * weight
      : incomingScore;
    profile.signals[key] = {
      ...signal,
      key,
      score: Math.max(-1, Math.min(1, blendedScore)),
      confidence: Math.max(previousConfidence, incomingConfidence),
      source: signal.source ?? 'behavior',
    };
