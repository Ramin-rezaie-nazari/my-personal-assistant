...
export class PersonalizationEngineService {
  ...
  getProfile(userId: string): PersonalizationProfile { return this.profiles.get(userId) ?? { userId, updatedAt: new Date(0), signals: {} }; }
  getSignal(userId: string, domain: PersonalizationDomain, key: string) { return this.getProfile(userId).signals[`${domain}.${key}`] ?? null; }
}
