...
export class NotificationDeliveryQueueService {
  ...
  markDelivered(id: string, now = new Date()) { const job = this.jobs.get(id); if (!job) return null; job.status = 'delivered'; job.deliveredAt = now.toISOString(); return job; }
  markFailed(id: string, error: string, _now = new Date()) { const job = this.jobs.get(id); if (!job) return null; job.attempts += 1; job.lastError = error; job.status = job.attempts >= job.maxAttempts ? 'failed' : 'queued'; return job; }
  expire(now = new Date()) { for (const job of this.jobs.values()) if (job.status === 'queued' && now >= new Date(job.expiresAt)) job.status = 'expired'; return [...this.jobs.values()].filter(job => job.status === 'expired'); }
