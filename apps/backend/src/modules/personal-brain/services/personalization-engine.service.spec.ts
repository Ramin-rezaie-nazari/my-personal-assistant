...
describe('PersonalizationEngineService', () => {
  it('blends repeated behavioral signals and keeps confidence', () => {
    ...
    service.upsertSignal('u1', 'notification', {
      key: 'preferred_channel.push',
      value: 'push',
      score: 0.9,
      confidence: 0.8,
    });
    const result = service.upsertSignal('u1', 'workout', {
      key: 'preferred_time.morning',
      value: 'morning',
      score: 0.7,
      confidence: 0.6,
    });
    ...
    expect(
      service.getProfile('u1').signals['notification.preferred_channel.push']
        .score,
    ).toBeCloseTo(0.9);
  });
