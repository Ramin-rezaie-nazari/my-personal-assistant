...
export class FullDaySchedulerService {
  ...
  async buildDay(userId: string, date = new Date()) {
    ...
    const items: ScheduleItem[] = []; const blocks: Block[] = [];
    ...
    const preferred = adaptive.bestHours[0]; let cursor = new Date(day); if (preferred !== undefined && preferred >= 6 && preferred < 22) cursor.setHours(preferred, 0, 0, 0);
    const conflicts = (start: Date, end: Date) => blocks.some(x => start < x.end && end > x.start);
    const findSlot = (minutes: number) => { let candidate = new Date(cursor); for (let i = 0; i < 96; i++) { const end = new Date(candidate.getTime() + minutes * 60000); if (candidate.getHours() >= 22 || end.getHours() > 22) return null; if (!conflicts(candidate, end)) return { start: candidate, end }; candidate = new Date(candidate.getTime() + 15 * 60000); } return null; };
    const ordered = tasks.map(t => ({ t, blocked: t.dependencies.some(d => d.dependsOnTask.status !== 'completed') })).sort((a, b) => { if (a.blocked !== b.blocked) return a.blocked ? 1 : -1; const ad = a.t.dueAt && a.t.dueAt <= next ? -2 : 0; const bd = b.t.dueAt && b.t.dueAt <= next ? -2 : 0; return (a.t.priority + ad) - (b.t.priority + bd); });
    ...
    for (const { t, blocked } of ordered) {
      ...
      const duration = Math.max(10, Math.min(180, t.estimatedMinutes)); const slot = findSlot(duration);
      if (!slot) { unscheduled.push({ id: t.id, title: t.title, reason: 'no compatible free slot today' }); continue; }
      const item: ScheduleItem = { type: 'task', id: t.id, title: t.title, start: slot.start.toISOString(), end: slot.end.toISOString(), priority: t.priority, reason: t.goal ? 'supports an active goal' : 'priority and deadline' };
      items.push(item); blocks.push({ start: slot.start, end: slot.end, source: 'task', id: t.id }); cursor = new Date(slot.end.getTime() + 15 * 60000);
    }
