...
type RawTask = { id: string; title: string; priority: number; energy: string; scheduledAt: Date | null; dueAt: Date | null; estimatedMinutes: number | null; goalId: string | null; blocked: boolean; goalTitle: string | null };
...
export class FullDaySchedulerService {
  ...
  async buildDay(userId: string, date = new Date()) {
    ...
    const [tasks, reminders, supplements, habits, adaptive] = await Promise.all([
      this.prisma.$queryRaw<RawTask[]>`SELECT t."id",t."title",t."priority",t."energy",t."scheduledAt",t."dueAt",t."estimatedMinutes",t."goalId",(g."id" IS NOT NULL) AS "hasGoal",g."title" AS "goalTitle",EXISTS (SELECT 1 FROM "TaskDependency" d JOIN "LifeTask" dep ON dep."id"=d."dependsOnTaskId" WHERE d."taskId"=t."id" AND dep."status" <> 'completed') AS "blocked" FROM "LifeTask" t LEFT JOIN "Goal" g ON g."id"=t."goalId" WHERE t."userId"=${userId} AND t."status" IN ('pending','in_progress') AND (t."dueAt" <= ${next} OR (t."scheduledAt" >= ${day} AND t."scheduledAt" < ${next}) OR t."scheduledAt" IS NULL) ORDER BY t."priority" ASC,t."dueAt" ASC NULLS LAST`,
