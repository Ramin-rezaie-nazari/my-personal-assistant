...
export type ScheduleItem = {
  type: 'task' | 'reminder' | 'supplement' | 'habit';
  id: string;
  title: string;
  start: string;
  end: string;
...
export class FullDaySchedulerService {
  ...
  async buildDay(userId: string, date = new Date()) {
    ...
    const blocks: Block[] = [];
    const addFixed = (item: ScheduleItem) => {
      items.push(item);
      blocks.push({
    ...
    for (const r of reminders)
      addFixed({
        ...
        end: new Date(r.scheduledAt.getTime() + 10 * 60000).toISOString(),
        priority: 1,
        reason: 'existing reminder',
      });
    for (const s of supplements) {
      ...
      if (Number.isFinite(h) && Number.isFinite(m)) {
        ...
        addFixed({
          ...
          end: new Date(start.getTime() + 5 * 60000).toISOString(),
          priority: 2,
          reason: 'scheduled supplement',
        });
      }
    }
    for (const h of habits) {
      ...
      addFixed({
        ...
        title: h.name,
        start: start.toISOString(),
        end: new Date(start.getTime() + 10 * 60000).toISOString(),
        priority: 3,
        reason: 'daily habit',
      });
    }
    ...
    if (
      preferred !== undefined &&
      preferred >= policy.workingWindow.startHour &&
      preferred < policy.workingWindow.endHour
    )
      cursor.setHours(preferred, 0, 0, 0);
    else cursor.setHours(policy.focusWindow.startHour, 0, 0, 0);
    const conflicts = (start: Date, end: Date) =>
      blocks.some((x) => start < x.end && end > x.start);
    const findSlot = (minutes: number) => {
      let candidate = new Date(cursor);
      for (let i = 0; i < 96; i++) {
        const end = new Date(candidate.getTime() + minutes * 60000);
        if (
          candidate.getHours() >= policy.workingWindow.endHour ||
          end.getHours() > policy.workingWindow.endHour
        )
          return null;
    ...
    const ordered = tasks
      .map((t) => ({
        t,
        blocked: t.dependencyStatus.some((status) => status !== 'completed'),
      }))
      .sort((a, b) => {
    ...
    for (const { t, blocked } of ordered) {
      ...
      const duration = Math.max(
        ...
        Math.min(
          180,
          t.estimatedMinutes ?? adaptive.preferredTaskMinutes ?? 30,
        ),
      );
    ...
    const overlapCount = items.reduce(
      (count, item, index) =>
        count +
        items
          .slice(index + 1)
    ...
    return {
      ...
      summary: {
        ...
        supplements: items.filter((i) => i.type === 'supplement').length,
