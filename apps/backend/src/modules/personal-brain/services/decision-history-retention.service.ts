...
export class DecisionHistoryRetentionService {
  ...
  cutoff(userId: string, now = Date.now()): number | null {
    ...
    const months: Record<Exclude<HistoryRetention, 'unlimited'>, number> = { '1_month': 1, '3_months': 3, '6_months': 6, '1_year': 12 };
    const date = new Date(now);
    date.setUTCMonth(date.getUTCMonth() - months[retention]);
    return date.getTime();
  }
