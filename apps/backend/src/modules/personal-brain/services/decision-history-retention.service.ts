...
export class DecisionHistoryRetentionService {
  ...
  setPolicy(userId: string, policy: HistoryRetentionPolicy) {
    ...
    if (normalized.deleteRecentActivityHours !== undefined) normalized.deleteRecentActivityHours = Math.min(Math.max(normalized.deleteRecentActivityHours, 0.25), 24 * 365);
    this.policies.set(userId, normalized); return normalized;
  }
  ...
  cutoff(userId: string, now = Date.now()): number | null {
    ...
    const durations: Record<Exclude<HistoryRetention, 'unlimited'>, number> = { '1_month': 30, '3_months': 90, '6_months': 180, '1_year': 365 };
    return now - durations[retention] * 24 * 60 * 60 * 1000;
  }
