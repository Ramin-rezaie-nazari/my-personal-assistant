...
describe('DecisionGuardrailService', () => {
  ...
  it('allows a new decision and remembers its result', () => {
    const service = new DecisionGuardrailService(
      { has: () => false, remember: jest.fn() } as any,
      { allow: () => true } as any,
    );
  ...
  it('blocks duplicate and rate-limited decisions', () => {
    const duplicate = new DecisionGuardrailService(
      { has: () => true } as any,
      { allow: () => true } as any,
    );
    expect(duplicate.check('u1', candidate).reason).toBe('already_executed');
    const limited = new DecisionGuardrailService(
      { has: () => false } as any,
      { allow: () => false } as any,
    );
