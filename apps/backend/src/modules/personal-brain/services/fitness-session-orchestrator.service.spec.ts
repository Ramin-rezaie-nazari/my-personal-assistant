...
describe('FitnessSessionOrchestratorService', () => {
        ...
        lifeContext: {
          ...
          fitness: {
            ...
            primaryGoal: {
              ...
              title: 'Body sculpt',
              targetAreas: ['thighs'],
              desiredOutcome: 'Improve fitness',
              priority: 1,
              avoidBulk: true,
              active: true,
            },
  ...
  it('generates calisthenics when bodyweight-only context wins', () => {
    const policy = {
      evaluate: () => ({
        ...
        recommendation: 'Best training branch today: calisthenics',
        confidence: 0.9,
        blockers: [],
        canDecide: true,
      }),
    ...
    const service = new FitnessSessionOrchestratorService(
      policy,
      yoga,
      calisthenics,
      gym,
    );
    const result = service.generate(context('calisthenics', ['none']), {
      durationMin: 30,
    });
  ...
  it('generates yoga when policy selects yoga', () => {
    const policy = {
      evaluate: () => ({
        intent: 'fitness-recommendation',
        recommendation: 'Best training branch today: yoga',
        confidence: 0.9,
        blockers: [],
        canDecide: true,
      }),
    ...
    const result = service.generate(context('yoga', ['yoga_mat']), {
      durationMin: 45,
    });
    expect(result.status).toBe('generated');
  ...
  it('generates gym when policy selects gym', () => {
    const policy = {
      evaluate: () => ({
        intent: 'fitness-recommendation',
        recommendation: 'Best training branch today: gym',
        confidence: 0.9,
        blockers: [],
        canDecide: true,
      }),
    ...
    const result = service.generate(context('gym', ['dumbbells']), {
      durationMin: 45,
    });
    expect(result.status).toBe('generated');
    expect(result.discipline).toBe('gym');
    expect(gym.generate).toHaveBeenCalledWith(
      expect.objectContaining({ equipment: ['dumbbells'], avoidBulk: true }),
    );
  });
});
