...
export class NotificationChannelIntelligenceService {
  rank(
    ...
    allowed: NotificationChannel[] = ['push', 'in_app', 'email', 'web_push'],
  ): ChannelRecommendation[] {
    return allowed
      .map((channel) => {
        const s = signals.find((x) => x.channel === channel) ?? {
          channel,
          delivered: 0,
          opened: 0,
          completed: 0,
        ...
        if (s.delivered === 0)
          return {
            channel,
            score: 0.5,
            reason: 'insufficient_delivery_history',
          };
        ...
        const score = Math.max(
          ...
          Math.min(
            1,
            openRate * 0.45 + completionRate * 0.45 + (1 - negativeRate) * 0.1,
          ),
        );
        return {
          channel,
          score,
          reason: 'ranked_from_delivery_and_engagement_history',
        };
      })
