...
export type BrainGoalSnapshot = { id: string; title: string; category: string; priority: number; progressPercent: number; targetDate: string | null; daysRemaining: number | null };
export type BrainFitnessPerformanceMemory = { windowDays: number; sessions: number; averageForm: number | null; averageCompletion: number | null; averageDifficulty: number | null; averageRecovery: number | null; formTrend: number | null; completionTrend: number | null; recoveryTrend: number | null; disciplineSummary: Record<string, { sessions: number; averageForm: number | null; averageDifficulty: number | null }>; exerciseTrends: Array<{ exerciseId: string | null; exerciseName: string | null; sessions: number; firstScore: number | null; latestScore: number | null; scoreTrend: number | null; latestReps: number | null; latestLoadKg: number | null }> };
...
export type BrainFitnessContext = { disciplines: string[]; primaryGoal: { id: string; kind: string; title: string; targetAreas: string[]; desiredOutcome: string; priority: number; avoidBulk: boolean; active: boolean } | null; equipment: string[]; constraints: string[]; targetAreas: string[]; performanceMemory?: BrainFitnessPerformanceMemory };
