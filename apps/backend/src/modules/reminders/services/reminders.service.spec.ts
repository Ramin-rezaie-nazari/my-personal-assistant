...
describe('RemindersService', () => {
  ...
  it('trims titles and defaults blank types to general', async () => {
    ...
    await service.createReminder('u1', {
      title: '  Water  ',
      type: '   ',
      time: '09:30',
    });
  ...
  it('rejects malformed times', async () => {
    ...
    ).rejects.toThrow('time must use HH:MM format');
    await expect(
      service.createReminder('user-1', {
        title: 'Test',
        type: 'general',
    ...
    ).rejects.toThrow('time must be a valid time');
    await expect(
      service.createReminder('user-1', {
        title: 'Test',
        type: 'general',
  ...
  it('rejects an invalid user timezone', async () => {
    ...
    ).rejects.toThrow('invalid user timezone');
  });
  ...
  it('completes and reopens only owned reminders', async () => {
    ...
    await expect(service.completeReminder('u1', 'r1')).resolves.toEqual({
      id: 'r1',
      completed: true,
    });
    await expect(service.reopenReminder('u1', 'r1')).resolves.toEqual({
      id: 'r1',
      completed: false,
    });
    expect(prisma.reminder.updateMany).toHaveBeenNthCalledWith(1, {
      where: { id: 'r1', userId: 'u1', completed: false },
      data: { completed: true },
    });
    expect(prisma.reminder.updateMany).toHaveBeenNthCalledWith(2, {
      where: { id: 'r1', userId: 'u1', completed: true },
  ...
  it('rejects completing or reopening another user reminder', async () => {
    ...
    await expect(
      service.completeReminder('u1', 'other'),
    ).rejects.toBeInstanceOf(NotFoundException);
    await expect(service.reopenReminder('u1', 'other')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('updates title and time only for the authenticated owner', async () => {
    ...
    prisma.reminder.findFirstOrThrow.mockResolvedValue({
      id: 'r1',
      title: 'Updated',
      type: 'health',
      scheduledAt: new Date(),
    ...
    await expect(
      service.updateReminder('u1', 'r1', {
        title: '  Updated  ',
        time: '18:30',
      }),
    ).resolves.toMatchObject({ id: 'r1', title: 'Updated' });
  ...
  it('rejects empty update patches and empty titles', async () => {
    ...
    await expect(
      service.updateReminder('u1', 'r1', { title: '   ' }),
  ...
  it('returns not found when updating or deleting another user reminder', async () => {
    ...
    await expect(
      service.updateReminder('u1', 'other', { title: 'Updated' }),
    ).rejects.toBeInstanceOf(NotFoundException);
    await expect(service.deleteReminder('u1', 'other')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('deletes only an owned reminder', async () => {
    ...
    await expect(service.deleteReminder('u1', 'r1')).resolves.toEqual({
      id: 'r1',
      deleted: true,
    });
  ...
  it('returns the next pending reminder', async () => {
    ...
    await expect(service.getNextReminder('u1')).resolves.toMatchObject({
      id: 'r2',
      completed: false,
    });
