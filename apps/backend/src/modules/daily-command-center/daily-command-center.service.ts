...
export class DailyCommandCenterService {
  ...
  async getToday(userId: string) {
    ...
    const supplementsTaken = supplements.filter(
      (item) => item.logs.length > 0,
    ).length;
    const priorities: string[] = [];

    if (unreadNotifications > 0)
      priorities.push(
        `Review ${unreadNotifications} unread assistant notification${unreadNotifications > 1 ? 's' : ''}`,
      );
    if (calendarEvents.length > 0)
      priorities.push(
        `You have ${calendarEvents.length} scheduled event${calendarEvents.length > 1 ? 's' : ''} today`,
      );
    if (!daily) priorities.push('Start your daily log');
    if (
      nutritionProfile?.waterGoalMl &&
      (daily?.waterMl ?? 0) < nutritionProfile.waterGoalMl * 0.5
    )
      priorities.push('Catch up on water');
    if (
      nutritionProfile?.proteinGoalGrams &&
      (daily?.protein ?? 0) < nutritionProfile.proteinGoalGrams * 0.5
    )
      priorities.push('Boost your protein');
    if (habits.length && habitsDone < habits.length)
      priorities.push(
        `Complete ${habits.length - habitsDone} habit${habits.length - habitsDone > 1 ? 's' : ''}`,
      );
    if (supplements.length && supplementsTaken < supplements.length)
      priorities.push(
        `Take ${supplements.length - supplementsTaken} supplement${supplements.length - supplementsTaken > 1 ? 's' : ''}`,
      );
    ...
    return {
      ...
      workouts: {
        countToday: workouts.length,
        latest: workouts[0]
          ? {
              name: workouts[0].name,
              type: workouts[0].type,
              durationMinutes: workouts[0].durationMinutes,
            }
          : null,
      },
    };
