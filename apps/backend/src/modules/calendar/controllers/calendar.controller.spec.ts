  ...
  it('returns service results unchanged', async () => {
    ...
    await expect(
      controller.createEvent(
        { user: { id: 'u1' } },
        { title: 'x', type: 'general', startsAt: '2026-08-14T18:00:00Z' },
      ),
    ...
    await expect(
      controller.updateEvent({ user: { id: 'u1' } }, 'e1', { title: 'x' }),
    ).resolves.toEqual({ id: 'e1' });
    await expect(
      controller.complete({ user: { id: 'u1' } }, 'e1'),
    ).resolves.toEqual({ id: 'e1', completed: true });
    await expect(
      controller.reopen({ user: { id: 'u1' } }, 'e1'),
    ).resolves.toEqual({ id: 'e1', completed: false });
    await expect(
      controller.delete({ user: { id: 'u1' } }, 'e1'),
    ).resolves.toEqual({ id: 'e1', deleted: true });
  });
});
