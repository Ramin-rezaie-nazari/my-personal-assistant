...
describe('CalendarService', () => {
  ...
  it('normalizes title and defaults missing or blank type to general', async () => {
    const prisma = makePrisma();
    prisma.reminder.create.mockResolvedValue(event({ title: 'Gym', type: 'general' }));
    const service = new CalendarService(prisma as never);

    await service.createEvent('u1', { title: '  Gym  ', type: '   ', startsAt: '2026-08-14T18:00:00Z' });
    expect(prisma.reminder.create).toHaveBeenCalledWith({
  ...
  it('rejects missing or blank titles and invalid dates', async () => {
    ...
    await expect(service.createEvent('u1', { title: '', type: 'general', startsAt: '2026-08-14T18:00:00Z' })).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.createEvent('u1', { title: '   ', type: 'general', startsAt: '2026-08-14T18:00:00Z' })).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.createEvent('u1', { title: 'Test', type: 'general', startsAt: '' })).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('rejects invalid event ranges, including equal start and end', async () => {
    ...
    await expect(service.createEvent('u1', { title: 'Test', type: 'general', startsAt: '2026-08-14T19:00:00Z', endsAt: '2026-08-14T19:00:00Z' })).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('normalizes update fields and clears an end time explicitly', async () => {
    const prisma = makePrisma();
    prisma.reminder.findFirst
      .mockResolvedValueOnce(event({ endsAt: new Date('2026-08-14T19:00:00Z') }))
      .mockResolvedValueOnce(event({ title: 'Dinner', type: 'general', endsAt: null }));
    prisma.reminder.updateMany.mockResolvedValue({ count: 1 });
    const service = new CalendarService(prisma as never);
    await service.updateEvent('u1', 'e1', { title: '  Dinner  ', type: '   ', endsAt: null });
    expect(prisma.reminder.updateMany).toHaveBeenCalledWith({
      where: { id: 'e1', userId: 'u1' },
  ...
  it('rejects empty updates, empty titles, invalid dates, and invalid ranges', async () => {
    ...
    await expect(service.updateEvent('u1', 'e1', { title: '   ' })).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.updateEvent('u1', 'e1', { startsAt: 'nope' })).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('updates time safely and rejects malformed or out-of-range times', async () => {
    ...
    expect(prisma.reminder.updateMany).toHaveBeenCalledWith({ where: { id: 'e1', userId: 'u1' }, data: { scheduledAt: new Date('2026-08-14T19:00:00Z') } });
    await expect(service.updateEventTime('u1', 'e1', '9:00')).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.updateEventTime('u1', 'e1', '24:00')).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.updateEventTime('u1', 'e1', '12:60')).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.updateEventTime('u1', 'e1', 'abc')).rejects.toBeInstanceOf(BadRequestException);
  });

  it('rejects a time change that would move the start after the existing end', async () => {
    const prisma = makePrisma();
    prisma.reminder.findFirst.mockResolvedValue(event({ endsAt: new Date('2026-08-14T18:30:00Z') }));
    const service = new CalendarService(prisma as never);
    await expect(service.updateEventTime('u1', 'e1', '19:00')).rejects.toBeInstanceOf(BadRequestException);
    expect(prisma.reminder.updateMany).not.toHaveBeenCalled();
  });
  ...
  it('rejects time changes for another user event', async () => {
    ...
    prisma.reminder.findFirst.mockResolvedValue(null);
    const service = new CalendarService(prisma as never);
    await expect(service.updateEventTime('u1', 'other', '19:00')).rejects.toBeInstanceOf(NotFoundException);
    expect(prisma.reminder.updateMany).not.toHaveBeenCalled();
  });
  ...
  it('completes, reopens, and deletes owned events', async () => {
    ...
    expect(prisma.reminder.updateMany).toHaveBeenNthCalledWith(1, { where: { id: 'e1', userId: 'u1', completed: false }, data: { completed: true } });
    expect(prisma.reminder.updateMany).toHaveBeenNthCalledWith(2, { where: { id: 'e1', userId: 'u1', completed: true }, data: { completed: false } });
    expect(prisma.reminder.deleteMany).toHaveBeenCalledWith({ where: { id: 'e1', userId: 'u1' } });
  });
