...
export class CalendarService {
  ...
  async createEvent(
    userId: string,
    data: { title: string; type: string; startsAt: string; endsAt?: string },
  ) {
    const title = typeof data.title === 'string' ? data.title.trim() : '';
    if (!title) throw new BadRequestException('title is required');
    const type =
      typeof data.type === 'string' ? data.type.trim() || 'general' : 'general';
  ...
  private async findOwnedEvent(userId: string, eventId: string) {
    const event = await this.prisma.reminder.findFirst({
      where: { id: eventId, userId },
    });
