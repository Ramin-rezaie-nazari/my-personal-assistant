...
export class GymSessionGeneratorService {
  ...
  generate(input: {
    ...
    const durationMin = Math.min(
      120,
      Math.max(10, Math.round(input.durationMin)),
    );
    const equipment: GymEquipment[] = input.equipment?.length
      ? input.equipment
      : ['none'];
    ...
    const steps = selected.map((exercise, index) => ({
      ...
      sets: Math.max(
        ...
        Math.min(
          ...
          Math.round(
            exercise.setsMin +
              (exercise.setsMax - exercise.setsMin) * 0.5 * volumeFactor,
          ),
        ),
      ),
      repsMin: exercise.repsMin,
      repsMax: exercise.repsMax,
      restSec: input.avoidBulk
        ? Math.max(45, Math.round(exercise.restSec * 0.82))
        : exercise.restSec,
      ...
      ? Number(
          (
            selected.reduce(
              (sum, item) =>
                sum +
                (item.level === level ? 0.65 : 0.45) +
                (item.compound ? 0.15 : 0),
              0,
            ) / selected.length
          ).toFixed(2),
        )
  ...
  private adaptLevel(
    ...
    if (
      progress.formScoreAvg >= 0.9 &&
      progress.completionRate >= 0.9 &&
      (progress.recentDifficulty ?? 0) < 0.7
    )
