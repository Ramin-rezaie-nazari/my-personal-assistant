...
describe('GymSessionGeneratorService', () => {
  ...
  it('uses available equipment when generating a session', () => {
    const session = service.generate({
      durationMin: 40,
      equipment: ['dumbbells'],
      focus: 'shoulders',
    });
    ...
    expect(
      session.steps.every((step) => step.exerciseId === 'lateral-raise'),
    ).toBe(true);
  });
  ...
  it('does not select unavailable cable-only exercises', () => {
    ...
    expect(
      session.steps.some(
        (step) =>
          step.exerciseId === 'cable-row' || step.exerciseId === 'lat-pulldown',
      ),
    ).toBe(false);
  });

  it('reduces volume when avoidBulk is true', () => {
    const normal = service.generate({
      durationMin: 40,
      equipment: ['dumbbells'],
      focus: 'full_body',
      avoidBulk: false,
    });
    const sculpt = service.generate({
      durationMin: 40,
      equipment: ['dumbbells'],
      focus: 'full_body',
      avoidBulk: true,
