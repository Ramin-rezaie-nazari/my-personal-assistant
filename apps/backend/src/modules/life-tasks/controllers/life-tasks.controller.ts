...
export class LifeTasksController {
  ...
  @Post() create(@Request() req: any, @Body() dto: CreateLifeTaskDto) {
    return this.service.create(req.user.id, dto);
  }
  @Get() all(@Request() req: any, @Query('status') status?: string) {
    return this.service.findAll(req.user.id, status);
  }
  @Get('next-best') next(@Request() req: any) {
  ...
  @Post(':id/events') event(
    ...
    return this.service.event(req.user.id, id, dto);
  }
  @Post(':id/dependencies/:dependsOnTaskId') dependency(
    @Request() req: any,
    @Param('id') id: string,
    @Param('dependsOnTaskId') dependsOnTaskId: string,
  ) {
    return this.service.addDependency(req.user.id, id, dependsOnTaskId);
  }
}
