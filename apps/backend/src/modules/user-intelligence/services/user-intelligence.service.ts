...
export class UserIntelligenceService {
  ...
  async analyzeBehavior(userId: string) {
    ...
    if (profile.bestHours.length) {
      ...
      await this.learning.createInsight(
        userId,
        'Best focus window',
        `The user has the strongest observed completion rate around ${hour}:00.`,
        profile.patterns[0]?.confidence ?? 0.5,
        2,
      );
    }
    if (profile.snoozeRate >= 0.35) {
      await this.learning.createInsight(
        userId,
        'High snooze signal',
        'Many actions are being snoozed; future plans should use fewer, better-timed prompts.',
        Math.min(0.95, profile.snoozeRate),
        2,
      );
    }
    if (profile.preferredTaskMinutes) {
      await this.learning.createInsight(
        userId,
        'Preferred task size',
        `Observed tasks average about ${profile.preferredTaskMinutes} minutes.`,
        0.6,
        1,
      );
    }
