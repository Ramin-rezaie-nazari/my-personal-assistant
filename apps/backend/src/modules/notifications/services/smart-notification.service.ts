...
export class SmartNotificationService {
  ...
  async generateForUser(
    ...
    const [daily, nutrition, habits, supplements, workouts] = await Promise.all(
      [
        ...
        this.prisma.workout.findMany({
          where: {
            userId,
            performedAt: { gte: new Date(`${dateKey}T00:00:00.000Z`) },
          },
          take: 1,
        }),
      ],
    ...
    if (
      nutrition?.waterGoalMl &&
      (daily?.waterMl ?? 0) < nutrition.waterGoalMl * 0.5
    ) {
    ...
    if (
      ...
      (daily?.protein ?? 0) < nutrition.proteinGoalGrams * 0.5
    ) {
      rules.push('protein-low');
      notifications.push({
        userId,
        title: fa
          ? 'پروتئین امروز عقب افتاده 🥩'
          : 'Protein is lagging today 🥩',
        body: fa
    ...
    if (pendingHabits.length > 0) {
      ...
      notifications.push({
        userId,
        title: fa
          ? `${pendingHabits.length} عادت هنوز بازه ✅`
          : `${pendingHabits.length} habit${pendingHabits.length > 1 ? 's are' : ' is'} still open ✅`,
        body: fa
          ? `برای حفظ ریتم امروزت ${names} رو انجام بده.`
          : `Keep your rhythm going with ${names}.`,
        type: 'habit',
        scheduledAt: now,
    ...
    if (pendingSupplements.length > 0) {
      ...
      const names = pendingSupplements
        .slice(0, 2)
        .map((item) => item.name)
        .join(fa ? ' و ' : ' and ');
      notifications.push({
        ...
        body: fa
          ? `${names} هنوز برای امروز ثبت نشده.`
          : `${names} ${pendingSupplements.length > 1 ? 'are' : 'is'} still marked as not taken today.`,
        type: 'supplement',
        scheduledAt: now,
    ...
    if (workouts.length === 0) {
      ...
      notifications.push({
        userId,
        title: fa
          ? 'کمی تحرک امروزت رو بهتر می‌کنه 🏃'
          : 'A little movement would help 🏃',
        body: fa
          ? 'امروز هنوز تمرینی ثبت نکردی؛ حتی یک پیاده‌روی کوتاه هم حساب می‌شه.'
          : 'You have not logged a workout today. Even a short walk counts.',
        type: 'workout',
        scheduledAt: now,
