...
import { FoodRecommendationDto } from '../dto/food-recommendation.dto';
...
export class RecommendationIntelligenceController {
  ...
  @Post('food')
  async food(@Request() req: { user: { id: string } }, @Body() body: FoodRecommendationDto) {
    const candidates = await this.engine.generateFoodRecommendations(req.user.id, body.limit ?? 10);
    return {
      recommendations: this.ranking.rankRecommendations(candidates),
