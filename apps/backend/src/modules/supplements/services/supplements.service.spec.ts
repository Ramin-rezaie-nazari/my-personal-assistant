...
describe('SupplementsService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new SupplementsService(prisma as never);
  });

  it('creates a user-scoped supplement with a default schedule', async () => {
    prisma.supplement.create.mockResolvedValue({
      id: 's1',
      userId: 'u1',
      name: 'Vitamin D',
      scheduledTime: '09:00',
    ...
    expect(prisma.supplement.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ userId: 'u1', scheduledTime: '09:00' }),
      }),
    );
  });
  ...
  it('enforces ownership', async () => {
    ...
    await expect(
      service.takeToday('u1', 'foreign', '2026-08-12'),
    ).rejects.toThrow('Supplement not found');
    await expect(service.deleteSupplement('u1', 'foreign')).rejects.toThrow(
      'Supplement not found',
    );
  });

  it('logs a daily dose idempotently', async () => {
    prisma.supplement.findFirst.mockResolvedValue({
      id: 's1',
      userId: 'u1',
      active: true,
    });
    ...
    expect(prisma.supplementLog.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          supplementId_dateKey: { supplementId: 's1', dateKey: '2026-08-12' },
        },
      }),
    );
  });
