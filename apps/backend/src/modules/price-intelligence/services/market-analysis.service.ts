...
export class MarketAnalysisService {
  ...
  analyze(
    ...
    const thirty = snapshots
      .filter(
        (item) => now.getTime() - item.observedAt.getTime() <= 30 * 86400000,
      )
      .map((item) => item.amount);
    const average = (values: number[]) =>
      values.length ? values.reduce((a, b) => a + b, 0) / values.length : null;
    const avg7 = average(seven),
      avg30 = average(thirty);
    const min30 = thirty.length ? Math.min(...thirty) : null;
    const max30 = thirty.length ? Math.max(...thirty) : null;
    const change = (avg: number | null) =>
      avg === null || avg === 0 ? null : (latest - avg) / avg;
    const change7 = change(avg7),
      change30 = change(avg30);
    const trend =
      change30 === null
        ? 'insufficient_data'
        : change30 < -0.05
    ...
    const buyScore = Math.max(
      ...
      Math.min(
        ...
        (change30 !== null && change30 < 0
          ? 0.5 + Math.min(0.5, Math.abs(change30) * 3)
          : 0.35) +
          (lowVs30 - 0.5) * 0.35,
      ),
    );
