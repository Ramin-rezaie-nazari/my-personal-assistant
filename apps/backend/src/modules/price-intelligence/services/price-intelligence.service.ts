...
export class PriceIntelligenceService {
  ...
  async analyze(productKey: string) {
    const rows = (await this.persistence.history(productKey, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))) as Array<{ amount: number; unitPrice?: number | null; observedAt: Date; currency: string; unit?: string | null }>;
    const prices = rows
      .filter((row) => String(row.currency).toUpperCase() === 'IRT')
      .map((row) => ({ value: Number(row.unitPrice ?? row.amount), observedAt: new Date(row.observedAt) }))
    ...
    const avg30 = average(prices.map((row) => row.value));
    const weekRows = prices.filter((row) => row.observedAt.getTime() >= Date.now() - 7 * 24 * 60 * 60 * 1000);
    const avg7 = average(weekRows.map((row) => row.value));
    const previous7 = prices.length > 7 ? average(prices.slice(0, -Math.min(7, prices.length)).map((row) => row.value)) : null;
    const previous30 = prices.length > 30 ? average(prices.slice(0, -30).map((row) => row.value)) : null;
    ...
    const min30 = Math.min(...prices.map((row) => row.value));
    const max30 = Math.max(...prices.map((row) => row.value));
