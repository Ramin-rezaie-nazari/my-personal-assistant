...
export class PriceIntelligenceService {
  ...
  async getLatestPrices(productKey?: string, countryCode?: string) {
    return { items: await this.persistence.latest(productKey, countryCode) };
  }
  ...
  async getHistory(
    ...
    return {
      productKey,
      countryCode,
      items: await this.persistence.history(productKey, from, to, sourceId, countryCode),
    };
  }
  ...
  async analyze(productKey: string, countryCode?: string) {
    ...
    const prices = rows.map((row) => Number(row.amount)).filter((v) => Number.isFinite(v) && v > 0);
    if (!prices.length)
      return {
        productKey, countryCode, current: null, average7d: null, average30d: null,
        min30d: null, max30d: null, changeVs7d: null, changeVs30d: null,
        trend: 'insufficient_data', buyScore: 0, recommendation: 'unavailable',
      };
    ...
    const weekRows = rows.filter(
      (row) => new Date(row.observedAt).getTime() >= Date.now() - 7 * 24 * 60 * 60 * 1000,
    );
    ...
    return {
      productKey, countryCode, current, average7d: avg7, average30d: avg30, min30d: min30,
      max30d: max30, changeVs7d, changeVs30d, trend, buyScore, recommendation,
    };
  }
  ...
  matchProduct(reference: ProductCandidate, candidates: ProductCandidate[]) {
    ...
    return {
      accepted: matches.filter((m) => m.confidence >= 0.78 && !m.ambiguous),
      review: matches.filter((m) => m.ambiguous),
      rejected: matches.filter((m) => m.confidence < 0.55),
    };
  }
}
