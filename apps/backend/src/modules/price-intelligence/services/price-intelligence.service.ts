...
export class PriceIntelligenceService {
  ...
  async analyze(productKey: string) {
    ...
    const previous7 =
      prices.length > 7
        ...
        : null;
    const previous30 =
      prices.length > 30 ? average(prices.slice(0, -30)) : null;
    const changeVs7d =
      previous7 && previous7 > 0
        ? ((current - previous7) / previous7) * 100
        : null;
    const changeVs30d =
      previous30 && previous30 > 0
        ? ((current - previous30) / previous30) * 100
        : null;
    ...
    const buyScore = Math.round(
      ((max30 - current) / Math.max(1, max30 - min30)) * 100,
    );
  ...
  matchProduct(reference: ProductCandidate, candidates: ProductCandidate[]) {
    ...
    return {
      accepted: matches.filter(
        (match) => match.confidence >= 0.78 && !match.ambiguous,
      ),
