...
describe('PriceSourceRegistryService', () => {
  it('exposes the configured Iranian market sources', () => {
    ...
    expect(sources.every((source) => source.searchUrlTemplate.includes('{query}'))).toBe(true);
    expect(sources.every((source) => source.baseUrl.startsWith('https://'))).toBe(true);
  });

  it('allows a source search URL to be overridden without changing code', () => {
    const original = process.env.PRICE_OKALA_SEARCH_URL;
    process.env.PRICE_OKALA_SEARCH_URL = 'https://example.test/search?q={query}';

    try {
      const source = new PriceSourceRegistryService().get('okala');
      expect(source?.searchUrlTemplate).toBe('https://example.test/search?q={query}');
    } finally {
