...
export class HttpPriceSourceAdapter {
    ...
    for (const productKey of productKeys) {
      ...
      const response = await fetch(url, {
        headers: {
          ...
          'user-agent': this.config.userAgent ?? 'MyPersonalAssistant/1.0 price-intelligence',
        },
  ...
  private parse(productKey: string, html: string, sourceUrl: string): NormalizedPrice[] {
    ...
    const add = (title: string, amount: number, currency: unknown, availability: unknown, url: string) => {
    ...
    for (const block of html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
      this.walkJson(this.safeJson(this.decode(block[1])), (node) => {
        ...
        const specifications = Array.isArray(node.priceSpecification) ? node.priceSpecification : node.priceSpecification ? [node.priceSpecification] : [];
        for (const offer of [...offers, ...specifications]) {
          if (!this.isRecord(offer)) continue;
          add(String(node.name ?? productKey), this.number(offer.price ?? offer.lowPrice ?? offer.highPrice), offer.priceCurrency ?? offer.currency, offer.availability, String(offer.url ?? node.url ?? sourceUrl));
        }
        if (node['@type'] === 'Offer' || node.price !== undefined) {
          add(String(node.name ?? productKey), this.number(node.price ?? node.lowPrice), node.priceCurrency ?? node.currency, node.availability, String(node.url ?? sourceUrl));
        }
      });
    }
    ...
    for (const block of html.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)) {
      ...
      if (!text || !/["'](?:price|lowPrice|currentPrice)["']\s*:/.test(text)) continue;
      this.walkJson(this.safeJson(this.decode(text)), (node) => {
        const amount = this.number(node.price ?? node.currentPrice ?? node.lowPrice);
        if (!amount) return;
        add(String(node.name ?? node.title ?? productKey), amount, node.priceCurrency ?? node.currency ?? this.currencyNear(text, node.price), node.availability, String(node.url ?? sourceUrl));
      });
    }

    if (!prices.length) {
      const meta = /<meta[^>]+itemprop=["']price["'][^>]+content=["']([^"']+)["'][^>]*>/i.exec(html);
      if (meta) add(productKey.replace(/-/g, ' '), this.number(meta[1]), this.currencyNear(html, meta[1]), 'unknown', sourceUrl);
    }
    ...
    if (!prices.length) {
      ...
      if (toman) add(productKey.replace(/-/g, ' '), this.number(toman[1]), 'IRT', 'unknown', sourceUrl);
      else if (rial) add(productKey.replace(/-/g, ' '), this.number(rial[1]), 'IRR', 'unknown', sourceUrl);
      else if (generic) add(productKey.replace(/-/g, ' '), this.number(generic[1]), this.currencyNear(html, generic[1]), 'unknown', sourceUrl);
    }
    return prices;
  }
  ...
  private normalize(productKey: string, title: string, amount: number, currency: unknown, availability: unknown, url: string): NormalizedPrice {
    ...
    const isRial = rawCurrency === 'IRR' || rawCurrency.includes('RIAL') || rawCurrency.includes('ریال');
  ...
  private currencyNear(html: string, value: unknown) {
    ...
    const window = index >= 0 ? html.slice(Math.max(0, index - 120), index + 120) : html;
  ...
  private availability(value: unknown): NormalizedPrice['availability'] {
    ...
    if (text.includes('outofstock') || text.includes('out_of_stock') || text.includes('ناموجود')) return 'out_of_stock';
    if (text.includes('instock') || text.includes('in_stock') || text.includes('موجود')) return 'in_stock';
    return 'unknown';
  }
