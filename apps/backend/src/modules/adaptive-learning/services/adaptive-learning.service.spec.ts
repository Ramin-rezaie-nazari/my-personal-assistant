...
describe('AdaptiveLearningService', () => {
  ...
  beforeEach(() => {
    ...
    prisma.nutritionProfile.findUnique.mockResolvedValue({ dailyCaloriesGoal: 2200, proteinGoalGrams: 180, waterGoalMl: 2500 });
    prisma.dailyLog.findMany.mockResolvedValue([
      { dateKey: '2026-08-06', calories: 2300, protein: 130, waterMl: 1400 }, { dateKey: '2026-08-07', calories: 2250, protein: 135, waterMl: 1500 }, { dateKey: '2026-08-08', calories: 2400, protein: 120, waterMl: 1600 }, { dateKey: '2026-08-09', calories: 2100, protein: 150, waterMl: 1800 }, { dateKey: '2026-08-10', calories: 2200, protein: 145, waterMl: 1700 },
    ]);
    ...
    prisma.meal.findMany.mockResolvedValue([{ name: 'Chicken Bowl', calories: 620, protein: 42, eatenAt: new Date('2026-08-11T13:00:00Z') }]);
  });
  ...
  it('returns a useful cold-start insight when there is no history', async () => {
    ...
    expect(result.insights[0]).toEqual(expect.objectContaining({ key: 'start-tracking', category: 'consistency' }));
  });
});
