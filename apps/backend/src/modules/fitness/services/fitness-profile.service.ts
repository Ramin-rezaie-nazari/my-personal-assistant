...
export class FitnessProfileService {
  ...
  removeEquipment(userId: string, equipmentId: string) {
    ...
    return this.save(userId, {
      ...profile,
      equipment: profile.equipment.filter((x) => x.id !== equipmentId),
    });
  }
  ...
  parseNaturalGoal(text: string): FitnessGoal {
    ...
    const pushTarget = (value: BodyTarget, ...keywords: string[]) => {
      if (keywords.some((keyword) => normalized.includes(keyword)))
        targetAreas.push(value);
    };
    ...
    const avoidBulk =
      ...
      /(?:نمی\s*خوام|نمی\s*گیرم|نگیرم|بدون|نه|نخوام)/.test(normalized);
    const fatLoss =
    ...
    return {
      ...
      kind,
      title: text.trim(),
      targetAreas: targetAreas.length
        ? [...new Set(targetAreas)]
        : ['full_body'],
