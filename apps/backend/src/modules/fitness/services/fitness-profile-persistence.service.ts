...
import { FitnessEquipment, FitnessGoal, FitnessProfile, FitnessRecommendationContext, BodyTarget } from '../models/fitness.model';
...
export class FitnessProfilePersistenceService {
  ...
  private defaults(): FitnessProfile {
    return { disciplines: [], goals: [], equipment: [], constraints: [], preferredSessionMinutes: [20, 30, 45, 60] };
  }
  ...
  private normalize(value: unknown): FitnessProfile {
    ...
    return {
      ...
      preferredSessionMinutes: Array.isArray(p.preferredSessionMinutes) ? p.preferredSessionMinutes : [20, 30, 45, 60],
    };
  }
