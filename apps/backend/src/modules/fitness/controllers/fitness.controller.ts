...
export class FitnessController {
  ...
  @Get('profile')
  getProfile(@Req() req: AuthenticatedRequest) { return this.profile.get(req.user.id); }

  @Get('context')
  context(@Req() req: AuthenticatedRequest) { return this.profile.buildRecommendationContext(req.user.id); }

  @Post('profile')
  save(@Req() req: AuthenticatedRequest, @Body() dto: SaveFitnessProfileDto) { return this.profile.save(req.user.id, dto.profile); }

  @Post('equipment')
  addEquipment(@Req() req: AuthenticatedRequest, @Body() dto: AddFitnessEquipmentDto) { return this.profile.addEquipment(req.user.id, dto.item); }

  @Delete('equipment/:id')
  removeEquipment(@Req() req: AuthenticatedRequest, @Param('id') id: string) { return this.profile.removeEquipment(req.user.id, id); }

  @Post('goal')
  addGoal(@Req() req: AuthenticatedRequest, @Body() dto: AddFitnessGoalDto) { return this.profile.addGoal(req.user.id, dto.goal); }
  ...
  @Get('exercises')
  listExercises(@Query() query: ExerciseQueryDto) { return this.exercises.list(query); }

  @Get('exercises/:id')
  getExercise(@Param('id') id: string) { return this.exercises.get(id); }
}
