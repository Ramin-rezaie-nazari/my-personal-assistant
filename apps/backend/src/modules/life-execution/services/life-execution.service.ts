...
export class LifeExecutionService {
  ...
  async create(userId: string, dto: CreateTaskDto) {
    ...
    await this.prisma.$executeRaw`INSERT INTO "LifeTask" ("id","userId","title","description","source","goalId","priority","energy","scheduledAt","dueAt","estimatedMinutes") VALUES (${id},${userId},${dto.title.trim()},${dto.description?.trim() ?? null},${dto.source ?? 'manual'},${dto.goalId ?? null},${dto.priority ?? 2},${dto.energy ?? 'medium'},${scheduledAt},${dueAt},${dto.estimatedMinutes ?? null})`;
    await this.event(userId, id, 'created', 'task_created');
    return this.one(userId, id);
  }

  async list(userId: string, status?: string) {
    if (status) return this.prisma.$queryRaw<TaskRow[]>`SELECT * FROM "LifeTask" WHERE "userId"=${userId} AND "status"=${status} ORDER BY "priority" ASC, "dueAt" ASC NULLS LAST, "scheduledAt" ASC NULLS LAST, "createdAt" DESC`;
    return this.prisma.$queryRaw<TaskRow[]>`SELECT * FROM "LifeTask" WHERE "userId"=${userId} AND "status" NOT IN ('completed','cancelled') ORDER BY CASE WHEN "dueAt" < CURRENT_TIMESTAMP THEN 0 ELSE 1 END, "priority" ASC, "dueAt" ASC NULLS LAST, "scheduledAt" ASC NULLS LAST`;
  }

  async nextBest(userId: string) {
    const tasks = await this.prisma.$queryRaw<TaskRow[]>`SELECT t.* FROM "LifeTask" t WHERE t."userId"=${userId} AND t."status" IN ('pending','in_progress') AND NOT EXISTS (SELECT 1 FROM "TaskDependency" d JOIN "LifeTask" dep ON dep."id"=d."dependsOnTaskId" WHERE d."taskId"=t."id" AND dep."status" <> 'completed') ORDER BY t."priority" ASC, CASE WHEN t."dueAt" IS NULL THEN 1 ELSE 0 END, t."dueAt" ASC NULLS LAST, CASE t."energy" WHEN 'low' THEN 0 WHEN 'medium' THEN 1 ELSE 2 END, t."createdAt" ASC LIMIT 20`;
    const now = Date.now();
    const ranked = tasks.map((task) => {
      let score = (4 - task.priority) * 30;
      if (task.dueAt) { const hours = (task.dueAt.getTime() - now) / 3600000; score += hours < 0 ? 45 : hours < 24 ? 35 : hours < 72 ? 20 : 5; }
  ...
  async update(userId: string, id: string, dto: UpdateTaskDto) {
    const existing = await this.raw(userId, id); if (!existing) throw new NotFoundException('Task not found');
    if (dto.priority !== undefined) this.validatePriority(dto.priority);
    const status = dto.status ?? existing.status;
    const completedAt = status === 'completed' ? new Date() : status === 'in_progress' || status === 'pending' ? null : existing.completedAt;
  ...
  async recordEvent(userId: string, id: string, dto: TaskEventDto) {
    const task = await this.raw(userId, id); if (!task) throw new NotFoundException('Task not found');
    await this.event(userId, id, dto.event, dto.reason, dto.metadata);
    const status = dto.event === 'completed' ? 'completed' : dto.event === 'started' ? 'in_progress' : dto.event === 'cancelled' ? 'cancelled' : dto.event === 'snoozed' ? 'snoozed' : task.status;
    if (status !== task.status) await this.prisma.$executeRaw`UPDATE "LifeTask" SET "status"=${status},"completedAt"=${status === 'completed' ? new Date() : null},"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${id} AND "userId"=${userId}`;
    return this.one(userId, id);
  }
  ...
  private reason(task: TaskRow) { if (task.status === 'in_progress') return 'Continue the task you already started'; if (task.dueAt && task.dueAt.getTime() < Date.now()) return 'This task is overdue'; if (task.dueAt && task.dueAt.getTime() - Date.now() < 86400000) return 'This task is due within 24 hours'; if (task.priority === 1) return 'Highest priority actionable task'; return 'Best available next action based on priority, timing and momentum'; }
}
