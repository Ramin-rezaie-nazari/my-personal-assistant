...
export class HouseholdPurchasePlannerService {
  ...
  plan(items: InventoryItem[], prices: HouseholdPrice[], budgetRemaining: number): { items: HouseholdPurchasePlanItem[]; totalEstimatedCost: number; budgetRemainingAfterPlan: number } {
    ...
    for (const item of forecasts) {
      const price = prices.find((p) => p.productKey === item.productKey);
      const quantity = item.recommendedQuantity;
      if (quantity <= 0) {
        plan.push({ productKey: item.productKey, quantity: 0, price: price?.price ?? null, estimatedCost: 0, urgency: item.urgency, action: 'skip', reason: 'no_reorder_needed' });
        continue;
      }
      const estimatedCost = price?.available ? quantity * price.price : null;
      const affordable = estimatedCost !== null && estimatedCost <= remaining;
      ...
      plan.push({ productKey: item.productKey, quantity, price: price?.price ?? null, estimatedCost, urgency: item.urgency, action, reason: action === 'buy' ? 'inventory_need_and_budget_align' : action === 'watch' ? 'monitor_price_or_stock' : estimatedCost === null ? 'price_unavailable' : 'budget_constraint' });
    }
