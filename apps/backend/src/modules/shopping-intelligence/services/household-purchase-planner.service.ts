...
export class HouseholdPurchasePlannerService {
  ...
  plan(items: InventoryItem[], prices: HouseholdPrice[], budgetRemaining: number): { items: HouseholdPurchasePlanItem[]; totalEstimatedCost: number; budgetRemainingAfterPlan: number } {
    ...
    for (const item of forecasts) {
      const price = prices.find((candidate) => candidate.productKey === item.productKey);
      ...
      if (item.urgency === 'critical' && item.safetyStock !== undefined && item.safetyStock > 0) {
        purchaseQuantity = Math.min(purchaseQuantity, item.safetyStock);
      }
      if (unitPrice !== null && unitPrice > 0 && purchaseQuantity * unitPrice > remaining && item.urgency === 'critical') {
        purchaseQuantity = Math.min(purchaseQuantity, Math.floor(remaining / unitPrice));
      }
      ...
      const action = item.urgency === 'critical' && affordable
        ? 'buy'
        : item.urgency === 'soon' && affordable && (price?.buyScore ?? 0.5) >= 0.5
          ? 'buy'
          : estimatedCost === null
            ? 'watch'
            : affordable
