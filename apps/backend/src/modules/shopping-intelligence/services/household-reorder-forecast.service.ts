...
export class HouseholdReorderForecastService {
  ...
  forecast(input: ReorderForecastInput, now = new Date()): ReorderForecast {
    const model = this.consumption.forecast(input.productKey, now); const dailyRate = model.dailyRate;
    if (dailyRate <= 0) return { ...input, daysRemaining: null, reorderPoint: 0, recommendedQuantity: 0, urgency: 'unknown' };
    const leadTimeDays = Math.max(0, input.leadTimeDays ?? 2); const safetyStockDays = Math.max(0, input.safetyStockDays ?? 2); const daysRemaining = input.currentQuantity / dailyRate;
    const reorderPoint = dailyRate * (leadTimeDays + safetyStockDays); const recommendedQuantity = Math.max(0, Math.ceil(model.next30DayNeed + reorderPoint - input.currentQuantity));
