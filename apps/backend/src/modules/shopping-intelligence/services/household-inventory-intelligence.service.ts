...
export class HouseholdInventoryIntelligenceService {
  forecast(items: InventoryItem[]): InventoryForecast[] {
    return items.map((item) => {
      ...
      const recommendedQuantity = Math.max(0, Math.ceil(targetQuantity - item.quantity));
      const urgency = item.quantity <= 0
        ? 'critical'
  ...
  prioritize(items: InventoryItem[]): InventoryForecast[] {
    const rank = { critical: 0, soon: 1, normal: 2, none: 3 };
    return this.forecast(items).sort(
      (a, b) => rank[a.urgency] - rank[b.urgency] || Number(Boolean(b.essential)) - Number(Boolean(a.essential)),
    );
  }
}
