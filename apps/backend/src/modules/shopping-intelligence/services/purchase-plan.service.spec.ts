...
describe('PurchasePlanService', () => {
  ...
  it('rejects items expressed in a different currency from the plan', () => {
    const plan = service.build({
      budgetRemaining: 100,
      currency: 'IRR',
      items: [{ productKey: 'usd', quantity: 1, unitPrice: 1, currency: 'USD', urgency: 1, decision: 'buy_now', score: 1, reason: 'sale' }],
    });
    expect(plan.selected).toHaveLength(0);
    expect(plan.deferred).toHaveLength(0);
    expect(plan.skipped).toHaveLength(1);
    expect(plan.skipped[0]).toMatchObject({ productKey: 'usd', reason: 'currency_mismatch' });
    expect(plan.selectedTotal).toBe(0);
  });
});
