...
export class YogaMotionAnalysisService {
  ...
  private rangeRule(name: string, min: number, max: number, measure: (landmarks: Record<string, BodyLandmark>) => number, cue: (value: number) => string) {
    return { name, min, max, measure, error: (value: number) => value < min ? (min - value) / Math.max(Math.abs(min), 1) : value > max ? (value - max) / Math.max(Math.abs(max), 1) : 0, cue };
  }
