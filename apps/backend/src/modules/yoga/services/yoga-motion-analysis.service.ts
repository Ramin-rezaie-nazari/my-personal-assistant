...
export class YogaMotionAnalysisService {
  analyze(poseId: string, frame: PoseFrame): MotionAnalysis {
    ...
    const metrics = definition.map((rule) => {
      ...
      const error = rule.error(value);
      return { name: rule.name, value, idealMin: rule.min, idealMax: rule.max, error };
    });
    ...
    const issues = definition
      .map((rule) => {
        ...
        if (metric.error < 0.08) return null;
        return {
          key: rule.name,
  ...
  private readonly definitions: Record<string, Array<{
    ...
    mountain: [
      this.rangeRule('shoulder_level', 0.03, 0.18, (p) => Math.abs((p.leftShoulder?.y ?? 0) - (p.rightShoulder?.y ?? 0)), () => 'Keep both shoulders level.', () => 0),
    ],
    warrior_ii: [
    ...
    plank: [
      this.rangeRule('body_line', 160, 195, (p) => this.angle(p.shoulder, p.hip, p.ankle), (v) => v < 160 ? 'Lift the hips slightly.' : 'Lower the hips slightly.'),
    ],
    downward_dog: [
  ...
  private angle(a?: BodyLandmark, b?: BodyLandmark, c?: BodyLandmark): number {
    if (!a || !b || !c) return 180;
