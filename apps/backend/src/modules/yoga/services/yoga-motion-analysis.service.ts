...
import {
  ...
  PoseAssessment,
  PoseFrame,
} from '../models/pose-provider.model';
...
export type MotionAnalysis = PoseAssessment & {
...
export class YogaMotionAnalysisService {
  analyze(poseId: string, frame: PoseFrame): MotionAnalysis {
    ...
    const metrics = definition.map((rule) => {
      ...
      return {
        ...
        idealMin: rule.min,
        idealMax: rule.max,
        error: rule.error(value),
      };
    });
    const averageError = metrics.length
      ? metrics.reduce((sum, metric) => sum + metric.error, 0) / metrics.length
      : 1;
    const score =
      this.clamp(1 - averageError) * this.clamp(frame.overallConfidence);
    ...
    return {
      ...
      stable:
        metrics.length > 0 && metrics.every((metric) => metric.error < 0.15),
      metrics,
      analyzedAt: Date.now(),
    };
  }
  ...
  > = {
    mountain: [
      this.rangeRule(
        ...
        0.03,
        0.18,
        (p) => Math.abs((p.leftShoulder?.y ?? 0) - (p.rightShoulder?.y ?? 0)),
    ...
    warrior_ii: [
      this.rangeRule(
        ...
        75,
        115,
        (p) => this.angle(p.hip, p.knee, p.ankle),
        (v) =>
          v > 115
            ? 'Bend the front knee a little more.'
            : 'Straighten the front knee slightly.',
      ),
    ],
    plank: [
      this.rangeRule(
        ...
        160,
        175,
        (p) => this.angle(p.shoulder, p.hip, p.ankle),
        (v) =>
          v < 160 ? 'Lift the hips slightly.' : 'Lower the hips slightly.',
      ),
    ],
    downward_dog: [
      this.rangeRule(
        ...
        70,
        125,
        (p) => this.angle(p.shoulder, p.hip, p.knee),
        (v) =>
          v < 70
            ? 'Lift the hips and lengthen the spine.'
            : 'Soften the bend and lengthen your back.',
      ),
    ],
  };
  ...
  private rangeRule(
    ...
    min: number,
    max: number,
    measure: (p: Record<string, BodyLandmark>) => number,
    cue: (v: number) => string,
  ) {
    return {
      name,
      ...
      error: (value: number) =>
        value < min
          ? (min - value) / Math.max(Math.abs(min), 1)
          : value > max
            ? (value - max) / Math.max(Math.abs(max), 1)
            : 0,
      cue,
    };
  }
