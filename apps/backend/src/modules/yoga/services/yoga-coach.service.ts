...
export type YogaCoachPhase =
  'idle' | 'enter' | 'hold' | 'exit' | 'rest' | 'completed';
export type YogaCoachState = {
  sessionId: string;
  stepIndex: number;
  phase: YogaCoachPhase;
  remainingSec: number;
...
export class YogaCoachService {
  ...
  start(session: YogaSession): YogaCoachState {
    ...
    return {
      ...
      stepIndex: 0,
      phase: first ? 'enter' : 'completed',
      remainingSec: first?.holdSec ?? 0,
      completedSteps: [],
      currentPoseId: first?.poseId ?? null,
      nextPoseId: session.steps[1]?.poseId ?? null,
    };
  }
  ...
  tick(
    ...
    const remaining = Math.max(0, state.remainingSec - safeElapsed);
    ...
    if (phase === 'exit' && remaining <= 0) {
      ...
      const completedSteps = state.completedSteps.includes(state.stepIndex)
        ? state.completedSteps
        : [...state.completedSteps, state.stepIndex];
      const nextIndex = state.stepIndex + 1;
      if (!session.steps[nextIndex])
        return {
          ...state,
          stepIndex: nextIndex,
          phase: 'completed',
      ...
      return {
        ...state,
        stepIndex: nextIndex,
        phase: next.restSec > 0 ? 'rest' : 'enter',
        remainingSec: nextDuration,
        completedSteps,
        currentPoseId: next.poseId,
        nextPoseId: session.steps[nextIndex + 1]?.poseId ?? null,
      };
    }
  ...
  cue(
    state: YogaCoachState,
  ): { poseId: string | null; phase: YogaCoachPhase; text: string } | null {
    if (!state.currentPoseId || state.phase === 'completed') return null;
    const pose = this.library.get(state.currentPoseId);
    if (!pose) return null;
    const cue =
      pose.cues.find((item) => item.phase === state.phase) ?? pose.cues[0];
    return {
      poseId: pose.id,
      phase: state.phase,
      text: cue?.text ?? pose.breathing,
    };
  }
}
