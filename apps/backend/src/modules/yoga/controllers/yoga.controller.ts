...
import type { YogaFocus, YogaLevel, YogaSession } from '../models/yoga.model';
...
export class YogaController {
  ...
  analyzeMotion(@Body() body: { poseId: string; frame: PoseFrame }) { const result = this.motion.analyze(body.poseId, body.frame); return result.confidence < 0.55 ? { ...result, coachReady: false, reason: 'low_pose_confidence' } : { ...result, coachReady: true }; }
}
