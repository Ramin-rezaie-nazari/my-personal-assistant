...
export class GoalsService {
  ...
  async create(userId: string, dto: CreateGoalDto) {
    this.validateTitle(dto.title); const priority = dto.priority ?? 2; this.validatePriority(priority);
    const targetDate = this.parseDate(dto.targetDate); const id = randomUUID();
  ...
  async update(userId: string, id: string, dto: UpdateGoalDto) {
    const existing = await this.getRaw(userId, id); if (!existing) throw new NotFoundException('Goal not found');
    if (dto.title !== undefined) this.validateTitle(dto.title); if (dto.priority !== undefined) this.validatePriority(dto.priority);
    ...
    const progress = dto.progressPercent ?? existing.progressPercent; const status = dto.status ?? (progress >= 100 ? 'completed' : existing.status);
  ...
  async checkin(userId: string, id: string, dto: CheckinGoalDto) {
    const existing = await this.getRaw(userId, id); if (!existing) throw new NotFoundException('Goal not found'); this.validateProgress(dto.progressPercent);
    const settings = await this.prisma.userSettings.findUnique({ where: { userId }, select: { timezone: true } });
    const dateKey = dto.dateKey ?? getDateKeyInTimezone(new Date(), settings?.timezone ?? 'UTC');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) throw new BadRequestException('dateKey must use YYYY-MM-DD format');
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRaw`INSERT INTO "GoalCheckin" ("id","goalId","dateKey","progressPercent","note") VALUES (${randomUUID()},${id},${dateKey},${dto.progressPercent},${dto.note?.trim() || null}) ON CONFLICT ("goalId","dateKey") DO UPDATE SET "progressPercent"=EXCLUDED."progressPercent", "note"=EXCLUDED."note", "createdAt"=CURRENT_TIMESTAMP`;
      await tx.$executeRaw`UPDATE "Goal" SET "progressPercent"=${dto.progressPercent}, "status"=${dto.progressPercent >= 100 ? 'completed' : 'active'}, "updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${id} AND "userId"=${userId}`;
    });
    return this.findOne(userId, id);
  }
  async remove(userId: string, id: string) {
    const existing = await this.getRaw(userId, id); if (!existing) throw new NotFoundException('Goal not found');
    await this.prisma.$executeRaw`DELETE FROM "Goal" WHERE "id"=${id} AND "userId"=${userId}`; return { deleted: true };
  }
  private async getRaw(userId: string, id: string) { const rows = await this.prisma.$queryRaw<GoalRow[]>`SELECT * FROM "Goal" WHERE "id"=${id} AND "userId"=${userId} LIMIT 1`; return rows[0]; }
