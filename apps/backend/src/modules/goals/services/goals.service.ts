...
export class GoalsService {
  ...
  async create(userId: string, dto: CreateGoalDto) {
    ...
    const id = randomUUID();
    await this.prisma
      .$executeRaw`INSERT INTO "Goal" ("id","userId","title","description","category","priority","targetDate","targetValue","currentValue","unit") VALUES (${id},${userId},${dto.title.trim()},${dto.description?.trim() ?? null},${dto.category?.trim() || 'general'},${priority},${targetDate},${dto.targetValue ?? null},${dto.currentValue ?? null},${dto.unit?.trim() || null})`;
  ...
  async findAll(userId: string, status?: string) {
    ...
    if (status)
      return this.prisma.$queryRaw<
        GoalRow[]
      >`SELECT * FROM "Goal" WHERE "userId"=${userId} AND "status"=${status} ORDER BY "priority" ASC, "targetDate" ASC NULLS LAST, "createdAt" DESC`;
    return this.prisma.$queryRaw<
      GoalRow[]
    >`SELECT * FROM "Goal" WHERE "userId"=${userId} ORDER BY CASE WHEN "status"='active' THEN 0 ELSE 1 END, "priority" ASC, "targetDate" ASC NULLS LAST, "createdAt" DESC`;
  }

  async findOne(userId: string, id: string) {
    const rows = await this.prisma.$queryRaw<
      GoalRow[]
    >`SELECT * FROM "Goal" WHERE "id"=${id} AND "userId"=${userId} LIMIT 1`;
    if (!rows[0]) throw new NotFoundException('Goal not found');
    const checkins = await this.prisma
      .$queryRaw`SELECT "id","dateKey","progressPercent","note","createdAt" FROM "GoalCheckin" WHERE "goalId"=${id} ORDER BY "dateKey" DESC LIMIT 14`;
  ...
  async remove(userId: string, id: string) {
    ...
    if (!existing) throw new NotFoundException('Goal not found');
    await this.prisma
      .$executeRaw`DELETE FROM "Goal" WHERE "id"=${id} AND "userId"=${userId}`;
    return { deleted: true };
  }
