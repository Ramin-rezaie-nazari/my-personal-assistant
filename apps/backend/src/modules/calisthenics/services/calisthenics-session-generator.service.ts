...
export class CalisthenicsSessionGeneratorService {
  ...
  generate(input: {
    ...
    equipment?: Equipment[];
    progress?: Partial<CalisthenicsProgress>;
  }): CalisthenicsSession {
    const equipment: Equipment[] = input.equipment?.length
      ? input.equipment
      : ['none'];
    const level = this.adaptLevel(
      input.level ?? input.progress?.currentLevel ?? 'beginner',
      input.progress,
    );
    const pool = this.library.list(level, input.focus, equipment);
    const selected = pool.slice(
      0,
      Math.max(4, Math.min(8, Math.floor(input.durationMin / 5))),
    );
    const steps = selected.map((exercise, index) => ({
      ...
      reps: exercise.holdSec
        ? null
        : Math.min(exercise.repsMax ?? 10, Math.max(exercise.repsMin ?? 5, 8)),
    ...
    return {
      ...
      equipment,
      steps,
      estimatedDifficulty: selected.length
        ? Number(
            (
              selected.reduce((sum, item) => sum + item.difficulty, 0) /
              selected.length
            ).toFixed(2),
          )
        : 0,
  ...
  private adaptLevel(
    ...
    if (
      !progress ||
      progress.formScoreAvg == null ||
      progress.completionRate == null
    )
      return level;
    const index = levelOrder.indexOf(level);
    if (progress.formScoreAvg >= 0.9 && progress.completionRate >= 0.9)
      return levelOrder[Math.min(index + 1, levelOrder.length - 1)];
    if (progress.formScoreAvg < 0.65 || progress.completionRate < 0.65)
