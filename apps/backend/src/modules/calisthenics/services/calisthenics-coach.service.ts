...
export class CalisthenicsCoachService {
  start(session: CalisthenicsSession): CalisthenicsCoachState {
    ...
    return {
      ...
      stepIndex: 0,
      setIndex: 1,
      phase: step ? 'work' : 'completed',
      remainingSec: this.workSeconds(session, 0),
      completedSteps: [],
      currentExerciseId: step?.exerciseId ?? null,
      nextExerciseId: session.steps[1]?.exerciseId ?? null,
    };
  }
  ...
  tick(
    ...
    elapsedSec = 1,
  ): CalisthenicsCoachState {
    if (state.phase === 'completed') return state;
    const next = {
      ...state,
      remainingSec: Math.max(0, state.remainingSec - elapsedSec),
    };
    ...
    if (state.phase === 'rest') {
      return {
        ...
        remainingSec: this.workSeconds(session, state.stepIndex),
      };
    }
    ...
    return nextStep
      ? {
          ...
          setIndex: 1,
          phase: 'work',
          remainingSec: this.workSeconds(session, nextIndex),
          completedSteps: completed,
          currentExerciseId: nextStep.exerciseId,
          nextExerciseId: session.steps[nextIndex + 1]?.exerciseId ?? null,
        }
      : {
          ...next,
          phase: 'completed',
