...
describe('HabitsService', () => {
  ...
  it('creates a user-scoped daily habit', async () => {
    ...
    const result = await service.createHabit('u1', {
      name: 'Walk',
      frequency: 'daily',
    });
    ...
    expect(prisma.habit.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ userId: 'u1', targetPerWeek: 7 }),
      }),
    );
  });
  ...
  it('enforces ownership on completion and deletes', async () => {
    ...
    await expect(
      service.completeToday('u1', 'foreign', '2026-08-12'),
    ).rejects.toThrow('Habit not found');
    prisma.habit.findFirst.mockResolvedValueOnce(null);
    await expect(service.deleteHabit('u1', 'foreign')).rejects.toThrow(
      'Habit not found',
    );
  });
  ...
  it('creates an idempotent completion log', async () => {
    ...
    expect(prisma.habitLog.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { habitId_dateKey: { habitId: 'h1', dateKey: '2026-08-12' } },
      }),
    );
  });
