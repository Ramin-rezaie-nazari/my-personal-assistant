...
export class HabitsService {
  ...
  async createHabit(userId: string, dto: CreateHabitDto) {
    ...
    return this.prisma.habit.create({
      data: {
        ...
        targetPerWeek:
          dto.targetPerWeek ?? (dto.frequency === 'weekly' ? 1 : 7),
      },
    });
  }
  ...
  async getHabits(userId: string) {
    const habits = await this.prisma.habit.findMany({
      ...
      include: { logs: { orderBy: { dateKey: 'desc' }, take: 14 } },
      orderBy: { createdAt: 'asc' },
    });
    return habits.map((habit) => ({
      ...habit,
      stats: this.stats(
        habit.logs.map((log) => log.dateKey),
        habit.targetPerWeek,
      ),
    }));
  }
  ...
  async updateHabit(userId: string, id: string, dto: UpdateHabitDto) {
    ...
    if (
      ...
      this.validate(
        ...
        dto.targetPerWeek ?? habit.targetPerWeek,
      );
  ...
  async completeToday(userId: string, id: string, dateKey = this.todayKey()) {
    ...
    return this.prisma.habitLog.upsert({
      ...
      create: { habitId: id, userId, dateKey },
  ...
  async getWeeklySummary(userId: string, dateKey = this.todayKey()) {
    ...
    const completedCount = habits.reduce(
      (sum, habit) => sum + habit.logs.length,
      0,
    );
    const possible = habits.reduce(
      (sum, habit) => sum + Math.min(habit.targetPerWeek, 7),
      0,
    );
    return {
      dateKey,
      ...
      completionPercent: possible
        ? Math.min(100, Math.round((completedCount / possible) * 100))
        : 0,
      habits: habits.map((habit) => ({
        id: habit.id,
        name: habit.name,
        targetPerWeek: habit.targetPerWeek,
        completedThisWeek: habit.logs.length,
        streak: this.stats(
          habit.logs.map((log) => log.dateKey),
          habit.targetPerWeek,
        ).streak,
  ...
  private stats(keys: string[], targetPerWeek: number) {
    ...
    for (let i = 0; i < 14; i += 1) {
      ...
      if (set.has(d.toISOString().slice(0, 10))) streak += 1;
      else break;
    }
  ...
  private validate(name: string, frequency: string, targetPerWeek?: number) {
    ...
    if (!['daily', 'weekly'].includes(frequency))
      throw new BadRequestException('frequency must be daily or weekly');
    const target = targetPerWeek ?? (frequency === 'weekly' ? 1 : 7);
    if (!Number.isInteger(target) || target < 1 || target > 7)
      throw new BadRequestException('targetPerWeek must be between 1 and 7');
  }
