...
export type WorkoutWeeklySummary = {
  ...
  lastWorkout: { name: string; type: string; performedAt: string } | null;
};
...
export class WorkoutService {
  ...
  async getWorkouts(userId: string) {
    return this.prisma.workout.findMany({ where: { userId }, orderBy: { performedAt: 'desc' } });
  }
  ...
  async updateWorkout(
    ...
    const existing = await this.prisma.workout.findFirst({ where: { id: workoutId, userId } });
    if (!existing) throw new NotFoundException('Workout not found');
    if (data.durationMinutes !== undefined || data.caloriesBurned !== undefined) {
      this.validateNumbers(data.durationMinutes ?? existing.durationMinutes, data.caloriesBurned ?? existing.caloriesBurned);
    }
    return this.prisma.workout.update({
      ...
      data: {
        ...
        ...(data.durationMinutes !== undefined ? { durationMinutes: Math.round(data.durationMinutes) } : {}),
        ...(data.caloriesBurned !== undefined ? { caloriesBurned: Math.round(data.caloriesBurned) } : {}),
  ...
  async deleteWorkout(userId: string, workoutId: string) {
    const result = await this.prisma.workout.deleteMany({ where: { id: workoutId, userId } });
    if (result.count === 0) throw new NotFoundException('Workout not found');
    return { deleted: true };
  }
  ...
  async getWeeklySummary(userId: string, dateKey?: string) {
    ...
    const toDateKey = dateKey ?? getDateKeyInTimezone(new Date(), timezone);
    this.assertDateKey(toDateKey);
    const fromDateKey = this.addDays(toDateKey, -6);
    const fromDate = zonedDateTimeToUtc(fromDateKey, '00:00:00', timezone);
    const endDate = zonedDateTimeToUtc(this.addDays(toDateKey, 1), '00:00:00', timezone);

    const workouts = await this.prisma.workout.findMany({
      where: { userId, performedAt: { gte: fromDate, lt: endDate } },
    ...
    for (const workout of workouts) {
      const key = getDateKeyInTimezone(workout.performedAt, timezone);
    ...
    return {
      ...
      totalMinutes,
      totalCaloriesBurned,
      averageMinutesPerWorkout: workouts.length === 0 ? 0 : Math.round(totalMinutes / workouts.length),
      consistencyPercent: Math.round((days.size / 7) * 100),
      currentStreak,
      lastWorkout: workouts[0]
        ? { name: workouts[0].name, type: workouts[0].type, performedAt: workouts[0].performedAt.toISOString() }
        : null,
