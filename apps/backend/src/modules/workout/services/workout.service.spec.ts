...
describe('WorkoutService', () => {
  it('creates a user-scoped workout', async () => {
    ...
    await service.createWorkout('user-1', { name: 'Morning Run', type: 'cardio', durationMinutes: 31, caloriesBurned: 280 });
    expect(create).toHaveBeenCalledWith({ data: expect.objectContaining({ userId: 'user-1', name: 'Morning Run', type: 'cardio', durationMinutes: 31, caloriesBurned: 280, performedAt: expect.any(Date) }) });
  });
  ...
  it('aggregates seven days and calculates a streak', async () => {
    ...
    const userSettings = { findUnique: jest.fn().mockResolvedValue({ timezone: 'UTC' }) };
    const prisma = { workout: { findMany }, userSettings } as unknown as PrismaService;
    const service = new WorkoutService(prisma);
