...
export class RecipesService {
  ...
  async updateRecipe(userId: string, id: string, data: Partial<RecipeInput>) {
    ...
    if (data.ingredients !== undefined) {
      this.validateInput({ name: data.name ?? existing.name, ingredients: data.ingredients });
    }
    return this.prisma.$transaction(async (tx) => {
      if (data.ingredients !== undefined) {
        const foodIds = [...new Set(data.ingredients.map((item) => item.foodId))];
        ...
        if (foods.length !== foodIds.length) throw new NotFoundException('One or more food items were not found');
        const byId = new Map(foods.map((food) => [food.id, food]));
        const ingredients = data.ingredients.map((item) => { const food = byId.get(item.foodId)!; return { foodId: food.id, quantity: item.quantity, unit: item.unit?.trim() || 'g', calories: Math.round(food.calories * item.quantity), protein: food.protein * item.quantity, carbs: food.carbs * item.quantity, fat: food.fat * item.quantity }; });
        ...
        return tx.recipe.update({ where: { id }, data: { name: data.name?.trim(), description: data.description?.trim() || undefined, imageUrl: data.imageUrl?.trim() || undefined, imageSource: data.imageSource?.trim() || undefined, ...totals, ingredients: { create: ingredients } }, include: { ingredients: { include: { food: true } } } });
      }
      return tx.recipe.update({ where: { id }, data: { name: data.name?.trim(), description: data.description?.trim() || undefined, imageUrl: data.imageUrl?.trim() || undefined, imageSource: data.imageSource?.trim() || undefined }, include: { ingredients: { include: { food: true } } } });
    });
  }
