...
describe('FoodOperatingLoopService', () => {
  ...
  const service = new FoodOperatingLoopService(
    prisma as never, scaling as never, shopping as never, budget as never,
    countryFood as never, countryFinance as never, safetyTaxonomy as never,
  );
  ...
  it('builds a target-serving plan and compares inventory using scaled quantities', async () => {
    ...
    expect(result.inventory.available[0]).toEqual({ foodId: 'food-1', name: 'Chicken', quantity: 5000, unit: 'g' });
    expect(result.financeContext).toEqual({ countryCode: 'JP', currencyCode: 'JPY' });
  });

  it('builds budget impact from scaled missing ingredients and keeps incompatible prices out', async () => {
    prisma.recipe.findFirst.mockResolvedValue({ id: 'recipe-1', name: 'Example', servings: 2, userId: null, verified: true, calories: 800, protein: 60, carbs: 90, fat: 20, ingredients: [
      ...
      { foodId: 'food-2', quantity: 100, unit: 'g', food: { name: 'Rice' } },
    ] });
    ...
    scaling.scale.mockReturnValue({ targetServings: 4, scaleFactor: 2, ingredients: [
      { ingredientId: 'food-1', baseQuantity: 200, scaledQuantity: 400, unit: 'g' },
      { ingredientId: 'food-2', baseQuantity: 100, scaledQuantity: 200, unit: 'g' },
    ], nutritionForFullBatch: { calories: 1600 }, nutritionPerServing: { calories: 400, proteinGrams: 30 } });
    countryFood.getLocalRecipeGuidance.mockReturnValue(null);
    countryFinance.getFinanceContext.mockReturnValue(null);
    budget.quoteItems.mockResolvedValue({
      items: [
    ...
    expect(result.budget).toMatchObject({ totalEstimatedCost: 8, budgetRemaining: 2, status: 'within_budget' });
    expect(result.budget.items[1]).toMatchObject({ status: 'currency_mismatch', price: null });
  });
  ...
  it('adds only budget-qualified missing ingredients to shopping', async () => {
    const plan = {
      ...
      inventory: { coveragePercent: 0, available: [], missing: [{ foodId: 'food-1', name: 'Chicken', quantity: 400, unit: 'g' }, { foodId: 'food-2', name: 'Rice', quantity: 200, unit: 'g' }] },
      shopping: { readyToAdd: [], source: 'recipe' as const }, localContext: null, financeContext: null,
      budget: { budget: 10, currency: 'USD', totalEstimatedCost: 8, budgetRemaining: 2, status: 'within_budget', generatedDeterministically: true, items: [
        { foodId: 'food-1', recommendedQuantity: 400, unit: 'g', status: 'priced' },
        { foodId: 'food-2', recommendedQuantity: 200, unit: 'g', status: 'currency_mismatch' },
      ] },
    };
    jest.spyOn(service, 'buildBudgetPlan').mockResolvedValue(plan as never);
    ...
    expect(shopping.addRecipeMissing).toHaveBeenCalledWith('user-1', 'recipe-1', [{ foodId: 'food-1', quantity: 400, unit: 'g' }]);
  ...
  it('recommends recipes using inventory coverage and nutrition targets', async () => {
    ...
    prisma.inventoryItem.findMany.mockResolvedValue([{ foodId: 'food-1', quantity: 1, unit: 'kg' }]);
    prisma.nutritionProfile.findUnique.mockResolvedValue({ dailyCaloriesGoal: 2000, proteinGoalGrams: 120 });
    ...
    expect(safetyTaxonomy.evaluate).toHaveBeenCalledWith(['Chicken'], {});
  ...
  it('passes allergy and diet constraints to the safety layer', async () => {
    prisma.recipe.findMany.mockResolvedValue([{ id: 'recipe-1', name: 'Chicken Bowl', servings: 1, userId: null, verified: true, calories: 500, protein: 30, carbs: 50, fat: 10, ingredients: [{ foodId: 'food-1', quantity: 100, unit: 'g', food: { name: 'Chicken' } }] }]);
    prisma.inventoryItem.findMany.mockResolvedValue([]);
    prisma.nutritionProfile.findUnique.mockResolvedValue(null);
    scaling.scale.mockReturnValue({ targetServings: 1, scaleFactor: 1, ingredients: [{ ingredientId: 'food-1', scaledQuantity: 100, unit: 'g' }], nutritionForFullBatch: { calories: 500 }, nutritionPerServing: { calories: 500, proteinGrams: 30 } });
    await service.recommend('user-1', 1, 'IR', undefined, undefined, { allergies: ['milk'], dietaryPreferences: ['vegetarian'] });
    expect(safetyTaxonomy.evaluate).toHaveBeenCalledWith(['Chicken'], { allergies: ['milk'], dietaryPreferences: ['vegetarian'] });
  });

  it('filters out recipes rejected by the safety taxonomy', async () => {
    prisma.recipe.findMany.mockResolvedValue([{ id: 'recipe-1', name: 'Unsafe Bowl', servings: 1, userId: null, verified: true, calories: 500, protein: 30, carbs: 50, fat: 10, ingredients: [{ foodId: 'food-1', quantity: 100, unit: 'g', food: { name: 'Milk' } }] }]);
    prisma.inventoryItem.findMany.mockResolvedValue([]);
    prisma.nutritionProfile.findUnique.mockResolvedValue(null);
    scaling.scale.mockReturnValue({ targetServings: 1, scaleFactor: 1, ingredients: [{ ingredientId: 'food-1', scaledQuantity: 100, unit: 'g' }], nutritionForFullBatch: { calories: 500 }, nutritionPerServing: { calories: 500, proteinGrams: 30 } });
    safetyTaxonomy.evaluate.mockReturnValue({ allowed: false, reason: 'allergy:milk:Milk', resolutions: [] });
    const result = await service.recommend('user-1', 1, 'IR', undefined, undefined, { allergies: ['milk'] });
