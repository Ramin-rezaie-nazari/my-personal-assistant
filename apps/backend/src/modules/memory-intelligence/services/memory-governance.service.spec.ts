...
describe('MemoryGovernanceService', () => {
  ...
  const base: MemoryGoverned = { memoryId: 'm1', userId: 'u1', layer: 'derived', source: 'behavior', visibility: 'internal', confidence: 0.7, importance: 0.8, retention: '3_months', createdAt: new Date('2026-08-12T00:00:00Z'), relatedMemoryIds: [], topicKeys: ['fitness'] };
  ...
  it('keeps unlimited memories without an expiration date', () => {
    expect(service.applyRetention({ ...base, retention: 'unlimited' }).expiresAt).toBeUndefined();
  });

  it('reinforces explicit memories and uncertain or important ones', () => {
    expect(service.shouldReinforce({ ...base, source: 'explicit_user' })).toBe(true);
