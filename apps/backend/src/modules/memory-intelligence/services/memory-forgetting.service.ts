...
export class MemoryForgettingService {
  matches(request: ForgetRequest, memory: MemoryGoverned): boolean {
    ...
    if (request.kind === 'memory')
      return request.memoryIds.includes(memory.memoryId);
    if (request.kind === 'topic')
      return memory.topicKeys.includes(request.topicKey);
  ...
  forgetForUser(
    ...
    memories: MemoryGoverned[],
  ): MemoryGoverned[] {
    return memories.filter(
      (memory) => memory.userId !== userId || !this.matches(request, memory),
    );
  }
}
