...
describe('ShoppingService', () => {
  ...
  const prisma = {
    ...
    shoppingItem: { findFirst: jest.fn(), findMany: jest.fn(), create: jest.fn(), update: jest.fn(), updateMany: jest.fn(), findUnique: jest.fn() },
    inventoryItem: { findUnique: jest.fn(), update: jest.fn(), create: jest.fn() },
  ...
  it('converts compatible units before merging an existing basket item', async () => {
    ...
    prisma.shoppingItem.findFirst.mockResolvedValue({ id: 'basket-1', quantity: 1, unit: 'kg', source: 'manual', priority: 'normal' });
    ...
    expect(prisma.shoppingItem.update).toHaveBeenCalledWith({ where: { id: 'basket-1' }, data: { quantity: { increment: 0.5 }, source: 'manual', priority: 'normal' } });
  });
  ...
  it('fails closed instead of merging incompatible units', async () => {
    ...
    prisma.shoppingItem.findFirst.mockResolvedValue({ id: 'basket-1', quantity: 2, unit: 'l', source: 'manual', priority: 'normal' });
  ...
  it('converts recipe quantities into the existing basket unit', async () => {
    ...
    const tx = {
      ...
      shoppingItem: { findUnique: jest.fn().mockResolvedValue({ id: 'basket-1', quantity: 1, unit: 'kg' }), update: jest.fn().mockResolvedValue({ id: 'basket-1' }), create: jest.fn() },
    };
    ...
    expect(tx.shoppingItem.update).toHaveBeenCalledWith({ where: { id: 'basket-1' }, data: { quantity: { increment: 0.25 }, source: 'recipe', priority: 'high' } });
  });

  it('rejects any recipe shopping item that is not part of the recipe', async () => {
    prisma.recipe.findFirst.mockResolvedValue({ id: 'recipe-1', ingredients: [{ foodId: 'food-1' }] });
    await expect(service.addRecipeMissing('user-1', 'recipe-1', [{ foodId: 'food-1', quantity: 1, unit: 'kg' }, { foodId: 'food-2', quantity: 1, unit: 'kg' }])).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('fails the recipe transaction when units are incompatible', async () => {
    ...
    const tx = {
      ...
      shoppingItem: { findUnique: jest.fn().mockResolvedValue({ id: 'basket-1', quantity: 2, unit: 'piece' }), update: jest.fn(), create: jest.fn() },
    };
    prisma.$transaction.mockImplementation(async (callback: (value: typeof tx) => Promise<unknown>) => callback(tx));
  ...
  it('marks a purchased basket item complete and adds its quantity to inventory', async () => {
    const tx = {
      shoppingItem: { findFirst: jest.fn().mockResolvedValue({ id: 'basket-1', userId: 'user-1', foodId: 'food-1', quantity: 500, unit: 'g', completed: false }), updateMany: jest.fn().mockResolvedValue({ count: 1 }) },
      inventoryItem: { findUnique: jest.fn().mockResolvedValue({ id: 'inventory-1', userId: 'user-1', foodId: 'food-1', quantity: 1, unit: 'kg' }), update: jest.fn().mockResolvedValue({ id: 'inventory-1', quantity: 1.5, unit: 'kg' }), create: jest.fn() },
    };
    prisma.$transaction.mockImplementation(async (callback: (value: typeof tx) => Promise<unknown>) => callback(tx));
  ...
  it('fails closed and rolls back when purchased and inventory units are incompatible', async () => {
    const tx = {
      shoppingItem: { findFirst: jest.fn().mockResolvedValue({ id: 'basket-1', userId: 'user-1', foodId: 'food-1', quantity: 2, unit: 'piece', completed: false }), updateMany: jest.fn().mockResolvedValue({ count: 1 }) },
      inventoryItem: { findUnique: jest.fn().mockResolvedValue({ id: 'inventory-1', userId: 'user-1', foodId: 'food-1', quantity: 2, unit: 'kg' }), update: jest.fn(), create: jest.fn() },
    };
    prisma.$transaction.mockImplementation(async (callback: (value: typeof tx) => Promise<unknown>) => callback(tx));
  ...
  it('creates a new inventory item when a purchased food is not tracked yet', async () => {
    const tx = {
      shoppingItem: { findFirst: jest.fn().mockResolvedValue({ id: 'basket-1', userId: 'user-1', foodId: 'food-1', quantity: 3, unit: 'piece', completed: false }), updateMany: jest.fn().mockResolvedValue({ count: 1 }) },
      inventoryItem: { findUnique: jest.fn().mockResolvedValue(null), update: jest.fn(), create: jest.fn().mockResolvedValue({ id: 'inventory-1' }) },
    };
    prisma.$transaction.mockImplementation(async (callback: (value: typeof tx) => Promise<unknown>) => callback(tx));
    await expect(service.complete('user-1', 'basket-1')).resolves.toEqual({ count: 1, inventorySynced: true });
