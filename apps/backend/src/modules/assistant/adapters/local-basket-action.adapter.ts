...
export class LocalBasketActionAdapter implements OnModuleInit {
  ...
  onModuleInit() {
    this.adapters.register({
      supports: (candidate) => candidate.action === 'add_to_basket' || candidate.action === 'remove_from_basket',
      execute: (candidate, context) => this.execute(candidate, context),
    });
  }

  private async execute(candidate: DecisionCandidate, context: Record<string, unknown>) {
