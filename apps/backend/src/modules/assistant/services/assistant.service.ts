...
export class AssistantService {
  ...
  private responseForLocalIntent(
    ...
    const map: Record<string, { intent: string; nextAction: string; message: string }> = {
      ...
      RECOMMEND_WORKOUT: { intent: 'workout', nextAction: 'recommend_workout', message: 'حتماً، تمرین‌هایی متناسب با هدفت و عضله‌ای که گفتی پیدا می‌کنم.' },
    ...
    return selected ? { ...selected, confidence: local.confidence, metadata: { local: true, entities: local.entities } } : undefined;
  }
