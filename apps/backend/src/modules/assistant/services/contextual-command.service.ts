...
export class ContextualCommandService {
  ...
  private normalize(input: string): string {
    return input.trim().toLowerCase().replace(/[۰-۹]/g, (digit) => String('۰۱۲۳۴۵۶۷۸۹'.indexOf(digit))).replace(/ي/g, 'ی').replace(/ك/g, 'ک').replace(/‌/g, ' ').replace(/[؟?!،؛]/g, ' ').replace(/\s+/g, ' ');
  }
  ...
  private splitClauses(text: string): string[] {
    return text.split(/\s+(?:و|ولی|اما|بعد|سپس|then|and|but|y luego|ensuite|dann|poi|depois|然后|之后|それから|그리고)\s+/i).map((part) => part.trim()).filter(Boolean);
  }
