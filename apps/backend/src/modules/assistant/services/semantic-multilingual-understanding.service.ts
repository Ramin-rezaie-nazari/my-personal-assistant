...
const PARAPHRASES: Partial<Record<SupportedLocalLanguage, Partial<Record<IntentCandidate['intent'], readonly string[]>>>> = {
  ...
  'zh-CN': {
    RECOMMEND_MEAL: ['我可以吃什么', '今晚吃什么', '晚饭吃什么好', '给我一些晚饭建议', '今天吃什么比较好', '有什么吃饭建议'],
    CREATE_REMINDER: ['稍后提醒我', '别让我忘记', '之后提醒我这个', '给我设个提醒', '晚点提醒我'],
    ADD_TO_BASKET: ['把鸡肉加进去', '把鸡肉放进购物车', '把这个放进购物车', '把这个加入购物清单'],
    GET_NUTRITION_SUMMARY: ['我今天的卡路里怎么样', '我今天吃得怎么样', '今天吃了多少卡路里', '今天吃了多少蛋白质'],
    CANCEL_REQUEST: ['忘了这个', '取消这个请求', '算了', '取消它', '不用了'],
  },
  'ar-SA': {
    RECOMMEND_MEAL: ['ماذا يمكنني أن آكل', 'ماذا آكل الليلة', 'ماذا أطبخ للعشاء', 'أفكار للعشاء', 'ماذا تقترح للعشاء', 'ماذا يمكن أن آكل اليوم'],
    CREATE_REMINDER: ['ذكرني لاحقاً', 'لا تدعني أنسى', 'ذكرني بهذا لاحقاً', 'ضع لي تذكيراً', 'ذكرني بعد قليل'],
    ADD_TO_BASKET: ['أضف الدجاج', 'ضع الدجاج في السلة', 'أضف هذا إلى السلة', 'أضف هذا إلى قائمة التسوق'],
    GET_NUTRITION_SUMMARY: ['كيف هي سعراتي اليوم', 'كيف كان غذائي اليوم', 'كم سعرة تناولت اليوم', 'كم بروتين تناولت اليوم'],
    CANCEL_REQUEST: ['انس هذا', 'ألغ الطلب', 'لا بأس اتركه', 'ألغ هذا', 'لا أريد ذلك'],
  },
};

const FILLERS: Partial<Record<SupportedLocalLanguage, readonly string[]>> = {
  'fa-IR': ['لطفا', 'فقط', 'یعنی'],
  'en-US': ['please', 'just', 'well'],
  'en-GB': ['please', 'just', 'well'],
  'es-ES': ['por favor', 'solo'],
  'es-MX': ['por favor', 'solo'],
  'fr-FR': ['s’il te plaît', 'juste'],
  'de-DE': ['bitte', 'nur'],
  'it-IT': ['per favore', 'solo'],
  'pt-BR': ['por favor', 'só'],
