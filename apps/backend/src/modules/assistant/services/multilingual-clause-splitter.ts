...
/**
 * Split a natural multi-intent utterance into executable clauses without
 * depending on ASCII whitespace or Latin-only punctuation.
 *
 * The splitter is deliberately deterministic and conservative:
 * - sentence punctuation is a hard boundary;
 * - language-family conjunctions are recognized with or without surrounding whitespace;
 * - CJK/Japanese clause markers work when the language omits spaces;
 * - returned clauses are normalized only after boundaries are identified.
 */
