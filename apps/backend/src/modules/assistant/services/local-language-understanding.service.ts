...
export class LocalLanguageUnderstandingService {
  understand(input: string): LocalUnderstanding {
    ...
    if (this.matches(normalizedText, ['لغو', 'کنسل', 'باطل', 'حذفش کن', 'بیخیال', 'cancel'])) return this.result('CANCEL_REQUEST', entities, referencesPrevious ? 0.98 : 0.91, normalizedText);
    if (referencesPrevious && this.matches(normalizedText, ['تغییر', 'عوض', 'کن', 'بکن', 'ویرایش', 'به جاش', 'بجاش', 'update', 'change'])) return this.result('UPDATE_REQUEST', entities, 0.96, normalizedText);
    if (food && this.matches(normalizedText, ['اضافه', 'بذار', 'بگذار', 'بخر', 'خرید', 'سبد', 'اضافه کن', 'add', 'basket'])) return this.result('ADD_TO_BASKET', entities, 0.97, normalizedText);
    if (food && this.matches(normalizedText, ['حذف', 'بردار', 'پاک', 'remove', 'delete'])) return this.result('REMOVE_FROM_BASKET', entities, 0.97, normalizedText);
    if (this.matches(normalizedText, ['یادم بنداز', 'یادآوری', 'یادآور', 'یادم نره', 'یادآوری کن', 'remind', 'reminder'])) return this.result('CREATE_REMINDER', entities, time ? 0.97 : 0.90, normalizedText);
    if (this.matches(normalizedText, ['چی بخور', 'چه بخور', 'شام', 'ناهار', 'صبحانه', 'غذا پیشنهاد', 'پیشنهاد غذا', 'غذا چی', 'meal', 'dinner', 'lunch'])) return this.result('RECOMMEND_MEAL', entities, 0.94, normalizedText);
    if (this.matches(normalizedText, ['کالری', 'پروتئین', 'تغذیه امروز', 'درشت مغذی', 'مواد مغذی', 'nutrition', 'calories', 'protein'])) return this.result('GET_NUTRITION_SUMMARY', entities, 0.95, normalizedText);
