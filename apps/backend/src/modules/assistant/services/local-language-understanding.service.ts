...
export class LocalLanguageUnderstandingService {
  ...
  private extractBudget(text: string):
    | { amount: number; currency: 'IRR' | 'IRT' | 'USD' | 'EUR' | 'GBP' }
    | undefined {
    ...
    const scale = match[2]?.toLowerCase() === 'میلیون' || match[2]?.toLowerCase() === 'm'
      ? 1_000_000
      : match[2]?.toLowerCase() === 'هزار' || match[2]?.toLowerCase() === 'k'
        ? 1_000
        : 1;
    const rawCurrency = (match[3] ?? 'IRT').toLowerCase();
    const currency = rawCurrency.includes('ریال') || rawCurrency === 'irr'
      ? 'IRR'
      : rawCurrency.includes('دلار') || rawCurrency === 'usd'
        ? 'USD'
        : rawCurrency.includes('یورو') || rawCurrency === 'eur'
          ? 'EUR'
          : rawCurrency.includes('پوند') || rawCurrency === 'gbp'
            ? 'GBP'
            : 'IRT';
  ...
  private findDietaryPreferences(text: string): string[] {
    const preferences: Array<[string, string]> = [
      ['وگان', 'vegan'], ['vegan', 'vegan'], ['گیاهخوار', 'vegetarian'], ['vegetarian', 'vegetarian'],
      ['بدون گلوتن', 'gluten_free'], ['gluten free', 'gluten_free'], ['کم کربوهیدرات', 'low_carb'],
      ['low carb', 'low_carb'], ['پر پروتئین', 'high_protein'], ['high protein', 'high_protein'],
    ];
    return [...new Set(preferences.filter(([phrase]) => text.includes(phrase)).map(([, value]) => value))];
  }
