...
export class LocalLanguageUnderstandingService {
  understand(input: string): LocalUnderstanding {
    ...
    if (this.isWaterLogRequest(normalizedText))
      return this.result('ADD_WATER', entities, waterAmountMl !== undefined ? 0.97 : 0.92, normalizedText);
  ...
  private extractWaterAmountMl(text: string): number | undefined {
    const explicit = text.match(/\b(\d+(?:\.\d+)?)\s*(?:ml|میلی\s*لیتر|میلیلیتر)\b/i);
    if (explicit) return Math.round(Number(explicit[1]));
    const glass = this.extractQuantity(text);
    if (this.matches(text, ['لیوان', 'glass']) && glass !== undefined) return Math.round(glass * 250);
    return undefined;
  }

  private isWaterLogRequest(text: string): boolean {
    const water = this.matches(text, ['آب', 'water']);
    const logged = this.matches(text, ['خوردم', 'نوشیدم', 'نوشید', 'خورده شد', 'ثبت کن', 'ثبتش کن', 'ثبت شد', 'نوشیدم', 'drank', 'logged', 'log']);
    return water && logged;
  }
  ...
  private findAllFoods(text: string): Record<string, string> { return { 'سینه مرغ': 'chicken', 'ماست کم چرب': 'yogurt', 'تخم مرغ': 'eggs', شیر: 'milk', مرغ: 'chicken', برنج: 'rice', ماست: 'yogurt', نان: 'bread', موز: 'banana', سیب: 'apple', پنیر: 'cheese' }; }
