...
import { compressRecipeImage, RECIPE_IMAGE_MAX_BYTES } from './recipe-image-pipeline.js';
...
describe('recipe image pipeline', () => {
  ...
  it('rejects empty input instead of producing a bogus image', async () => {
    await expect(compressRecipeImage(Buffer.alloc(0))).rejects.toThrow('non-empty Buffer');
  });
});
