...
const copy = localizedCopy({
  en: {
    ...
    loadError: 'Unable to load price history.', sourceAttribution: 'Price data: Open Prices / Open Food Facts · ODbL',
  },
  fa: {
    ...
    loadError: 'دریافت تاریخچه قیمت انجام نشد.', sourceAttribution: 'داده قیمت: Open Prices / Open Food Facts · مجوز ODbL',
  },
});
...
export default function PriceHistoryScreen() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={[styles.content, rtl && styles.rtl]}>
    ...
    <Pressable accessibilityRole="link" onPress={() => void Linking.openURL('https://openfoodfacts.github.io/open-prices/')}><Text style={[styles.attribution, rtl && styles.textRight]}>{text.sourceAttribution}</Text></Pressable>
  </ScrollView></SafeAreaView>;
}

const styles = StyleSheet.create({
  safe:{flex:1,backgroundColor:'#F7F8FA'}, center:{flex:1,alignItems:'center',justifyContent:'center',backgroundColor:'#F7F8FA'}, content:{padding:20,gap:14,paddingBottom:40}, rtl:{direction:'rtl'}, textRight:{textAlign:'right'}, back:{fontWeight:'900',color:'#111827'}, eyebrow:{fontSize:10,letterSpacing:1.5,fontWeight:'900',color:'#6B7280'}, title:{fontSize:28,fontWeight:'900',color:'#111827'}, periods:{flexDirection:'row',gap:7}, period:{paddingHorizontal:12,paddingVertical:8,borderRadius:18,backgroundColor:'#FFF'}, periodActive:{backgroundColor:'#111827'}, periodText:{fontSize:11,fontWeight:'800',color:'#6B7280'}, periodTextActive:{color:'#FFF'}, hero:{backgroundColor:'#111827',borderRadius:22,padding:20}, label:{fontSize:11,color:'#9CA3AF'}, current:{fontSize:28,fontWeight:'900',color:'#FFF',marginTop:5}, change:{fontSize:12,fontWeight:'800',color:'#D1D5DB',marginTop:6}, card:{backgroundColor:'#FFF',borderRadius:20,padding:17}, section:{fontSize:16,fontWeight:'900',color:'#111827',marginBottom:10}, chart:{height:170,borderLeftWidth:1,borderBottomWidth:1,borderColor:'#E5E7EB',position:'relative',overflow:'hidden'}, point:{position:'absolute',width:7,height:7,marginLeft:-3.5,marginTop:-3.5}, dot:{width:7,height:7,borderRadius:4,backgroundColor:'#111827'}, chartLegend:{flexDirection:'row',justifyContent:'space-between',marginTop:8}, muted:{fontSize:11,color:'#9CA3AF'}, grid:{flexDirection:'row',flexWrap:'wrap',gap:10}, stat:{backgroundColor:'#FFF',borderRadius:16,padding:14,width:'48%'}, statValue:{fontSize:15,fontWeight:'900',color:'#111827',marginTop:5}, row:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingVertical:12,borderBottomWidth:1,borderBottomColor:'#F3F4F6'}, name:{fontSize:13,fontWeight:'900',color:'#111827'}, price:{fontSize:13,fontWeight:'900',color:'#111827'}, attribution:{fontSize:10,lineHeight:16,color:'#6B7280'}, error:{color:'#B91C1C',fontWeight:'800'},
});
