          ...
          <View style={styles.authCard}>
            {mode === 'register' ? (
              <View style={styles.row}>
                <TextInput value={firstName} onChangeText={setFirstName} placeholder="First name" placeholderTextColor="#9CA3AF" style={[styles.input, styles.halfInput]} autoCapitalize="words" />
                <TextInput value={lastName} onChangeText={setLastName} placeholder="Last name" placeholderTextColor="#9CA3AF" style={[styles.input, styles.halfInput]} autoCapitalize="words" />
              </View>
            ) : null}
            ...
            <Pressable style={styles.switchButton} onPress={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(null); }}>
              <Text style={styles.switchText}>{mode === 'login' ? 'Need an account? Create one' : 'Already have an account? Sign in'}</Text>
            </Pressable>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.headerCopy}>
            ...
            <Text style={styles.subtitle}>{today?.dateKey ?? 'Today'} · your personal command center</Text>
          </View>
        ...
        <View style={styles.hero}>
          ...
          <Text style={styles.heroTitle}>{today?.nutrition.caloriesRemaining.toLocaleString() ?? 0} kcal</Text>
          <Text style={styles.heroMuted}>remaining within your daily target</Text>
          {primaryGoal ? <Text style={styles.goalText}>🎯 {primaryGoal}</Text> : <Text style={styles.goalText}>🎯 Set a primary goal to make your assistant more personal</Text>}
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.inlineStats}>
            <View><Text style={styles.cardLabel}>Avg calories</Text><Text style={styles.inlineValue}>{weekly?.averageCalories.toLocaleString() ?? 0}</Text></View>
            <View><Text style={styles.cardLabel}>Avg protein</Text><Text style={styles.inlineValue}>{weekly?.averageProtein ?? 0} g</Text></View>
          </View>
          <View style={styles.divider} />
          <Text style={styles.muted}>{(weekly?.totalWaterMl ?? 0).toLocaleString()} ml water logged · {(weekly?.totalProtein ?? 0).toLocaleString()} g protein · {(weekly?.totalCalories ?? 0).toLocaleString()} kcal total</Text>
        </View>
        ...
        <View style={styles.card}>
          {workouts?.latest ? (
            <>
              ...
              <Text style={styles.workoutName}>{workouts.latest.name}</Text>
              <Text style={styles.muted}>{workouts.latest.type} · {workouts.latest.durationMinutes} min · {workouts.latest.caloriesBurned} kcal</Text>
            </>
          ) : (
            <>
              <Text style={styles.cardLabel}>Ready when you are</Text>
              <Text style={styles.workoutName}>No workout logged this week yet</Text>
              <Text style={styles.muted}>Start tracking your training and your assistant will build the history automatically.</Text>
            </>
          )}
          <View style={styles.trainingRow}>
            <Text style={styles.trainingMetric}>{workouts?.activeDays ?? 0} active days</Text>
            <Text style={styles.trainingMetric}>{workouts?.totalMinutes ?? 0} min</Text>
            <Text style={styles.trainingMetric}>{workouts?.totalCaloriesBurned ?? 0} kcal</Text>
          </View>
        </View>
        ...
        <View style={styles.card}>
          {today?.meals.length ? today.meals.map((meal) => (
            <View key={meal.id} style={styles.mealRow}>
              <View style={styles.mealCopy}><Text style={styles.mealName}>{meal.name}</Text><Text style={styles.muted}>{meal.type} · {Math.round(meal.protein)} g protein</Text></View>
              <Text style={styles.mealCalories}>{meal.calories} kcal</Text>
            </View>
          )) : <Text style={styles.muted}>No meals logged today yet.</Text>}
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  hero: { backgroundColor: '#111827', borderRadius: 26, padding: 22, marginVertical: 4 },
  heroEyebrow: { color: '#9CA3AF', fontSize: 11, fontWeight: '800', letterSpacing: 1.4 },
  heroTitle: { color: '#FFFFFF', fontSize: 38, fontWeight: '800', marginTop: 7 },
  heroMuted: { color: '#D1D5DB', marginTop: 2 },
  goalText: { color: '#FFFFFF', fontWeight: '600', marginTop: 18, lineHeight: 20 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  statCard: { width: '48%', backgroundColor: '#FFFFFF', borderRadius: 18, padding: 16 },
  statLabel: { color: '#6B7280', fontSize: 11, fontWeight: '700' },
  statValue: { color: '#111827', fontSize: 23, fontWeight: '800', marginTop: 6 },
  statHelper: { color: '#6B7280', fontSize: 11, marginTop: 4 },
  ...
  sectionHint: { color: '#9CA3AF', fontSize: 10, fontWeight: '700' },
  card: { backgroundColor: '#FFFFFF', borderRadius: 20, padding: 18 },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  cardLabel: { color: '#6B7280', fontSize: 13, fontWeight: '700' },
  cardPercent: { color: '#111827', fontSize: 12, fontWeight: '800' },
  cardValue: { color: '#111827', fontSize: 27, fontWeight: '800', marginTop: 4 },
  ...
  muted: { color: '#6B7280', fontSize: 12, marginTop: 7, lineHeight: 18 },
  inlineStats: { flexDirection: 'row', justifyContent: 'space-between' },
  inlineValue: { color: '#111827', fontSize: 23, fontWeight: '800', marginTop: 4 },
  divider: { height: StyleSheet.hairlineWidth, backgroundColor: '#E5E7EB', marginVertical: 15 },
  workoutName: { color: '#111827', fontSize: 22, fontWeight: '800', marginTop: 5 },
  trainingRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 17, paddingTop: 14, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#E5E7EB' },
  trainingMetric: { color: '#111827', fontSize: 12, fontWeight: '800' },
  mealRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 11, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#E5E7EB' },
  mealCopy: { flex: 1, paddingRight: 12 },
  mealName: { color: '#111827', fontWeight: '700' },
  mealCalories: { color: '#111827', fontWeight: '800' },
