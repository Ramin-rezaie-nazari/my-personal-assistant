...
import { getDailyCommandCenter, DailyCommandCenterResponse, getDecisionTrace, getPlanHistory, getNutritionSummary, NutritionSummary, hasAuthSession, PlanExecutionState, DecisionTrace } from '../lib/api';
...
export default function CommandCenterScreen() {
  ...
  const load = useCallback(async () => {
    try { setError(null); const [daily, plans, traces, nutritionSummary] = await Promise.all([getDailyCommandCenter(), getPlanHistory(1), getDecisionTrace(), getNutritionSummary()]); setData(daily); setPlan(plans[0] ?? null); setTrace(traces[0] ?? null); setNutrition(nutritionSummary); }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }} />}>
        ...
        <View style={[styles.card, rtl && styles.rtl]}>
          <View style={styles.rowBetween}><Text style={styles.cardTitle}>{ui.nutritionDetail}</Text><Pressable onPress={() => router.push('/daily')}><Text style={styles.openLink}>Open →</Text></Pressable></View>
          <View style={styles.nutritionGrid}>
            <NutritionMetric label={ui.calories} value={`${Math.round(nutrition?.meals.calories ?? data?.nutrition.calories ?? 0)}`} goal={calorieGoal} unit="kcal" />
            <NutritionMetric label={ui.protein} value={`${Math.round(nutrition?.meals.protein ?? data?.nutrition.protein ?? 0)}`} goal={proteinGoal} unit="g" />
            <NutritionMetric label={ui.waterDetail} value={`${Math.round((waterGoal ?? 0) - (nutrition?.remaining.waterMl ?? 0))}`} goal={waterGoal} unit="ml" />
          </View>
          <Text style={styles.muted}>{nutrition?.status.calories ?? ui.none} · {nutrition?.status.protein ?? ''}</Text>
        </View>
        <View style={[styles.card, rtl && styles.rtl]}>
