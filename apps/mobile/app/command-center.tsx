...
import { getDailyCommandCenter, DailyCommandCenterResponse, getDecisionTrace, getPlanHistory, hasAuthSession, PlanExecutionState, DecisionTrace } from '../lib/api';
...
export default function CommandCenterScreen() {
  ...
  const load = useCallback(async () => {
    try { setError(null); const [daily, plans, traces] = await Promise.all([getDailyCommandCenter(), getPlanHistory(1), getDecisionTrace()]); setData(daily); setPlan(plans[0] ?? null); setTrace(traces[0] ?? null); }
