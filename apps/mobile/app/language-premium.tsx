...
export default function LanguagePremiumScreen() {
  ...
  const continueToApp = async () => { setBusy(true); await setStoredLocale(locale); I18nManager.allowRTL(isRTL(locale)); router.replace('/auth'); };
  ...
  return <View style={[styles.screen, rtl && styles.rtl]}>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <AnimatedIn delay={130}><View style={styles.panel}><Text style={styles.panelTitle}>{locale === 'fa' || locale.startsWith('fa-') ? 'زبانت را انتخاب کن' : 'Choose your language'}</Text><Text style={styles.panelHint}>{locale === 'fa' || locale.startsWith('fa-') ? 'این انتخاب روی گفتگو و صدای دستیار اثر می‌گذارد؛ اطلاعاتت مستقل می‌ماند.' : 'Your conversation and voice adapt to this choice. Your data stays independent.'}</Text>
