...
export default function OnboardingScreen() {
  ...
  const selectGender = (gender: OnboardingState['gender']) => {
    const nextState = { ...state, gender };
    setState(nextState);
    setBrandTheme(gender === 'female' ? 'female' : 'default');
    void setOnboardingState(nextState);
  };

  const profileComplete = state.fullName.trim().length >= 2 && !!state.gender && !!state.birthDate && Number(state.heightCm) >= 90 && Number(state.heightCm) <= 250 && Number(state.weightKg) >= 25 && Number(state.weightKg) <= 350;
  const stepComplete = screen !== 2 || profileComplete;
        ...
        <View style={styles.shell}>
          ...
          <Animated.View style={[styles.content, { opacity: fade, transform: [{ translateY: slide }] }]}>
            <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              {screen === 2 && <ProfileCard rtl={rtl} state={state} bmi={bmi} onUpdate={update} onSelectGender={selectGender} />}
...
const styles = StyleSheet.create({
  safe:{flex:1,backgroundColor:BRAND.colors.canvas},flex:{flex:1},shell:{flex:1,paddingHorizontal:22},glowTop:{position:'absolute',width:260,height:260,borderRadius:130,backgroundColor:BRAND.colors.peach,top:-105,right:-95,opacity:.68},glowBottom:{position:'absolute',width:190,height:190,borderRadius:95,backgroundColor:BRAND.colors.mint,bottom:75,left:-115,opacity:.58},header:{paddingTop:10,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},progressWrap:{width:120,alignItems:'flex-end'},progressText:{color:BRAND.colors.muted,fontSize:10,fontWeight:'900',marginBottom:5},progressRail:{width:120,height:5,borderRadius:5,backgroundColor:BRAND.colors.border,overflow:'hidden'},progressFill:{height:5,borderRadius:5,backgroundColor:BRAND.colors.primary},backButton:{flexDirection:'row',alignItems:'center',gap:5,marginTop:11,alignSelf:'flex-start',paddingVertical:4},backText:{color:BRAND.colors.inkSoft,fontSize:13,fontWeight:'800'},content:{flex:1},scrollContent:{paddingTop:13,paddingBottom:16},eyebrow:{color:BRAND.colors.primary,fontSize:10,fontWeight:'900',letterSpacing:1.4,marginBottom:7},title:{maxWidth:390,color:BRAND.colors.ink,fontSize:31,lineHeight:38,fontWeight:'900',letterSpacing:-.5},subtitle:{maxWidth:410,color:BRAND.colors.muted,fontSize:14,lineHeight:21,marginTop:8,marginBottom:17},rtl:{textAlign:'right'},welcomeCard:{minHeight:410,backgroundColor:BRAND.colors.startup,borderRadius:30,padding:22,overflow:'hidden',justifyContent:'space-between'},welcomeHalo:{position:'absolute',width:255,height:255,borderRadius:128,borderWidth:1,borderColor:BRAND.colors.violet,right:-62,top:-65,opacity:.5},welcomeHaloSmall:{position:'absolute',width:180,height:180,borderRadius:90,borderWidth:1,borderColor:BRAND.colors.primary,right:-25,top:-25,opacity:.35},sparkleBox:{width:82,height:82,borderRadius:28,backgroundColor:BRAND.colors.primaryStrong,alignItems:'center',justifyContent:'center',shadowColor:BRAND.colors.primaryStrong,shadowOpacity:.34,shadowRadius:20,shadowOffset:{width:0,height:10},elevation:7},welcomeCopy:{maxWidth:330,marginTop:20},badge:{alignSelf:'flex-start',paddingHorizontal:10,paddingVertical:6,borderRadius:999,backgroundColor:BRAND.colors.startupSurface,marginBottom:11},badgeText:{color:BRAND.colors.startupMuted,fontSize:9,fontWeight:'900',letterSpacing:1.1},welcomeTitle:{color:BRAND.colors.white,fontSize:27,lineHeight:33,fontWeight:'900'},welcomeBody:{color:BRAND.colors.startupMuted,fontSize:14,lineHeight:21,marginTop:10},featureRow:{flexDirection:'row',gap:8,flexWrap:'wrap'},featurePill:{flexDirection:'row',alignItems:'center',gap:6,paddingHorizontal:11,paddingVertical:9,borderRadius:14,backgroundColor:BRAND.colors.startupSurface},featureText:{color:BRAND.colors.startupMuted,fontSize:11,fontWeight:'800'},card:{backgroundColor:BRAND.colors.surface,borderRadius:22,borderWidth:1,borderColor:BRAND.colors.border,padding:16,shadowColor:BRAND.colors.ink,shadowOpacity:.05,shadowRadius:18,shadowOffset:{width:0,height:8},elevation:2},permissionRow:{minHeight:76,borderRadius:17,borderWidth:1,borderColor:BRAND.colors.border,paddingHorizontal:12,marginBottom:9,flexDirection:'row',alignItems:'center',gap:10},permissionGranted:{borderColor:BRAND.colors.flower,backgroundColor:BRAND.colors.primarySoft},permissionIcon:{width:43,height:43,borderRadius:14,backgroundColor:BRAND.colors.primarySoft,alignItems:'center',justifyContent:'center'},permissionTitle:{color:BRAND.colors.ink,fontSize:14,fontWeight:'900'},permissionHint:{color:BRAND.colors.muted,fontSize:10,lineHeight:15,marginTop:2},permissionDot:{width:22,height:22,borderRadius:11,borderWidth:1.5,borderColor:BRAND.colors.border,alignItems:'center',justifyContent:'center'},permissionDotGranted:{borderColor:BRAND.colors.primary,backgroundColor:BRAND.colors.primary},cardTop:{flexDirection:'row',alignItems:'center',gap:11,marginBottom:17},iconCircle:{width:42,height:42,borderRadius:15,backgroundColor:BRAND.colors.primarySoft,alignItems:'center',justifyContent:'center'},cardTitle:{color:BRAND.colors.ink,fontSize:16,fontWeight:'900'},cardHint:{color:BRAND.colors.muted,fontSize:11,marginTop:2},inputBlock:{marginBottom:13},fieldLabel:{color:BRAND.colors.inkSoft,fontSize:11,fontWeight:'900',marginBottom:7},input:{minHeight:50,borderRadius:14,borderWidth:1,borderColor:BRAND.colors.border,backgroundColor:BRAND.colors.canvas,paddingHorizontal:14,color:BRAND.colors.ink,fontSize:14,fontWeight:'700'},genderGrid:{gap:8,marginBottom:13},genderChip:{minHeight:44,borderRadius:14,borderWidth:1,borderColor:BRAND.colors.border,paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:8},genderSelected:{borderColor:BRAND.colors.flower,backgroundColor:BRAND.colors.primarySoft},genderText:{color:BRAND.colors.inkSoft,fontSize:12,fontWeight:'800',flex:1},genderTextSelected:{color:BRAND.colors.primary},twoColumns:{flexDirection:'row',gap:10},bmi:{marginTop:2,padding:12,borderRadius:16,backgroundColor:BRAND.colors.primarySoft,flexDirection:'row',alignItems:'center',gap:10},bmiLabel:{color:BRAND.colors.muted,fontSize:10,fontWeight:'700'},bmiValue:{color:BRAND.colors.ink,fontSize:20,fontWeight:'900',marginTop:1},bmiTag:{color:BRAND.colors.primary,fontSize:10,fontWeight:'900'},choice:{minHeight:78,borderRadius:18,borderWidth:1,borderColor:BRAND.colors.border,paddingHorizontal:12,marginBottom:9,flexDirection:'row',alignItems:'center',gap:11,backgroundColor:BRAND.colors.surface},choiceSelected:{backgroundColor:BRAND.colors.primarySoft,borderColor:BRAND.colors.flower},choicePressed:{transform:[{scale:.99}]},choiceIcon:{width:44,height:44,borderRadius:14,backgroundColor:BRAND.colors.primarySoft,alignItems:'center',justifyContent:'center'},choiceIconSelected:{backgroundColor:BRAND.colors.primary},choiceTitle:{color:BRAND.colors.ink,fontSize:14,fontWeight:'900'},choiceHint:{color:BRAND.colors.muted,fontSize:10,lineHeight:16,marginTop:3},radio:{width:21,height:21,borderRadius:11,borderWidth:1.5,borderColor:BRAND.colors.border,alignItems:'center',justifyContent:'center'},radioSelected:{borderColor:BRAND.colors.primary},radioDot:{width:9,height:9,borderRadius:5,backgroundColor:BRAND.colors.primary},topField:{marginTop:18},numberRow:{flexDirection:'row',gap:7},dayChoice:{flex:1,minHeight:64,borderRadius:14,borderWidth:1,borderColor:BRAND.colors.border,alignItems:'center',justifyContent:'center'},dayNumber:{color:BRAND.colors.ink,fontSize:18,fontWeight:'900'},dayLabel:{color:BRAND.colors.muted,fontSize:9,fontWeight:'800',marginTop:2},timeChoice:{flex:1,minHeight:56,borderRadius:14,borderWidth:1,borderColor:BRAND.colors.border,alignItems:'center',justifyContent:'center',flexDirection:'row',gap:3},timeNumber:{color:BRAND.colors.ink,fontSize:16,fontWeight:'900'},selectedText:{color:BRAND.colors.primary},setupRow:{flexDirection:'row',gap:8},setupChoice:{flex:1,minHeight:51,borderRadius:14,borderWidth:1,borderColor:BRAND.colors.border,alignItems:'center',justifyContent:'center',gap:5},setupText:{color:BRAND.colors.inkSoft,fontSize:10,fontWeight:'800'},detected:{marginTop:16,padding:11,borderRadius:15,backgroundColor:BRAND.colors.mint,flexDirection:'row',alignItems:'center',gap:8},detectedText:{color:BRAND.colors.inkSoft,fontSize:10,fontWeight:'700',lineHeight:16,flex:1},validation:{color:BRAND.colors.primary,fontSize:11,lineHeight:17,marginTop:9},bottomBar:{paddingTop:7,paddingBottom:Platform.OS==='ios'?6:10},privacyNote:{color:BRAND.colors.muted,fontSize:10,lineHeight:16,marginBottom:9},primary:{minHeight:58,borderRadius:19,paddingHorizontal:18,backgroundColor:BRAND.colors.primary,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:9,shadowColor:BRAND.colors.primary,shadowOpacity:.22,shadowRadius:14,shadowOffset:{width:0,height:8},elevation:6},primaryText:{color:BRAND.colors.white,fontSize:15,fontWeight:'900'},primaryPressed:{opacity:.87,transform:[{translateY:1}]},primaryDisabled:{opacity:.45}
});
