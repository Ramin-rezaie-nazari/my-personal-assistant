...
const LANGUAGES = [
  ...
  ['es-MX', 'Español — México'], ['fr-FR', 'Français — France'], ['de-DE', 'Deutsch — Deutschland'],
  ['zh-CN', '中文 — 中国'], ['ja-JP', '日本語 — 日本'], ['it-IT', 'Italiano — Italia'],
  ['pt-BR', 'Português — Brasil'], ['ko-KR', '한국어 — 대한민국'], ['ar-SA', 'العربية — السعودية'],
  ['ar-AE', 'العربية — الإمارات'], ['ar-EG', 'العربية — مصر'], ['hi-IN', 'हिन्दी — भारत'],
  ['tr-TR', 'Türkçe — Türkiye'], ['ru-RU', 'Русский — Россия'],
] as const;
const COUNTRIES = [
...
export default function SettingsScreen() {
  ...
  const text = useMemo(() => locale === 'fa' ? {
    title: 'تنظیمات جهانی', subtitle: 'زبان، کشور، واحدها و صدای دستیار را برای خودت تنظیم کن.', language: 'زبان دستیار', country: 'کشور', currency: 'ارز', units: 'سیستم اندازه‌گیری', timezone: 'منطقه زمانی', voice: 'صدای دستیار', save: 'ذخیره تغییرات', saved: 'تنظیمات ذخیره شد', loading: 'در حال بارگذاری…', error: 'بارگذاری تنظیمات ناموفق بود', retry: 'تلاش دوباره', metric: 'متریک', us: 'آمریکایی', uk: 'بریتانیا', custom: 'انتخاب نشده', timezoneHint: 'مثلاً Asia/Tehran', back: 'بازگشت', offline: 'این صدا در معماری آفلاین قابل استفاده است',
  } : {
    title: 'Global settings', subtitle: 'Choose the language, country, units and assistant voice that fit you.', language: 'Assistant language', country: 'Country', currency: 'Currency', units: 'Measurement system', timezone: 'Timezone', voice: 'Assistant voice', save: 'Save changes', saved: 'Settings saved', loading: 'Loading…', error: 'Could not load settings', retry: 'Retry', metric: 'Metric', us: 'US customary', uk: 'UK imperial', custom: 'Not set', timezoneHint: 'For example Europe/Madrid', back: 'Back', offline: 'This voice is supported by the offline architecture',
  } as const, [locale]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSettings(value);
      setDraft({ languageTag: value.languageTag, countryCode: value.countryCode ?? '', currencyCode: value.currencyCode ?? '', measurementSystem: value.measurementSystem, timezone: value.timezone, voiceProfile: value.voiceProfile.id });
    } catch (err) {
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.segmentRow}>
            {([['metric', text.metric], ['us-customary', text.us], ['uk-imperial', text.uk]] as const).map(([value, label]) => {
        ...
        <View style={styles.card}>
          ...
          <TextInput value={draft.timezone} onChangeText={(timezone) => setDraft((current) => ({ ...current, timezone }))} placeholder={text.timezoneHint} placeholderTextColor={BRAND.colors.muted} style={styles.input} autoCapitalize="none" autoCorrect={false} />
        </View>

        {settings?.voiceProfile.offlineCapable ? <View style={styles.note}><Text style={styles.noteDot}>•</Text><Text style={styles.noteText}>{text.offline}</Text></View> : null}
        ...
        <Pressable accessibilityRole="button" disabled={saving} onPress={() => void save()} style={({ pressed }) => [styles.primaryButton, pressed && styles.pressed, saving && styles.disabled]}><Text style={styles.primaryButtonText}>{saving ? '…' : text.save}</Text></Pressable>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  centerText: { marginTop: 12, color: BRAND.colors.muted },
  ...
  backText: { fontSize: 28, color: BRAND.colors.ink, marginTop: -2 },
  headerText: { flex: 1 },
  eyebrow: { fontSize: 11, letterSpacing: 1.5, fontWeight: '700', color: BRAND.colors.primaryStrong, marginBottom: 6 },
  title: { fontSize: 28, fontWeight: '800', color: BRAND.colors.ink },
  subtitle: { marginTop: 6, color: BRAND.colors.muted, lineHeight: 20 },
  ...
  rowTitle: { color: BRAND.colors.ink, fontWeight: '700', fontSize: 16 },
  rowValue: { marginTop: 5, color: BRAND.colors.muted },
  chevron: { color: BRAND.colors.primaryStrong, fontSize: 28, marginLeft: 10 },
  segmentRow: { flexDirection: 'row', gap: 8, marginTop: 12 },
  segment: { flex: 1, paddingVertical: 12, borderRadius: 12, alignItems: 'center', backgroundColor: BRAND.colors.canvas, borderWidth: 1, borderColor: BRAND.colors.border },
  segmentActive: { backgroundColor: BRAND.colors.primarySoft, borderColor: BRAND.colors.primary },
  segmentText: { color: BRAND.colors.muted, fontSize: 12, fontWeight: '700' },
  segmentTextActive: { color: BRAND.colors.primaryStrong },
  input: { marginTop: 12, backgroundColor: BRAND.colors.canvas, borderWidth: 1, borderColor: BRAND.colors.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: BRAND.colors.ink },
  note: { flexDirection: 'row', gap: 7, paddingHorizontal: 4, alignItems: 'center' },
  noteDot: { color: BRAND.colors.primaryStrong, fontSize: 20 },
  noteText: { color: BRAND.colors.muted, flex: 1 },
  inlineError: { padding: 12, borderRadius: 12, backgroundColor: '#FEF2F2' },
  ...
  primaryButtonText: { color: BRAND.colors.white, fontWeight: '800', fontSize: 16 },
  disabled: { opacity: 0.6 },
  pressed: { opacity: 0.82, transform: [{ scale: 0.99 }] },
  errorTitle: { fontSize: 20, fontWeight: '800', color: BRAND.colors.ink },
  errorBody: { marginTop: 8, textAlign: 'center', color: BRAND.colors.muted },
  ...
  modalTitle: { fontSize: 20, fontWeight: '800', color: BRAND.colors.ink },
  close: { fontSize: 28, color: BRAND.colors.muted, paddingHorizontal: 5 },
  ...
  optionText: { color: BRAND.colors.ink, fontSize: 15 },
