...
export default function NotificationsScreen() {
  ...
  const markAllRead = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Unable to clear inbox');
    } finally {
      setBusy(false);
    }
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.titleRow}>
          ...
          {items.length ? <Pressable onPress={() => void markAllRead()} disabled={busy} style={styles.clearButton}><Text style={styles.clearText}>{busy ? 'Clearing…' : 'Mark all read'}</Text></Pressable> : null}
        </View>

        {error ? <View style={styles.card}><Text style={styles.cardTitle}>Inbox notice</Text><Text style={styles.body}>{error}</Text><Pressable onPress={() => void load()} style={styles.button}><Text style={styles.buttonText}>Retry</Text></Pressable></View> : null}
        ...
        {items.map((item) => (
          <View key={item.id} style={styles.card}>
            ...
            <Pressable onPress={() => void read(item.id)} disabled={busy} style={styles.button}><Text style={styles.buttonText}>Mark as read</Text></Pressable>
          </View>
        ))}
      </ScrollView>
    </SafeAreaView>
