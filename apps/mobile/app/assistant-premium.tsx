...
export default function AssistantPremiumScreen() {
  ...
  const startVoice = async () => {
    ...
    recognitionRef.current = await startRecognition(normalizeLocale(locale), ({ transcript, isFinal }) => { transcriptRef.current = transcript; setDraft(transcript); if (isFinal && !submittedRef.current) void submit(transcript, true); }, () => { const transcript = transcriptRef.current.trim(); recognitionRef.current?.remove(); recognitionRef.current = null; if (transcript && !submittedRef.current) void submit(transcript, true); else setVoiceState('idle'); }, (message) => { recognitionRef.current?.remove(); recognitionRef.current = null; setVoiceState('idle'); setError(message || ui.mic); });
