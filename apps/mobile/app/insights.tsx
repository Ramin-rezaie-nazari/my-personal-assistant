...
export default function InsightsScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      setData(await getPersonalInsights());
    } catch (err) {
  ...
  useEffect(() => {
    void hasAuthSession().then((hasSession) => {
      ...
      >
        ...
        {error ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Insights unavailable</Text>
            <Text style={styles.muted}>{error}</Text>
            <Pressable onPress={() => void load()} style={styles.retry}><Text style={styles.retryText}>Retry</Text></Pressable>
          </View>
        ) : null}
        ...
        {data?.insights.map((insight) => (
          <View key={insight.key} style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.icon}>{categoryEmoji[insight.category] ?? '🧠'}</Text>
              <View style={styles.headerCopy}>
                <Text style={styles.cardTitle}>{insight.title}</Text>
                <Text style={styles.score}>Priority {insight.score}</Text>
              </View>
            </View>
        ...
        {data?.insights.length === 0 ? (
          <View style={styles.card}>
            ...
            <Text style={styles.description}>A little more daily activity will give your assistant more signal and better recommendations.</Text>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  score: { color: '#9CA3AF', fontSize: 11, fontWeight: '700', marginTop: 4 },
  description: { color: '#374151', fontSize: 13, lineHeight: 20, marginTop: 13 },
  muted: { color: '#6B7280', fontSize: 12, lineHeight: 18, marginTop: 6 },
  retry: { marginTop: 12, alignSelf: 'flex-start', backgroundColor: '#111827', paddingHorizontal: 12, paddingVertical: 9, borderRadius: 10 },
  retryText: { color: '#FFFFFF', fontWeight: '800' },
  footer: { color: '#9CA3AF', textAlign: 'center', fontSize: 10, marginTop: 8 },
});
