...
const mapHistory = (turns: AssistantHistoryTurn[]): ChatMessage[] => turns.map((turn) => ({ id: turn.id, role: turn.role, text: turn.text, meta: turn.role === 'assistant' && turn.action ? turn.action : undefined }));
...
export default function AssistantScreen() {
  ...
  const send = async () => {
    ...
    try { const response = await sendAssistantMessage(text); const executionMeta = response.execution ? response.execution.executed ? ui.done : ui.understood : null; const meta = [executionMeta, response.intent, typeof response.confidence === 'number' ? `confidence ${Math.round(response.confidence * 100)}%` : null].filter(Boolean).join(' · '); setMessages((current) => [...current, { id: `a-${Date.now()}`, role: 'assistant', text: response.message, meta: meta || undefined }]); }
    catch { setError(ui.error); } finally { setSending(false); }
  };

  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={12}>
    <View style={[styles.header, rtl && styles.rtl]}><Pressable onPress={() => router.back()} style={styles.backButton}><Text style={styles.backText}>{rtl ? '→' : '←'} {ui.back}</Text></Pressable><View style={styles.headerCenter}><Text style={styles.eyebrow}>{rtl ? 'مغز شخصی' : 'PERSONAL BRAIN'}</Text><Text style={styles.title}>{ui.title}</Text></View><View style={styles.brainBadge}><Text style={styles.brainEmoji}>🧠</Text></View></View>
    <View style={[styles.subHeader, rtl && styles.rtl]}><Text style={styles.subtitle}>{ui.subtitle}</Text></View>
    <ScrollView contentContainerStyle={styles.messages} keyboardShouldPersistTaps="handled">{loadingHistory ? <View style={styles.loadingHistory}><ActivityIndicator size="small" /><Text style={styles.meta}>{ui.restoring}</Text></View> : null}{messages.map((message) => <View key={message.id} style={[styles.bubble, message.role === 'user' ? styles.userBubble : styles.assistantBubble, rtl && styles.rtlBubble]}><Text style={[styles.bubbleText, message.role === 'user' ? styles.userText : styles.assistantText, rtl && styles.rtlText]}>{message.text}</Text>{message.meta ? <Text style={[styles.meta, rtl && styles.rtlText]}>{message.meta}</Text> : null}</View>)}{historyNotice ? <View style={styles.noticeCard}><Text style={[styles.noticeText, rtl && styles.rtlText]}>{ui.historyError}</Text></View> : null}{sending ? <View style={[styles.bubble, styles.assistantBubble]}><View style={styles.typing}><ActivityIndicator size="small" /><Text style={styles.meta}>{ui.thinking}</Text></View></View> : null}{error ? <View style={styles.errorCard}><Text style={[styles.errorText, rtl && styles.rtlText]}>{error}</Text></View> : null}</ScrollView>
    <View style={[styles.composer, rtl && styles.rtl]}><TextInput value={draft} onChangeText={setDraft} onSubmitEditing={() => void send()} placeholder={ui.placeholder} placeholderTextColor="#9CA3AF" style={[styles.input, rtl && styles.rtlInput]} multiline maxLength={1000} /><Pressable disabled={!draft.trim() || sending} onPress={() => void send()} style={({ pressed }) => [styles.sendButton, (!draft.trim() || sending) && styles.disabled, pressed && styles.pressed]}><Text style={styles.sendText}>{sending ? '…' : ui.send}</Text></Pressable></View>
