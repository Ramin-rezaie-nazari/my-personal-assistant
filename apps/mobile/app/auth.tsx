...
export default function AuthScreen() {
  ...
  const switchTitle = mode === 'login'
    ? (isFa ? 'حساب نداری؟' : 'Don’t have an account?')
    : (isFa ? 'قبلاً حساب داری؟' : 'Already have an account?');
  const switchAction = mode === 'login'
    ? (isFa ? 'ساخت حساب' : 'Create one')
    : (isFa ? 'ورود' : 'Sign in');
    ...
    <View style={[styles.container, isFa && styles.rtl]}>
      ...
      <Animated.View style={[styles.card,{transform:[{translateY}],opacity:entry}]}>
        ...
        <Pressable onPress={()=>{setMode(mode==='login'?'register':'login');setError(null);}} style={styles.switch}>
          <Text style={styles.switchQuestion}>{switchTitle}</Text>
