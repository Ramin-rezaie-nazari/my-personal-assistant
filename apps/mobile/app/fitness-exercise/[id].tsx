...
import { ExerciseDetail, ExerciseMedia, getExerciseDetail } from '../../lib/fitness-content-api';

const ui = {
  en: { eyebrow: 'EXERCISE', back: 'Back', loading: 'Loading movement…', unavailable: 'Exercise unavailable', retry: 'Try again', watch: 'Watch demonstration', noVideo: 'Video not available yet', instructions: 'How to perform', cues: 'Coach cues', mistakes: 'Common mistakes', cautions: 'Safety notes', equipment: 'Equipment', muscles: 'Primary muscles', alternatives: 'Alternatives', progression: 'Progression', regression: 'Regression', open: 'Open exercise', source: 'Source', noMedia: 'No approved demonstration media yet.' },
  fa: { eyebrow: 'تمرین', back: 'برگشت', loading: 'در حال بارگذاری حرکت…', unavailable: 'تمرین در دسترس نیست', retry: 'تلاش دوباره', watch: 'مشاهده ویدئو', noVideo: 'ویدئو هنوز آماده نیست', instructions: 'روش اجرا', cues: 'نکات مربی', mistakes: 'اشتباهات رایج', cautions: 'نکات ایمنی', equipment: 'تجهیزات', muscles: 'عضلات اصلی', alternatives: 'جایگزین‌ها', progression: 'پیشرفت', regression: 'پسرفت', open: 'باز کردن تمرین', source: 'منبع', noMedia: 'هنوز مدیای تأییدشده‌ای برای این تمرین وجود ندارد.' },
} as const;
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.titleRow, rtl && styles.rtlRow]}>
          <View style={styles.titleCopy}>
            ...
            <Text style={[styles.subtitle, rtl && styles.rtlText]}>{exercise.discipline} · {exercise.difficulty}</Text>
          </View>
          <View style={styles.badge}><Text style={styles.badgeText}>{exercise.videoReady ? 'VIDEO' : 'MEDIA'}</Text></View>
        </View>

        {images.length ? <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={styles.mediaScroller} contentContainerStyle={styles.mediaContent}>
          {images.map((item) => <Image key={item.id} source={{ uri: item.url }} style={styles.heroImage} resizeMode="cover" />)}
        </ScrollView> : <View style={styles.mediaPlaceholder}><Text style={styles.mediaPlaceholderTitle}>{exercise.videoReady ? text.watch : text.noMedia}</Text></View>}

        {videos.length ? <View style={styles.videoCard}>
          <View style={styles.videoCardCopy}><Text style={[styles.videoTitle, rtl && styles.rtlText]}>{text.watch}</Text><Text style={[styles.videoMeta, rtl && styles.rtlText]}>{videos[0].sourceProvider} · {videos[0].durationSeconds ? `${videos[0].durationSeconds}s` : 'video'}</Text></View>
          <Pressable onPress={() => void Linking.openURL(videos[0].url)} style={styles.playButton}><Text style={styles.playText}>▶</Text></Pressable>
        </View> : <View style={styles.noVideo}><Text style={[styles.noVideoText, rtl && styles.rtlText]}>{text.noVideo}</Text></View>}
