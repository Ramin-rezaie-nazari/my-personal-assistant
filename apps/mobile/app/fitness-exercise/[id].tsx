...
function InfoCard({ title, values, rtl, locale }: { title: string; values: string[]; rtl: boolean; locale: import('../../lib/languages').AppLocale }) { return <View style={styles.section}><Text style={[styles.sectionTitle, rtl && styles.rtlText]}>{title}</Text><View style={styles.pillWrap}>{values.map((value) => <FitnessLocalizedText key={value} en={value} locale={locale} style={styles.pillText} />)}</View></View>; }
function TextBlock({ title, body, rtl, locale }: { title: string; body: string; rtl: boolean; locale: import('../../lib/languages').AppLocale }) { return <View style={styles.section}><Text style={[styles.sectionTitle, rtl && styles.rtlText]}>{title}</Text><FitnessLocalizedText en={body} locale={locale} style={{...styles.body, ...(rtl ? styles.rtlText : {})}} /></View>; }
function ListBlock({ title, values, rtl, locale }: { title: string; values: string[]; rtl: boolean; locale: import('../../lib/languages').AppLocale }) { if (!values.length) return null; return <View style={styles.section}><Text style={[styles.sectionTitle, rtl && styles.rtlText]}>{title}</Text>{values.map((value, index) => <View key={`${value}-${index}`} style={[styles.listRow, rtl && styles.rtlRow]}><View style={styles.dot} /><FitnessLocalizedText en={value} locale={locale} style={{...styles.body, ...styles.listText, ...(rtl ? styles.rtlText : {})}} /></View>)}</View>; }
