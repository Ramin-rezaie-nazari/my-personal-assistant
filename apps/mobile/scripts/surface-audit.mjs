...
if (directLocalhost.length) {
  failures.push(`Direct localhost API URLs outside api-base.ts: ${directLocalhost.join(', ')}`);
}

const routerTargets = new Set();
for (const file of routeFiles) {
  const text = fs.readFileSync(file, 'utf8');
  for (const match of text.matchAll(/(?:router\.(?:push|replace)|pathname)\s*[:=(]\s*['"](\/[^'"?}]+)/g)) routerTargets.add(match[1]);
  for (const match of text.matchAll(/router\.(?:push|replace)\(\s*['"](\/[^'"?}]+)/g)) routerTargets.add(match[1]);
}
