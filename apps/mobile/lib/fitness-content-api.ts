...
export function calculateFitness(input: { sex:'male'|'female'; age:number; heightCm:number; weightKg:number; activityFactor?:number; bodyFatPercent?:number; workoutMinutes?:number; workoutCaloriesPerMinute?:number; waterActivityLevel?:'low'|'moderate'|'high'; }) { const q=queryString(input); return request<FitnessCalculatorResult>(`/fitness/calculators?${q}`); }
