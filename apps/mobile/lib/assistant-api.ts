...
import { getStoredAccessToken, getStoredRefreshToken, setAuthSession, type AuthResponse } from './api';
...
class AssistantNetworkError extends Error {
  ...
  constructor(code: AssistantNetworkError['code'], message: string) { super(message); this.name='AssistantNetworkError'; this.code=code; }
}
...
function mergeAbortSignals(externalSignal: AbortSignal|undefined, timeoutMs:number) {
  ...
  return { signal:controller.signal, didTimeout:()=>timedOut, cleanup:()=>{ clearTimeout(timeout); externalSignal?.removeEventListener('abort', abortFromExternal); } };
}

async function refreshAssistantSession(): Promise<string|null> {
  const refreshToken=await getStoredRefreshToken(); if (!refreshToken) return null;
  try {
    ...
    await setAuthSession(auth); return auth.accessToken;
  } catch { return null; }
}
...
async function authorizedFetch(path:string, init:RequestInit={}, timeoutMs=ASSISTANT_REQUEST_TIMEOUT_MS) {
  ...
  try {
    let response=await fetch(`${API_URL}${path}`, {...init, signal:request.signal, headers:{'Content-Type':'application/json', Authorization:`Bearer ${token}`,...(init.headers ?? {})}});
    if (response.status===401) {
      const refreshed=await refreshAssistantSession();
      if (refreshed) response=await fetch(`${API_URL}${path}`, {...init, signal:request.signal, headers:{'Content-Type':'application/json', Authorization:`Bearer ${refreshed}`,...(init.headers ?? {})}});
    }
    return response;
  } catch(error) {
...
async function responseError(response:Response):Promise<AssistantNetworkError> { const body=await response.text(); return new AssistantNetworkError('HTTP', body || `Request failed with ${response.status}`); }
...
export async function getAssistantHistory(limit=24, signal?:AbortSignal):Promise<AssistantHistoryTurn[]> {
  ...
  let data:unknown; try { data=await response.json(); } catch { throw new AssistantNetworkError('INVALID_RESPONSE','ASSISTANT_INVALID_HISTORY_RESPONSE'); }
...
export async function sendAssistantMessage(message:string, signal?:AbortSignal, locale?:string):Promise<AssistantResponse> {
  ...
  const response=await authorizedFetch('/assistant',{method:'POST',signal,body:JSON.stringify({message:normalizedMessage,locale:preferredLocale})});
  if (!response.ok) throw await responseError(response);
  let data:unknown; try { data=await response.json(); } catch { throw new AssistantNetworkError('INVALID_RESPONSE','ASSISTANT_INVALID_RESPONSE'); }
