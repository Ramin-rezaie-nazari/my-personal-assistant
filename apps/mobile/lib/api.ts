...
export type RecipeBudgetItem = { foodId:string;name:string;productKey:string;recommendedQuantity:number;unit:string;price:number|null;estimatedCost:number|null;currency:string;priceObservedAt:string|null;priceSourceId:string|null;status:'priced'|'price_unavailable'|'currency_mismatch'|'unit_mismatch'|'stale_price'|'over_budget';urgency:'critical'|'soon'|'normal'|'none';reason:string };
export type RecipeBudgetPlanResponse = { recipe:{id:string;name:string;baseServings:number;targetServings:number;scaleFactor:number};scaledRecipe:unknown;inventory:{coveragePercent:number;available:Array<{foodId:string;name:string;quantity:number;unit:string}>;missing:Array<{foodId:string;name:string;quantity:number;unit:string}>};shopping:{readyToAdd:Array<{foodId:string;name:string;quantity:number;unit:string}>;source:'recipe'};budget:{budget:number;currency:string;totalEstimatedCost:number;budgetRemaining:number|null;items:RecipeBudgetItem[];status:string;generatedDeterministically:true};localContext:unknown;financeContext:unknown};
async function secureGet(key:string){try{return await SecureStore.getItemAsync(key);}catch{return null}}
async function secureSet(key:string,value:string){await SecureStore.setItemAsync(key,value,{keychainAccessible:SecureStore.WHEN_UNLOCKED_THIS_DEVICE_ONLY})}
async function secureDelete(key:string){try{await SecureStore.deleteItemAsync(key);}catch{}}
export async function setAuthSession(auth:AuthResponse){await Promise.all([secureSet(ACCESS_TOKEN_KEY,auth.accessToken),secureSet(REFRESH_TOKEN_KEY,auth.refreshToken)]);}
export function setAccessToken(token:string){return secureSet(ACCESS_TOKEN_KEY,token)}
export async function getStoredAccessToken(){return (await secureGet(ACCESS_TOKEN_KEY))}
export async function getStoredRefreshToken(){return (await secureGet(REFRESH_TOKEN_KEY))}
export async function hasAuthSession(){const [a,r]=await Promise.all([getStoredAccessToken(),getStoredRefreshToken()]);return Boolean(a||r)}
export async function clearAuthSession(){await Promise.all([secureDelete(ACCESS_TOKEN_KEY),secureDelete(REFRESH_TOKEN_KEY)])}
export function clearAccessToken(){return secureDelete(ACCESS_TOKEN_KEY)}
...
export function getRecipeFoodBudget(recipeId:string,servings:number,budget:number,currency:string,countryCode?:string){const params=new URLSearchParams({servings:String(servings),budget:String(budget),currency:currency.trim().toUpperCase()});if(countryCode?.trim())params.set('countryCode',countryCode.trim().toUpperCase());return request<RecipeBudgetPlanResponse>(`/recipes/${encodeURIComponent(recipeId)}/food-plan/budget?${params.toString()}`)}
export function addBudgetQualifiedRecipeShopping(recipeId:string,servings:number,budget:number,currency:string){const params=new URLSearchParams({servings:String(servings),budget:String(budget),currency:currency.trim().toUpperCase()});return request<{plan:RecipeBudgetPlanResponse;shopping:{recipeId:string;added:number}}>(`/recipes/${encodeURIComponent(recipeId)}/food-plan/budget-shopping?${params.toString()}`,{method:'POST'})}
