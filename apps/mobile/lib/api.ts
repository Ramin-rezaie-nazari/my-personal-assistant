...
export async function hasAuthSession() { const [accessToken, refreshToken] = await AsyncStorage.multiGet([ACCESS_TOKEN_KEY, REFRESH_TOKEN_KEY]); return Boolean(accessToken[1] || refreshToken[1]); }
...
async function rawRequest<T>(path: string, init: RequestInit = {}, token?: string): Promise<Response> { const headers = new Headers(init.headers); headers.set('Content-Type', 'application/json'); if (token) headers.set('Authorization', `Bearer ${token}`); return fetch(`${API_URL}${path}`, { ...init, headers }); }
...
export function register(data: { email: string; password: string; firstName?: string; lastName?: string }) { return request<AuthResponse>('/auth/register', { method: 'POST', body: JSON.stringify(data) }).then(async (auth) => { await setAuthSession(auth); return auth; }); }
export function login(email: string, password: string) { return request<AuthResponse>('/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) }).then(async (auth) => { await setAuthSession(auth); return auth; }); }
...
export function getTodayDashboard(dateKey?: string) { const query = dateKey ? `?dateKey=${encodeURIComponent(dateKey)}` : ''; return request<DashboardResponse>(`/dashboard/today${query}`); }
export function getDashboardOverview(dateKey?: string) { const query = dateKey ? `?dateKey=${encodeURIComponent(dateKey)}` : ''; return request<DashboardOverviewResponse>(`/dashboard/overview${query}`); }
export function getPersonalInsights(dateKey?: string) { const query = dateKey ? `?dateKey=${encodeURIComponent(dateKey)}` : ''; return request<PersonalInsightsResponse>(`/adaptive-learning/insights${query}`); }
export function addWater(amountMl: number, dateKey?: string) { const query = dateKey ? `?dateKey=${encodeURIComponent(dateKey)}` : ''; return request<{ waterMl: number }>(`/daily/water${query}`, { method: 'POST', body: JSON.stringify({ amountMl }) }); }
...
export function getReminders(includeCompleted = false) { const query = includeCompleted ? '?includeCompleted=true' : ''; return request<Reminder[]>(`/reminders${query}`); }
...
export function completeReminder(reminderId: string) { return request<{ completed: true }>(`/reminders/${reminderId}/complete`, { method: 'POST' }); }
export function deleteReminder(reminderId: string) { return request<{ deleted: true }>(`/reminders/${reminderId}`, { method: 'DELETE' }); }
...
export function completeHabit(habitId: string, dateKey?: string) { const query = dateKey ? `?dateKey=${encodeURIComponent(dateKey)}` : ''; return request(`/habits/${habitId}/complete${query}`, { method: 'POST' }); }
export function updateHabit(habitId: string, data: { name?: string; frequency?: 'daily' | 'weekly'; targetPerWeek?: number; active?: boolean }) { return request<Habit>(`/habits/${habitId}`, { method: 'PATCH', body: JSON.stringify(data) }); }
export function deleteHabit(habitId: string) { return request<{ deleted: true }>(`/habits/${habitId}`, { method: 'DELETE' }); }
export function getHabitSummary(dateKey?: string) { const query = dateKey ? `?dateKey=${encodeURIComponent(dateKey)}` : ''; return request<HabitSummary>(`/habits/summary${query}`); }
