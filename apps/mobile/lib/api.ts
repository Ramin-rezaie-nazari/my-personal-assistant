...
export async function setAuthSession(auth:AuthResponse){await Promise.all([SecureStore.setItemAsync(ACCESS_TOKEN_KEY,auth.accessToken),SecureStore.setItemAsync(REFRESH_TOKEN_KEY,auth.refreshToken)])}
export function setAccessToken(token:string){return SecureStore.setItemAsync(ACCESS_TOKEN_KEY,token)}
export function getStoredAccessToken(){return SecureStore.getItemAsync(ACCESS_TOKEN_KEY)}
export function getStoredRefreshToken(){return SecureStore.getItemAsync(REFRESH_TOKEN_KEY)}
...
export async function clearAuthSession(){await Promise.all([SecureStore.deleteItemAsync(ACCESS_TOKEN_KEY),SecureStore.deleteItemAsync(REFRESH_TOKEN_KEY)])}
export function clearAccessToken(){return SecureStore.deleteItemAsync(ACCESS_TOKEN_KEY)}
...
export function apiRequest<T>(path:string,init:RequestInit={}):Promise<T>{return request<T>(path,init)}
