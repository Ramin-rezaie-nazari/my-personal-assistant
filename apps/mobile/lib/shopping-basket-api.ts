...
async function request(path:string, options:RequestInit={}) { const token=await getStoredAccessToken(); const response=await fetch(`${MOBILE_API_URL}${path}`,{...options,headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})}}); if(!response.ok) throw new Error((await response.text())||`Request failed with ${response.status}`); return response.json(); }
