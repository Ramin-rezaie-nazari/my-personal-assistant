...
export function PremiumGlow({ accent = 'primary', size = 220, opacity = 0.24 }: { accent?: 'primary' | 'cyan' | 'mint' | 'amber' | 'rose'; size?: number; opacity?: number }) {
  ...
  return (
    <Animated.View pointerEvents="none" style={[styles.glow, { width: size, height: size, borderRadius: size / 2, opacity, backgroundColor: PREMIUM.colors[accent], transform: [{ scale: pulse }] }]} />
  );
}

const styles = StyleSheet.create({
  glow: { position: 'absolute', filter: undefined as never },
});
