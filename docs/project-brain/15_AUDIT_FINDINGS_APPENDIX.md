...
Evidence: `recipe-api.ts`, `shopping-api.ts`, `shopping-basket-api.ts`, `inventory-api.ts`, and `assistant-api.ts` obtain the stored access token and then issue their own `fetch()`/request helpers without a 401→refresh→retry path. `calendar-api.ts`, `price-api.ts`, and `brain-execution.ts` implement refresh/retry behavior explicitly, while the central `api.ts` also owns token-refresh handling. Impact: after access-token expiry, recipe, shopping, inventory, or assistant actions can fail while other domain surfaces transparently recover, producing inconsistent session behavior. Root cause: multiple domain clients duplicated transport/auth logic instead of using one canonical request interceptor.
...
Evidence: the workflow runs `pnpm recipe:content:import` and `pnpm recipe:content:audit` from `apps/backend`. The audited `apps/backend/package.json` defines recipe image and recipe intelligence scripts but contains no `recipe:content:import` or `recipe:content:audit` entries. Repository search for these exact script names finds only the workflow references, not package definitions. Impact: a manually triggered recipe content release reaches these steps but the requested pnpm commands are undefined, so the intended release/import pipeline cannot execute as declared. This is distinct from PB-188 because PB-188 concerns the importer implementation/schema mismatch; PB-206 concerns the workflow-to-package entrypoint contract itself.
...
Evidence: the mobile onboarding `finish()` path calls only `setOnboardingState({ ...state, completed: true })`, and `setOnboardingState()` persists the full onboarding payload exclusively into AsyncStorage under `@my-personal-assistant/onboarding`. Repository search shows this setter is consumed by the onboarding screen only. The backend separately exposes authenticated onboarding/profile contracts and persists `UserOnboarding`, `UserProfile`, `HealthProfile` and `NutritionProfile`, but no call from the mobile onboarding flow synchronizes those records. Impact: a user can complete onboarding locally while the authenticated backend account remains unaware of the collected goal, diet, gender, body measurements, training preferences and completion state; reinstall/multi-device/account-state flows can therefore diverge and server-side personalization may not receive the onboarding context. This is distinct from PB-182/PB-071 because the core defect here is missing cross-layer synchronization, not merely the local storage mechanism.
