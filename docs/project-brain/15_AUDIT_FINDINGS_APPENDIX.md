...
| PB-267 | CLOSED — REMEDIATED; CI VERIFIED | Budget status semantics could overstate success when only part of a recipe's missing-price evidence was available. `deriveBudgetStatus()` now distinguishes `within_budget`, `over_budget`, `partial_price_evidence`, and `insufficient_price_data`; the Food Operating Loop and mobile consumer preserve the fail-closed meaning. |
...
| PB-270 | CLOSED — REMEDIATED; PRIOR CI VERIFIED | Inventory/recipe quantities could be merged into an existing ShoppingItem by `foodId` without unit compatibility checks, corrupting the numeric meaning when, for example, grams and pieces were combined under one retained unit. Shopping now converts compatible mass/volume/count units into the existing basket unit and rejects incompatible unit kinds. Direct ShoppingService regression tests cover compatible conversion and fail-closed rejection in both manual basket and recipe-missing paths. |
| PB-271 | CLOSED — REMEDIATED; PRIOR CI VERIFIED | Completing a ShoppingItem previously changed only basket state despite the Mobile "Mark as bought" semantics. Completion is now transactional: it marks the owned item completed once, then increments or creates the user's InventoryItem using compatible unit conversion; incompatible units fail closed and the basket write rolls back. Duplicate completion cannot double-count inventory. |
