...
Status: IN_PROGRESS
Completed reads: Mobile domain/auth/branding/calendar/design-system/onboarding/command-action/price/recipe/shopping/inventory clients listed above; backend AppModule/startup/config/database core listed above; BrandMark/BrandWordmark component pair.
Reconciliations: `apps/mobile/lib/brand.ts` appears unused while `apps/mobile/lib/branding.ts` is the documented/consumed branding source; this remains a source-hygiene candidate pending broader repository-wide import closure. `apps/mobile/lib/command-actions.ts` is actively consumed by `command-center-v2.tsx`; its returned user-facing English strings establish PB-218.
Findings added: PB-218.
No production code changed; audit branch contains documentation/catalog updates only.
Unresolved: exhaustive remaining Mobile libraries/providers/tests/assets and exhaustive backend modules/scripts/platform source; full matrices and runtime validation.
