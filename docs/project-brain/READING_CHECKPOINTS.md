...
Status: IN_PROGRESS
Completed reads in this continuation: `apps/mobile/README.md`, `.env.example`, `package.json`, `tsconfig.json`, `app.json`, `app/_layout.tsx`, `app/assistant.tsx`, `app/auth.tsx`, `app/brain-overview.tsx`, `app/calendar.tsx`, `app/command-center-v2.tsx`, `app/daily.tsx`, `lib/api.ts` (source-level), `lib/notifications/push-registration.ts`, `lib/notifications/push-runtime.ts`, `lib/local-persian-tts.ts`, `scripts/start-lan.cjs`; `scripts/prepare-khadijah-tts-model.cjs` was already in the audited scope.
New finding: PB-215.
