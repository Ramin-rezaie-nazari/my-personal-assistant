...
The `recipe-content-audit.mjs` script was found to call the same non-final Prisma delegates (`recipeStep`/`recipeMedia`) implicated by PB-188, so PB-188 scope was broadened to both content operational scripts.
...
`apps/backend/scripts/recipe-media-guaranteed-mirror-v5.mjs` was read in segments; it persists a manifest and processes a bounded dataset prefix, while generated and downloaded media are constrained to 64KB. No new canonical finding was added from this script yet.
Findings added: PB-218, PB-219.
