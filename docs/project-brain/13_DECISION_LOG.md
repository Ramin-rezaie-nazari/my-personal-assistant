...
A work item is marked green only after implementation, relevant automated validation and Project Brain documentation agree. A remediation commit is not itself proof of successful validation. Failed native CI findings must remain represented in the appendix until a fresh successful run closes them.
