...
async function discoverCommons(source, record) {
  ...
  url.search = new URLSearchParams({ action: 'query', format: 'json', generator: 'search', gsrsearch: `${record.name} exercise`, gsrnamespace: '6', gsrlimit: String(maxResults), prop: 'imageinfo', iiprop: 'url|mime|size|sha1|extmetadata', iiextmetadatalanguage: 'en' }).toString();
  ...
  return Object.values(data?.query?.pages ?? {}).flatMap((page) => {
    ...
    return [{ sourceId: source.id, sourceProvider: 'wikimedia_commons', exerciseId: record.exerciseId ?? null, query: record.name, title, url: `https://commons.wikimedia.org/wiki/${encodeURIComponent(page.title).replace(/%2F/g, '/')}`, mediaUrl: info.url, mimeType: info.mime ?? null, bytes: Number(info.size ?? 0) || null, sha1: info.sha1 ?? null, license, licenseClass, licenseUrl: null, creator: creator || null, description: description || null, score, exactMatch: score >= 0.9, rightsGate: ['cc0', 'cc-by', 'public-domain'].includes(licenseClass) ? 'narrow-open-license-candidate' : 'rights-review-required', machineApproved: false }];
  });
}
...
async function discoverWebIndex(source, record) {
  ...
  let domain = String(source.domain ?? '');
  if (!domain && source.url) { try { domain = new URL(source.url).hostname; } catch { domain = ''; } }
  const data = await getJson(fillTemplate(searchEndpoint, { query: `${record.name} exercise video`, domain, limit: String(maxResults) }));
  ...
  return rows.slice(0, maxResults).map((row) => {
    ...
    return { sourceId: source.id, sourceProvider: source.type ?? 'web_index', exerciseId: record.exerciseId ?? null, query: record.name, title, url: sourceUrl, mediaUrl, mimeType: row?.mimeType ?? row?.mime_type ?? null, license, licenseClass, licenseUrl: row?.licenseUrl ?? row?.license_url ?? null, creator: row?.creator ?? row?.author ?? null, description: row?.description ?? null, score, exactMatch: score >= 0.9, rightsGate: row?.licenseUrl && ['cc0', 'cc-by', 'public-domain'].includes(licenseClass) ? 'narrow-open-license-candidate' : 'rights-review-required', machineApproved: false };
  });
}
...
const output = {
  ...
  candidates: [...deduped.values()].sort((a, b) => b.score - a.score), failures,
};
